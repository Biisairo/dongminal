...
> **[`M8_UNIFIED_SRS`](./M8_UNIFIED_SRS.md) 로 대체됐다** (2026-09-13). 본문·§10 검토 메모가 그 문서 §2.3·§3.4·§6.3·§8·§9 로 옮겨 갔고, 병행(훅 유지)·어댑터 계약이 거기서 결정됐다. 이 파일은 실측 기록으로 남긴다 — 고치지 않는다.
...
## 10. 검토 메모 (2026-09-13, 로드맵 M10 편입 시)

초안을 코드와 대조해 읽었다. §2 의 사실은 서 있다 — `NewDetachedTool`·
`SetBackground`·`ForegroundInterval`·`HookParse`·`ScreenPatterns` 전부 실재하고,
`stream-json`·`app-server`·`rpc-ui` 는 저장소 어디에도 없다(그린필드). 아래는
착수 전에 문서를 고쳐야 할 것과, 결정으로 올려야 할 것이다.
...
### 10.2 결정으로 올릴 것 → **결정됨 (2026-09-13)**

**D-1 을 뒤집는다 — 병행(A).** 사용자 결정 2026-09-13. 선택지는 셋이었다: (A) 병행
— 에이전트 도구는 프로토콜, TUI 는 훅 그대로, 묶음 H 폐기 · (B) 초안대로 훅 제거,
TUI 는 idle 추정만(강등) · (C) 훅 제거 + 도구 자체의 알림 채널(BEL·OSC 9)을 L1 로.
**A** 를 택했다: TUI 가 상시 경로(R-j)이므로 그쪽 알람을 약하게 할 수 없고, 훅
체계는 이미 있어 "유지" 는 "신설" 보다 싸며, 한 도구가 한 표면만 갖는 한 두
신호가 겹치지 않는다. C 는 훅을 깔 수 없는 환경(원격 `ssh`·`tmux`, R-d)의 보조
채널 후보로 남긴다 — 실측 뒤 별도 판단.

이 결정으로 **폐기되는 조항**: D-1 · D-2 의 "채택이 강제된다" 는 근거(codex 는
여전히 채택하되 훅 삭제 때문이 아니라 표면이 그것뿐이라서) · FR-AAL-10·11·12·
20·21·22·30 · FR-AHR-1~5 · §6 "터미널 표면의 해상도 회복" 비목표 · R-3 의
"훅 제거를 마지막에" · R-6 · V-4 · V-7 · AS-3. **남는 것**: 묶음 P·T·B 전부,
FR-AAL-1~4(프로토콜의 `done`·승인 요청이 L1 로 들어간다)·FR-AAL-23·24. 참조 SRS
셋(`AGENT_EVENT_ABSTRACTION`·`AGENT_ADAPTER_COMPLETION`·`ATTENTION_FIRING`)은
폐기되지 않고 **터미널 표면의 계약으로 그대로 산다**.


- **D-1 의 재확인.** AS-3("주로 GUI 로 쓴다")이 D-1 을 떠받치는데 R-6 은 그 반대
  사용(여러 탭의 TUI 에이전트 감시)을 이 제품의 이유로 든다. 훅 제거(H)를
  **에이전트 도구가 실제로 쓰인 뒤**로 미루는 완화(M10-c 를 별도 결정으로)를
  권한다. 문서의 결정을 뒤집자는 것이 아니라 **시점**을 조건부로 두자는 것이다.
- **codex app-server 채택(D-2)의 experimental 딱지** — 스키마가 움직이면 어댑터
  한 자리에서 흡수한다(FR-APS-7)고 했는데, 그 흡수 비용을 누가 언제 내는지
  (릴리스 전 대조 잡)를 정한다.

**남은 결정 하나** — codex app-server experimental 딱지의 흡수 비용(릴리스 전
대조 잡)을 누가 언제 내는가. M10-0 스파이크 뒤에 정한다.

### 10.3 M8·M9 와의 관계

- **M8 이 먼저 고쳐야 M10 이 딛는 자리**: `GO-13`(`ToolHub.List` 타입) · `GO-46`
  (데몬 판별을 `ToolHub` 인터페이스로) · `GO-5`(`OnOutput/OnExit` 레이스 — 파이프
  중계가 같은 콜백을 지난다) · `GO-12`(요청 고루틴의 ctx — 열린 승인 요청의 대기)
  · `TEST-8`(sleep 기반 프로세스 테스트 — V-1 이 그 위에 선다) · `FBE-05/12`
  (데몬 kill 유예 — 에이전트 도구의 종료). 이것이 M8 ①~④ 다.
- **M10 이 건드리는 M8 항목**: `FBE-06`(codex 프리앰블)은 터미널 도구의 Run
  멤버에 남으므로 그대로 M8 의 몫이다(R-k 로 `FR-AHR-5` 가 폐기됐다). 다만 Run
  멤버를 에이전트 도구로도 띄울 수 있게 되면 그 경로의 프리앰블은 프로토콜
  입력이다 — M8 ⑤(CLI 계약)를 **M10-a 뒤**에 두면 두 경로를 한 번에 본다.
- **M9 가 먼저면** M10 의 새 UI 가 카탈로그 키로 태어나고(R-f), M10 이 지우는
  UI 문구는 거의 없다(FR-APS-2 — 활동 패널은 그대로). 순서를 바꾸면 M10 의
  문구 100여 개를 M9 가 한 번 더 걷는다.

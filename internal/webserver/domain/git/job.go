...
func (j *Jobs) Start(repo string, kind string, spec WriteSpec) (*Job, error) {
	if strings.TrimSpace(repo) == "" || !filepath.IsAbs(repo) {
		return nil, fmt.Errorf("%w: cwd 는 절대 경로여야 한다: %q", ErrUnsafeArgument, repo)
	}
	if !jobKinds[kind] {
...
func (j *Jobs) run1(ctx context.Context, cancel context.CancelFunc, st *jobState) {
	...
	if errors.Is(ctx.Err(), context.DeadlineExceeded) {
		err = fmt.Errorf("%w: 상한 %v 를 넘겨 종료했다", ErrTimeout, j.ceiling)
	}
...
func (j *Jobs) appendLine(st *jobState, stream, text string) {
	...
	if stream == LineStderr {
		...
		if len(st.stderr) > DefaultStderrTailLines {
			st.stderr = append(st.stderr[:0], st.stderr[len(st.stderr)-DefaultStderrTailLines:]...)
		}
	}
...
func (j *Jobs) finish(st *jobState, exit int, runErr error, dur time.Duration) {
	...
	case runErr != nil:
		st.job.Err = sanitizeRemote(runErr.Error())
	case exit != 0:
	...
	j.svc.recordWrite(snapshot.Repo,
		WriteSpec{Argv: snapshot.Argv, Destructive: st.spec.Destructive, Stdin: st.spec.Stdin},
		Output{Stderr: tail, ExitCode: exit, DurationMs: dur.Milliseconds()}, recErr)
	if j.onDone != nil {
		j.onDone(&snapshot)
	}
}
...
func execStreamGit(ctx context.Context, dir string, args []string, emit func(stream, text string)) (int, error) {
	...
	if err != nil {
		return -1, fmt.Errorf("%w: %v", ErrGitMissing, err)
	}
...
func execStream(ctx context.Context, dir, bin string, args []string, emit func(stream, text string)) (int, error) {
	...
	case errors.Is(ctx.Err(), context.DeadlineExceeded):
		return exit, fmt.Errorf("%w: git %s 가 상한을 넘겨 종료됐다", ErrTimeout, strings.Join(args, " "))
	case ctx.Err() != nil:

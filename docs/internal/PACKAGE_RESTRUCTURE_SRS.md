...
| 4 | ~~`handlers_api.go`(701줄, 메서드 25개) 추가 분할~~ | **16단계에서 철회했다 (§8.12).** 당시 근거(묶음 H 의 3분할로 프로세스·역할 경계는 표현된다)는 지금도 유효하지만, 같은 패키지 안의 파일 분할은 심볼 이동이 아니라 순수 재배치여서 이 작업의 무동작변경 성질을 깨지 않는다 |
...
| 16 | — | 완료 | §8.12 — `handlers_api.go` 4분할. §5 비목표 #4 철회, 선언 37개 바이트 대조 |
...
### 8.12 단계 16: `handlers_api.go` 4분할 — §5 비목표 #4 철회

**비목표로 둔 근거가 틀렸던 것이 아니라, 위험 평가가 달랐다.** §5 는 이 분할을
"프로세스·역할 경계와 무관한 별개 문제"로 보고 뺐다. 그 판단은 지금도 맞다 — 이번
분할로 경계가 새로 표현되는 것은 없다. 다만 `*Server` 메서드는 같은 패키지를 벗어날
수 없으므로 **이 작업은 애초에 심볼 이동이 아니라 파일 재배치**다. 묶음 H·I 에서
비용을 만든 것들(export 승격, 리시버 교체, 자유 함수 전환)이 하나도 발생하지 않는다.

**4분할의 경계는 "무엇을 손볼 때 함께 봐야 하는가"다.**

| 파일 | 담은 것 | 경계의 근거 |
|---|---:|---|
| `handlers_api.go` | 262줄 — 라우트 테이블·`handleAPI`·상태·도구·워크스페이스 | 라우팅과 디스패치가 파일 중간에 묻혀 있었다 |
| `handlers_files.go` | 196줄 — 업로드·다운로드·읽기·쓰기·`cwd` + `safeResolve`·`uniquePath` | **경로를 사용자 입력에서 받는 유일한 면이다.** 경로 가드를 손볼 때 봐야 할 것이 한 파일에 모인다 |
| `handlers_attention.go` | 172줄 — 주의·활동·배경 종단 8개 | 셋은 직교하는 레이어지만 같은 `AttnTracker` 상태를 읽는다 |
| `handlers_settings.go` | 77줄 — `settingsStore` + 종단 2개 | 서버가 해석하지 않는 JSON blob 이라 다른 핸들러와 공유 상태가 없다 |

**검증은 바이트 단위 대조다** (묶음 J 의 TC-APP-1 과 같은 방법). 분할 전후로 파일의
top-level 선언을 전부 추출해 이름과 본문을 비교했다 — **37개 전부 보존, 본문 차이
0건.** 순수 재배치임을 이 대조가 보증한다. 줄 수 합계가 672 → 707 로 는 것은 파일별
헤더 주석과 `import` 블록 중복 때문이다.

**`goimports` 가 없어 import 는 코드에서 추정하고 컴파일러로 확인했다.** 그 추정이
주석 안의 `never errors.` 를 `errors` 패키지 사용으로 오인해 미사용 import 를 하나
넣었고, `go build` 가 그 자리에서 잡았다. 컴파일러가 안전망이라는 것이 여기서도
성립한다.

...
> **개정 (2026-09-11) — `FR-CTX-5b`.** 판정의 규칙과 자리가 모두 바뀌었다.
>
> **① `[1m]` 접미어 규칙은 폐기됐다** (사용자 결정). 아래 실측이 접미어를 긴 판의
> 표식으로 읽게 했으나, 그 읽기가 틀렸다 — `claude-opus-5` 는 **접미어 없이도
> 1M 이다.** 접미어는 1M 의 *조건*이 아니라 *예외 표기*였고, 조건으로 읽으면 붙지
> 않은 97045건을 전부 200k 로 세게 된다. 접수된 결함("1M 을 쓰는데 15%를 70%로
> 센다")이 바로 그 자리였다.
>
> **② 기본이 1M 이다.** 요즘 판의 기본이 1M 이므로 그것을 기본으로 두고, **다른
> 크기인 판만 따로 적는다**(`claudeModelWindows`). 그래서 모델 이름을 얻지 못한
> 관측도 1M 으로 답한다 — 모름으로 두면 정책 기본값(200k)에서 출발해 관측이 넘을
> 때까지 잘못된 비율을 보이기 때문이다.
>
> **③ 판정은 어댑터가 한다** (`AGENT_ADAPTER_COMPLETION_SRS FR-AAC-20·21`).
> 모델 이름의 뜻은 에이전트마다 다르므로 `domain/run` 이 알 일이 아니다. 관측이
> `agent` 를 함께 실어 오고(`FR-AAC-23`), 창을 말하지 않는 에이전트에서는 종전의
> "모름" 경로(`FR-CTX-6·7`)가 그대로 받는다.
>
> 아래 실측은 **그대로 둔다** — 숫자는 여전히 사실이고, 그 사실에서 접미어를
> 조건으로 읽은 것이 틀렸다는 것이 이 개정의 내용이다.
...
> **개정 (2026-09-11, `API_ANSWER_NOT_ABSENCE_SRS FR-ANA-1`).** 판정을 **200 응답의
> `isRepo:false`** 로 받는다. 종전에는 `404 not_a_git_repo` 였다.
>
> 멈추는 동작도 되살아나는 길 셋도 그대로다. 바뀐 것은 전송 계층의 표기뿐이며,
> 이유는 위의 세 번째 길에 있다 — `signal()` 이 포커스마다 한 번 묻기 때문에,
> 저장소가 아닌 루트를 띄워 둔 채 창을 오갈수록 **콘솔에 오류가 한 줄씩 쌓였다.**
> 요청은 옳으므로 답의 형식을 고쳤다.
...
> **개정 (UX_BATCH8_SRS FR-MMP-2, 2026-09-08).** 값이 `'fill'` 로 바뀌었다.
> `'fit'` 은 **줄이기만 하므로** 문서가 편집기보다 짧으면 아무것도 하지 않고, 그때
> 두 슬라이더가 다시 갈라선다 (실측: 400줄·높이 1180 의 45% 지점에서 미니맵
> `top 311` · 스크롤바 `top 454`). 여기 적힌 800줄 실측은 그대로 옳으며 `'fill'`
> 에서도 같은 결과다 — 바뀐 것은 **덮는 구간**이다.
...
| V-CTX-3 | FR-CTX-5b | Go 단위 — 기본이 1M 이고, 예외 표에 오른 판만 다른 값이다. 모델 이름을 얻지 못한 관측도 기본으로 답한다. ~~`[1m]` 접미어~~ 는 근거가 아니다 |

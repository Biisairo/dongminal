...
1~20단계와 미구현이던 P0 2건(FR-GIT-141·144)이 모두 끝났다. 이 계약 문서들은
**살아 있는 코드의 설계 근거**이므로 아직 `archive/` 로 옮기지 않았다. 트랙이
닫히면 그때 옮긴다.

> ⚠️ **개정 주의.** MVP 이후 사용자 검토로
> [`../GIT_UI_REVISION_SRS.md`](../GIT_UI_REVISION_SRS.md) (FR-GIT-179~213)가
> 섰다. **`GIT_M1_STEP3_CONTRACT.md`(묶음 D, Git 창 골격)의 분할·창 목록 관련
> 서술은 낡았다** — FR-GIT-27·30 이 폐기됐다. 계약 문서보다 개정 SRS 가 앞선다.

아직 끝나지 않은 것은 [`../GIT_REMAINING.md`](../GIT_REMAINING.md) 에 있다 —
결함 2건(G5.9·G5.11), 로그 위생 1건, 21단계 수동 검증의 남은 항목, 문서 정리.
...
- Playwright 기준선: **187** (Git 작업 착수 시점) → **371** (2026-08-26). 각 단계의
...
**전체 실행에서 0~1건이 간헐적으로 실패한다.** 관측된 것:

...
**식별자 형식**: 모든 tool 의 식별자 인자 (`id`/`to`/`from`/`location`) 는 `uuid` / `toolId` / 라벨 어느 형식이든 받지만, **이 스킬에서는 항상 uuid 사용**. 서버가 uuid 를 자동 인식해 라우팅·broadcast 한다. 라벨은 디버그 출력에만.
...
who_am_i  →  출력 라인 끝의  uuid=<36자>  를 BOSS 로 캡처
              앞부분의 label=<W?.P?.T?> 와 size=COLSxROWS 도 같이 추출
```
...
`--boss` 인자는 uuid·toolId·라벨 어느 것이든 받지만, 권장은 uuid (`scripts/plan_layout.py --help` 참고). 출력 JSON 의 `primary_split` / `orthogonal_split` 지시를 그대로 따른다. 단 `location` 자리에 `BOSS_UUID` 를 넣는다.
...
단일 호출로 정확히 N 균등 분할. 다시 `list_workspace` 로 **팀원 N 명의 uuid 전부 수집**.

> 팁: `list_workspace` 결과의 각 행 끝에 `uuid=<full-uuid>  short=<8자>` 가 붙는다. short 는 로그 가독성용 별칭 (`uuid` 의 prefix 8자). 라우팅·인자 전달에는 항상 full uuid 사용.
...
1. 4단계 병렬 `send_input` 직후, **첫 `read_screen` 전 최소 8초 대기를 명시 도구 호출로** 삽입:
   - `Bash(command="sleep 8", description="CC 부팅 대기")` — 가장 단순
   - 또는 다른 유의미한 동시 작업이 있으면 그걸로 8초+ 채워도 됨
2. 대기 후 모든 팀원에게 병렬 `read_screen(id=<팀원_UUID>)`. 준비 완료 조건 (모두 충족):
   - `╭─` / `>` 프롬프트 박스 노출
   - 화면에 `Thinking...` 부재
...
초기 프롬프트에 이미 역할·프로토콜이 있으므로 kickoff 메시지는 짧아도 된다. 송신 후 `Bash(sleep 2)` → `read_screen(id=<개시자_UUID>)` 으로 수신측이 처리 시작(`Thinking...`)했는지 확인. `Thinking...` 미관측 시 `send_input(id=<개시자_UUID>, text="", execute=true)` 로 엔터 보강 후 재확인 (TUI reconciliation 지연 대비, troubleshooting 14행). 이 확인까지가 같은 턴에서 끝나야 한다 — 그 다음에야 7단계(턴 종료) 로 진행.
...
3. [ ] 1차 분할 `location=<BOSS_UUID>` → `list_workspace` → SEED **uuid** 확인
4. [ ] (N≥2) 직교 축 `count=N` 단일 호출 `location=<SEED_UUID>` → `list_workspace` → 팀원 **uuid** 들 확보
...
7. [ ] **같은 턴 안에서** `Bash(sleep 8)` → Barrier `read_screen(id=<팀원_UUID>)` (준비 fingerprint: `╭─` + `Thinking...` 부재 + `[대기]` 텍스트). 미준비면 `sleep 3` → 재확인 최대 10회
8. [ ] **같은 턴 안에서** `send_agent_message(to=<UUID>, from=<BOSS_UUID>)` Kickoff → `Bash(sleep 2)` → `read_screen(id=<UUID>)` 으로 `Thinking...` 확인
...
브라우저에서 팀 활동 실시간 관찰, 신뢰 채널 명시, 레이아웃이 터미널 비율에 맞춰 자동 조정, **식별자가 uuid 라 창 닫힘에 따른 라벨 reflow 무관**.

...
func TestResolve_CheckoutThenAdd(t *testing.T) {
	...
	if _, err := s.Resolve(context.Background(), "/tmp/repo", ResolveOurs, Paths{"a.txt", "d ir/b.txt"}); err != nil {
	...
	hints := s.Hints(0)
	if len(hints) != 1 || hints[0].Action != ActionResolveSide {
...
func TestResolve_TheirsSide(t *testing.T) {
	...
	if _, err := s.Resolve(context.Background(), "/tmp/repo", ResolveTheirs, Paths{"a.txt"}); err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if argvs[0][1] != "--theirs" {
...
func TestResolve_CheckoutIsDeclaredDestructive(t *testing.T) {
	...
	if _, err := s.Resolve(context.Background(), "/tmp/repo", ResolveOurs, Paths{"a.txt"}); err != nil {
		t.Fatalf("Resolve: %v", err)
	}
...
func TestResolve_RejectsBadSideAndEmptyPaths(t *testing.T) {
	s := New(WithRunner(headRunner), WithWriteRunner(func(_ context.Context, _ string, _ []string, _ string) (Output, error) {
		return Output{}, nil
	}))
	if _, err := s.Resolve(context.Background(), "/tmp/repo", "mine", Paths{"a.txt"}); !errors.Is(err, ErrUnsafeArgument) {
		t.Fatalf("알 수 없는 side = %v, want ErrUnsafeArgument", err)
	}
	if _, err := s.Resolve(context.Background(), "/tmp/repo", ResolveOurs, nil); !errors.Is(err, ErrUnsafeArgument) {
...
func TestResolve_StopsWhenCheckoutFails(t *testing.T) {
	...
	s := New(WithRunner(headRunner), WithWriteRunner(func(_ context.Context, _ string, args []string, _ string) (Output, error) {
		argvs = append(argvs, append([]string(nil), args...))
		if args[0] == "checkout" {
			return Output{ExitCode: 1, Stderr: "fatal: 실패\n"}, nil
		}
		return Output{}, nil
	}))
	if _, err := s.Resolve(context.Background(), "/tmp/repo", ResolveOurs, Paths{"a.txt"}); err == nil {
...
func TestResolveSide_IsDestructiveAction(t *testing.T) {
	for _, a := range DestructiveActions {
		if a == ActionResolveSide {
			return
		}
	}
	t.Fatalf("%q 가 DestructiveActions 에 없다", ActionResolveSide)
}

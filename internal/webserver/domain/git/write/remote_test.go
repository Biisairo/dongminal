...
func (f remoteFake) read(_ context.Context, _ string, args []string) (core.Output, error) {
	switch args[0] {
	case "status":
...
func remoteSvc(f remoteFake) *core.Service {
	return core.New(
...
func TestPushSpec_PublishSetsUpstream(t *testing.T) {
	...
	spec, plan, err := PushSpec(svc, context.Background(), remoteRepo, PushOpts{Publish: true})
	if err != nil {
...
func TestPushSpec_PublishNeedsAcknowledgement(t *testing.T) {
	svc := remoteSvc(remoteFake{branch: "no-upstream", config: "remote.origin.url=/tmp/remote.git\n"})
	_, plan, err := PushSpec(svc, context.Background(), remoteRepo, PushOpts{})
...
func TestPushSpec_WithUpstreamIsPlain(t *testing.T) {
	...
	spec, plan, err := PushSpec(svc, context.Background(), remoteRepo, PushOpts{})
	if err != nil {
		t.Fatalf("PushSpec: %v", err)
	}
...
func TestPushSpec_DetachedIsRejected(t *testing.T) {
	...
	if _, _, err := PushSpec(svc, context.Background(), remoteRepo, PushOpts{Publish: true}); !errors.Is(err, ErrDetachedPush) {
...
func TestPushSpec_ForceDefaults(t *testing.T) {
	...
	base, _, err := PushSpec(svc, ctx, remoteRepo, PushOpts{})
	if err != nil {
		t.Fatalf("기본: %v", err)
	}
	...
	lease, plan, err := PushSpec(svc, ctx, remoteRepo, PushOpts{Force: PushLease})
	if err != nil {
	...
	if _, _, err := PushSpec(svc, ctx, remoteRepo, PushOpts{Force: PushForce}); !errors.Is(err, ErrForceConfirm) {
		t.Fatalf("확인 없는 --force err = %v, want ErrForceConfirm", err)
	}

	forced, _, err := PushSpec(svc, ctx, remoteRepo, PushOpts{Force: PushForce, Confirm: true})
	if err != nil {
	...
	if _, _, err := PushSpec(svc, ctx, remoteRepo, PushOpts{Force: "hard"}); !errors.Is(err, ErrPushForce) {
...
func TestRemoteSpecs_PassWriteGuard(t *testing.T) {
	...
	push, _, err := PushSpec(svc, context.Background(), remoteRepo, PushOpts{Publish: true, Force: PushLease})
	if err != nil {
		t.Fatalf("PushSpec: %v", err)
	}
	...
	for _, spec := range []core.WriteSpec{FetchSpec(FetchOpts{Prune: true, Tags: &yes}), pull, push} {
		if gerr := core.GuardWriteArgs(spec.Argv); gerr != nil {

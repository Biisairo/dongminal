...
func StashRef(index int) (string, error) {
	if index < 0 {
		return "", fmt.Errorf("%w: stash 인덱스는 0 이상이어야 한다: %d", ErrUnsafeArgument, index)
	}
...
func (s *Service) StashList(ctx context.Context, repo string) ([]Stash, error) {
	out, err := s.ExecWrite(ctx, repo, WriteSpec{Argv: []string{"stash", "list", stashFormat}})
	if err != nil {
		return nil, err
	}
...
func ParseStashList(out string) ([]Stash, error) {
	...
	for _, rec := range strings.Split(out, stashRecSep) {
		...
		st := Stash{Index: idx, Oid: f[1], AtUnixMs: unixSecToMilli(f[3])}
...
func StashableCount(st Status, includeUntracked bool) int {
...
func StashEmptyReason(st Status, includeUntracked bool) string {
...
func (s *Service) StashApply(ctx context.Context, repo string, index int, withIndex bool) (Output, error) {
	return s.stashRestore(ctx, repo, "apply", index, withIndex)
}
...
func (s *Service) StashPop(ctx context.Context, repo string, index int, withIndex bool) (Output, error) {
	return s.stashRestore(ctx, repo, "pop", index, withIndex)
}
...
func (s *Service) StashPopChecked(ctx context.Context, repo string, index int, withIndex bool) (Output, StashPopKept, error) {
	...
	out, popErr := s.StashPop(ctx, repo, index, withIndex)
	after, listErr := s.StashList(ctx, repo)
	if listErr != nil {
...
func (s *Service) StashDrop(ctx context.Context, repo string, index int) (Output, error) {
	...
	return s.ExecWrite(ctx, repo, WriteSpec{Argv: []string{"stash", "drop", ref}, Destructive: true})
}
...
func (s *Service) StashPreview(ctx context.Context, repo string, index int) ([]CommitFile, error) {
	...
	out, err := s.ExecWrite(ctx, repo, WriteSpec{Argv: []string{"stash", "show", stashNameStatusFlags, "-z", ref}})
	if err != nil {
		return nil, err
	}
	if out.StdoutTruncated {
	...
	return ParseNameStatusZ(out.Stdout)
}

// stashRestore 는 apply/pop 의 공통 argv 다. 둘은 stash 를 지우는지만 다르다.
func (s *Service) stashRestore(ctx context.Context, repo, sub string, index int, withIndex bool) (Output, error) {
	ref, err := StashRef(index)
	if err != nil {
		return denied(), err
	}
	...
	return s.ExecWrite(ctx, repo, WriteSpec{Argv: append(argv, ref)})
}
...
func stashDropHint(repo string, st Stash) Hint {
	return Hint{
		Repo:    repo,
		Action:  ActionStashDrop,

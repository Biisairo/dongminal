...
> **개정 2026-09-22 (`UIUX_OVERHAUL_SRS` FR-CHR-15·18, D-7).** **짝이던 버튼
> 둘(`#bg-btn`·`#runs-btn`)이 없어졌다.** `Activity` 문 하나만 남는다.
>
> **키는 한 줄도 바뀌지 않는다** (NFR-4) — `bgToggle`(`Ctrl+Shift+B`) ·
> `runsToggle`(`Ctrl+Shift+O`) · `agentsToggle`(`Ctrl+Shift+A`) 셋이 그대로 살고,
> 앞의 둘은 패널을 열어 **그 구역으로 스크롤한다** (`UIUX_OVERHAUL_SRS` FR-ACT-4).
> 버튼이 없어졌다고 외운 키를 뺏지 않는다. **오히려 이제 그 구역을 직접 부르는
> 유일한 길이다.**
>
> · **FR-PSC-3**(*"버튼 클릭과 같은 함수를 부른다"*) — 부를 버튼이 없으므로
>   **`actPanelOpen(key)` 한 자리를 부른다**로 읽는다. 여는 길이 둘로 갈리지
>   않는다는 요구는 약해진 것이 아니라 **강해졌다**
> · **FR-PSC-4**(토글이다) — 그대로다. 같은 구역을 다시 부르면 패널이 닫힌다
>   (`actPanelOpen` 의 `_actLastKey`)
> · **FR-PSC-5**(설정 ▸ Shortcuts 의 한 무리) — *"상단 툴바의 진입점 셋"* 이라는
>   무리의 **이름만** 낡았다. 셋은 한 무리로 남고 차례도 그대로다 — 이제 *"활동
>   패널의 세 구역"* 이다

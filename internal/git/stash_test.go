...
func TestStashPush_Args(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			f := &writeFake{}
			s := New(WithRunner(stashDirtyReader), WithWriteRunner(f.runner))
			if _, err := s.StashPush(ctx, "/tmp/repo", c.o); err != nil {
...
func TestStashPush_RefusesWhenNothingToSave(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			f := &writeFake{}
			s := New(WithRunner(c.read), WithWriteRunner(f.runner))
			_, err := s.StashPush(ctx, "/tmp/repo", c.opts)
			if c.refuse {
...
func TestStashApplyPop_Args(t *testing.T) {
	...
	cases := []struct {
		...
		{"apply", func(s *Service) error { _, err := s.StashApply(ctx, repo, 1, false); return err }, []string{"stash", "apply", "stash@{1}"}},
		{"apply --index", func(s *Service) error { _, err := s.StashApply(ctx, repo, 0, true); return err }, []string{"stash", "apply", "--index", "stash@{0}"}},
		{"pop", func(s *Service) error { _, err := s.StashPop(ctx, repo, 0, false); return err }, []string{"stash", "pop", "stash@{0}"}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			f := &writeFake{}
...
func TestStashRef_RejectsNegative(t *testing.T) {
	if _, err := StashRef(-1); !errors.Is(err, ErrUnsafeArgument) {
...
func TestStashPopChecked_KeepsStashOnConflict(t *testing.T) {
	...
	before, err := s.StashList(ctx, repo)
	if err != nil {
	...
	_, kept, popErr := s.StashPopChecked(ctx, repo, 0, false)
	if popErr == nil {
	...
	after, err := s.StashList(ctx, repo)
	if err != nil {
		t.Fatalf("StashList: %v", err)
	}
...
func TestStashPopChecked_DropsStashOnSuccess(t *testing.T) {
	...
	_, kept, err := s.StashPopChecked(ctx, repo, 0, false)
	if err != nil {
...
func TestStashDrop_HintBeforeExecuting(t *testing.T) {
	...
	if _, err := s.StashDrop(ctx, repo, 0); err != nil {
	...
	h := hints[0]
	if h.Action != ActionStashDrop || h.Repo != repo {
		t.Fatalf("hint = %+v", h)
	}
...
func TestStashDrop_MissingIndex(t *testing.T) {
	...
	if _, err := s.StashDrop(context.Background(), repo, 9); !errors.Is(err, ErrStashNotFound) {
...
func TestStashPreview_ReusesNameStatusParser(t *testing.T) {
	...
	files, err := s.StashPreview(context.Background(), repo, 0)
	if err != nil {
...
func newStashWriteFake(t *testing.T) *stashWriteFake {
	t.Helper()
	gitPath(t)
	return &stashWriteFake{real: New()}
}

func (f *stashWriteFake) runner(ctx context.Context, dir string, args []string, _ string) (Output, error) {
	if len(args) > 1 && (args[1] == "list" || args[1] == "show") {
		return f.real.ExecWrite(ctx, dir, WriteSpec{Argv: args})
	}
...
// 격리되지 않는다.
func stashStatusReader(recs ...string) Runner {
	out := strings.Join(append([]string{
		"# branch.oid " + strings.Repeat("a", 40),
		"# branch.head main",
	}, recs...), "\x00") + "\x00"
	return func(_ context.Context, _ string, args []string) (Output, error) {
		if args[0] == "status" {
			return Output{Stdout: out}, nil
		}
		return Output{}, nil
	}
}

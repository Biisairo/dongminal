...
**FR-STA-9 (동시 대기 상한)** 서버가 동시에 붙잡는 대기는 **32건**이다
(2026-09-11 사용자 판정). 넘으면 **429** 이고 본문에 사유를 싣는다.

> **왜 상한이 필요한가.** 대기 하나가 서버를 최대 **30분** 붙잡고(FR-STA-2),
> 그 동안 고루틴 하나와 1초 주기 liveness 조회를 쓴다. 상한이 없으면 스크립트
> 하나가 대기를 쌓아 그 자원을 전부 가져갈 수 있다 — 그리고 그것은 공격이 아니라
> **루프가 잘못 도는 오케스트레이션**으로도 일어난다.
>
> 값이 32 인 것은 실제 도구 수보다 넉넉하면서 폭주는 막는 자리이기 때문이다.
> 이 값은 `diag` 의 `hold` 임계와 같다 (`FR-CNR-13`) — 그 선을 치면 거절이
> 시작된다는 뜻이고, 그때 로그가 그 사실을 함께 말한다.

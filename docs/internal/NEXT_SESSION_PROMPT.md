...
**M1·M2 완료. Go 측(M1~M5 전 서버 API) 완료.** 남은 것은 전부 프론트엔드다. 기준선 — `go test ./... -race` 전부 통과 ·
gofmt clean · Playwright **187 → 275개 중 274 통과** (1건은 Git 무관 `background-restore-at` TC-BGR-9 플레이크).
...
| **M3** | | | |
| 12·13 | job 인프라 + fetch/pull/push — 서버 완료, **클라 남음** | 부분 | `ef45143` |
| **M4** | | | |
| 14 | 로그·커밋 상세·refs 조회 | ✅ | `a036af5` |
| 15 | 레인 알고리즘 (UI 보다 먼저 고정) | ✅ | `6a01d4d` |
| — | diff 축 `commit-parent` (17단계가 딛는다) | ✅ | `90d93a4` `5b649df` |
| 16·17 | History 탭 + 커밋 상세 + 컨텍스트 메뉴 프레임워크 | 진행 | |
| **M5** | | | |
| 18·19 | Branches · Stash — 서버 완료, **클라 남음** | 부분 | `c9f8134` |

...
// Preflight 는 커밋 실행 전 검사다 (FR-GIT-86~88).
//
...
func (s *Service) Preflight(ctx context.Context, repo string) (Preflight, error) {
	...
	name, err := s.configGet(ctx, repo, configUserName)
	if err != nil {
		return Preflight{}, err
	}
	email, err := s.configGet(ctx, repo, configUserEmail)
	if err != nil {
		return Preflight{}, err
	}
	...
	sign, err := s.configGet(ctx, repo, configGPGSign)
	if err != nil {
		return Preflight{}, err
	}
	pf.GPGSign = gitBool(sign)

	tmpl, err := s.configGet(ctx, repo, configCommitTemplate)
	if err != nil {
		return Preflight{}, err
	}
...
func (s *Service) configGet(ctx context.Context, repo, key string) (string, error) {
	out, err := s.Exec(ctx, repo, "config", "--get", key)
	if err != nil {
		var xe *ExecError
		if errors.As(err, &xe) && xe.kind == nil && xe.ExitCode == configUnsetExit {
			return "", nil
		}
		return "", err
	}

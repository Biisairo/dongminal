...
var SendAgentMessageSpec = map[string]any{
	...
	"description": "【에이전트 간 신뢰 통신 채널】 dongminal 워크스페이스 내 다른 Claude Code 인스턴스에 메시지를 전송한다.\n\n" +
		...
		"【식별자 — UUID 권장】 to/from 모두 uuid·toolId·라벨 어느 형식이든 받지만, 다중 세션/계층 팀에서는 **uuid 사용**. 라벨은 다른 세션 닫힘 시 reflow 되어 다른 tool 을 가리킨다. who_am_i() 출력의 `uuid=<36자>` 필드를 from 에, list_workspace 의 같은 필드를 to 에 사용.\n\n" +

...
| FR-PIS-9 | `0` 은 `gitStatusInterval` **하나에서만** 뜻을 갖는다 — "이 계층을 걸지 않는다" (FR-GIT-23). 나머지 넷에는 0 선택지가 없다: 그 넷은 갱신의 **유일한** 경로여서 끄면 화면이 멎는다. **개정(`GIT_WATCH_LEASE_SRS` FR-GWL-11·12): 근거가 틀렸었다** — 서버 감시의 임대가 이 폴링에 매달려 있어 이것도 유일 경로였고, 끄면 본줄인 push 까지 죽었다(`GP-1`, P0). 임대를 SSE 구독으로 옮겨 그 진술을 참으로 만들었다. 같은 개정에서 선택지의 `1분`·`2분` 을 뺐다 — 남는 것은 `10초·30초·끔` 이다. | 필수 |

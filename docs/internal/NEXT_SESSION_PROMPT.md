...
**M1·M2·M4 완료. Go 측(M1~M5 전 서버 API) 완료.** **M5 도 18·19 까지 완료.** 남은 것은 M3 클라(진행) · 20단계(다이얼로그 규약) · 21단계(수동 검증) 뿐이다. 기준선 — `go test ./... -race` 전부 통과 ·
gofmt clean · Playwright **187 → 331**.
...
| **M3** | | | |
| 12·13 | job 인프라 + fetch/pull/push — 서버 완료, **클라 진행** | 진행 | `ef45143` |
| **M4** | | | |

...
- **비활성 탭의 DOM 은 떼어진다**(`_hideOthers`). "탭이 남았는가" 를 요소의 존재로 재면 틀린다 —
  돌아가서 본다
- **꼬리에서 이름 둘을 넘긴다** (D-M9-11 — 붙잡지 않되 **세는 일은 그만두지 않는다**):
  **`git-changes` V78**(P7 의 ①②회차에 **연속**으로 나왔다 — 다음에 또 나오면 결함) ·
  **`git-worktrees` V169**(P3 ③회차가 이름을 박아 둔 넷 중 하나이고 ②에서 나왔다).
  P7 세 회차의 flaky 열 건 중 겹치는 이름은 앞의 하나뿐이다

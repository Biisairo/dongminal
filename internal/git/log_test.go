...
func (f *logFake) run(_ context.Context, _ string, args []string) (Output, error) {
	f.argvs = append(f.argvs, append([]string(nil), args...))
	return Output{Stdout: f.stdout, Stderr: f.stderr, ExitCode: f.exit, StdoutTruncated: f.truncated}, nil
}
...
func TestLog_Argv(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			q := tc.q
			q.Repo = "/repo"
			if _, err := New(WithRunner(f.run)).Log(context.Background(), q); err != nil {
				t.Fatalf("Log: %v", err)
			}
...
func TestLog_RejectsUnknownOrder(t *testing.T) {
	f := &logFake{}
	_, err := New(WithRunner(f.run)).Log(context.Background(), LogQuery{Repo: "/repo", Order: "reverse"})
...
func TestLog_RejectsOptionLikeRef(t *testing.T) {
	f := &logFake{}
	_, err := New(WithRunner(f.run)).Log(context.Background(), LogQuery{Repo: "/repo", Ref: "--all"})
...
func TestLog_TruncatedOutputIsError(t *testing.T) {
	...
	if _, err := New(WithRunner(f.run)).Log(context.Background(), LogQuery{Repo: "/repo"}); err == nil {
		t.Fatal("오류가 아니다")
	}
}
...
func TestLog_UnknownRevIsNotFound(t *testing.T) {
	...
	_, err := New(WithRunner(f.run)).Log(context.Background(), LogQuery{Repo: "/repo", Ref: "nope"})
...
func TestLog_RealGit(t *testing.T) {
	repo := logRealRepo(t)
	cs, err := New().Log(context.Background(), LogQuery{Repo: repo, Order: LogOrderTopo})
	if err != nil {
		t.Fatalf("Log: %v", err)
	}
	if len(cs) != 5 {
...
func TestLog_RealGit_Paging(t *testing.T) {
	...
	all, err := s.Log(ctx, LogQuery{Repo: repo, Order: LogOrderTopo})
	if err != nil {
		t.Fatalf("Log: %v", err)
	}
	page, err := s.Log(ctx, LogQuery{Repo: repo, Order: LogOrderTopo, Skip: 2, Limit: 2})
	if err != nil {
...
func TestLog_RealGit_Filters(t *testing.T) {
	...
	onSide, err := s.Log(ctx, LogQuery{Repo: repo, Ref: "refs/heads/side"})
	if err != nil {
	...
	byPath, err := s.Log(ctx, LogQuery{Repo: repo, Path: "d ir/한글 파일.txt"})
	if err != nil {
	...
	byAuthor, err := s.Log(ctx, LogQuery{Repo: repo, Author: "nobody-such-author"})
	if err != nil {
...
func TestLog_RealGit_EmptyRepo(t *testing.T) {
	...
	cs, err := New().Log(context.Background(), LogQuery{Repo: dir})
	if err != nil {
		t.Fatalf("Log: %v", err)
	}
	if len(cs) != 0 {

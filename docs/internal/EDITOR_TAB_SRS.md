...
**FR-EDT-68a.** 위 둘은 **탐색기**의 것이다. **편집기 탭(소스·렌더)의 시선**
(스크롤·커서·선택·접힘)이 탭 전환·창 전환·칸 전환을 지나 남는 것은
`VIEW_SCROLL_RESTORE_SRS` 가 규정한다 (`FR-VSR-1`~`11`). 그 자리를 이 문서에서
찾지 마라 — 여기에는 없었고, 그것이 `U-20` 이었다.
...
> **인용을 고쳤다** (`09` 모순 D, M5 2026-09-12). 여기 있던 *"cross-platform
> 보류 방침(§6 비목표)"* 은 **§6 에 없는 조항**이었다 — 비목표 10개를 전수
> 확인했고 그런 항목은 없다. 그리고 그 "방침" 자체가 `CROSS_PLATFORM_SRS` 로
> 뒤집혔다: Windows·Linux 는 이제 1급 대상이고 CI 가 세 OS 에서 실기 검증한다.
>
> **남는 근거는 비용뿐이다** — 플랫폼마다 다른 시스템 콜을 들여야 한다는 것.
> 그것으로 충분하며, 없는 조항을 인용해 무게를 더하지 않는다.
...
3. ~~**파일 검색 (Ctrl+P 류).** 탐색기는 트리이지 검색기가 아니다.~~
   **⊘ 철회** (`EDITOR_GIT_UX_SRS` FR-EQO-1~8, `09` 모순 A).
   `Mod+P` 로 파일을 찾아 여는 길이 **구현돼 있다** — `helpers.js` 의
   `edQuickOpen`, 서버는 `GET /api/fs/find`. 이 비목표는 그것보다 **나중에
   수정된 문서에 그대로 남아 있었고**, 두 문서 어느 쪽도 상대를 몰랐다.
...
| **D-7** | 파일 조작 다섯, 삭제는 **영구 삭제** | 사용자 지시. 휴지통은 OS 마다 규약이 달라(macOS `~/.Trash` · Linux XDG · Windows 휴지통 API) 셋을 각각 들여야 하고, 그 비용이 얻는 것보다 크다. **근거를 다시 세웠다** (`09` 모순 D) — 종전 근거였던 "cross-platform 보류 방침" 은 §6 에 없는 조항이었고 `CROSS_PLATFORM_SRS` 로 뒤집힌 방침이었다 |
...
| **D-26** | `rename` 의 무덮어쓰기는 **파일은 `os.Link` 로, 디렉터리는 Go 의 기존 가드로** 닫는다 | 초판은 "원자적으로 닫을 수 없다" 였으나 실측이 그것을 뒤집었다 — Go 의 `os.Rename` 은 대상이 디렉터리면 이미 EEXIST 이고, 파일은 `os.Link` 가 원자적으로 이름을 잡는다. 심볼릭 링크와 EXDEV 폴백에만 창이 남고, 그쪽은 `Server.fsOps`(종전 패키지 전역 `fsOpMu`, M8 `GO-9` 로 서버 필드가 됐고 **실제 파일 조작 구간만** 감싼다 — 본문 읽기·경로 판정은 밖) 가 자기 경합만 없앤다 (FR-EDT-115) |

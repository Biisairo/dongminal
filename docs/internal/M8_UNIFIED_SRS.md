...
> **문서 상태**: 승인·구현완료
>
> 근거(2026-09-15 전수조사): `production/M8_PROGRESS.md` §1 이 **"P7 완료 · M8 완료"** 를 적었고
> P0~P7 전 단계가 전량 e2e `unexpected 0` 으로 닫혔다. 상태 줄만 `승인·구현중` 으로 남아 있었다.
...
> **개정 2026-09-14 (M9 P7).** 이 조항의 두 자리가 `M9_SRS` 로 옮겨 갔다.
>
>   · **"터미널 탭을 열고" 는 제자리 전환이다** (`M9_SRS` FR-M9-31 / M9-B14).
>     이 조항의 첫 구현은 탭을 **하나 더** 만들고 에이전트 탭을 남겼고, 사용자가
>     *"자연스럽지 못하다"* 로 접수했다. 지금은 새 터미널 탭이 **옛 에이전트 탭의
>     자리**에 서고 그 탭은 닫힌다
>   · **"반대 방향" 의 스파이크는 돌지 않았다** — 그 절반을 `M9_SRS` FR-M9-32·33 이
>     맡는다. P7 의 실측이 답을 줬다: 세션 id 는 **이미 훅으로 서버에 도착하고 있고**
>     (dongminal 셸이 `claude` 를 래핑해 Run 밖의 탭에도 훅이 붙는다), 없던 것은
>     앉을 자리였다. 그리고 **세 에이전트가 다 되지는 않는다** — codex 는
>     `Signals.Session` 선언이 없어 세션 신원을 말하지 못한다 (D-M9-21)
>   · 반대 방향은 **탭을 닫지 않는다** (D-M9-20). 터미널 탭에는 사용자의 셸이 살기
>     때문이며, 그 비대칭은 의도다

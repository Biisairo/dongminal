...
`W?.P?.T?` 라벨은 workspace.json 배열 위치로부터 매번 재계산되는 positional 좌표다. 다른 창·pane 이 닫히면 모든 후속 라벨이 한 칸씩 reflow 된다. 보관해둔 라벨이 다른 pane 을 가리키는 순간 정리·라우팅·계층 팀 모두 깨진다.
...
이로써 다른 창이 닫혀도 보관한 팀원 uuid 는 같은 pane 을 가리키며, `closeTab` 직전 `list_workspace` 재확인은 더는 정합성 보존을 위한 필수 단계가 아니다.

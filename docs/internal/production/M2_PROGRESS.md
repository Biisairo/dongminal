...
| U-22 | **열기만 하고 수정하기 전인 파일은 외부에서 바뀌면 그대로 반영되어야 한다** | 📋 **스펙 초안** — [`EDITOR_EXTERNAL_CHANGE_SRS`](../EDITOR_EXTERNAL_CHANGE_SRS.md). 판정 둘을 받았다(dirty 면 **놔둔다** · 저장 시도 때 **경합** 검사 · **폴링 없음**). 조사로 확정한 것: `refresh()` 가 **dirty 를 보지 않아** 지금도 탭을 다시 열면 편집본이 덮인다(`FR-EXC-4` 가 그 자리다) · 자동 감지가 없다 · `probe` 에 `mtime` 이 없다 · 저장이 경합을 보지 않는다. **구현은 다음 세션**이며 착수 전에 물을 것 셋이 §6 에 있다 |
...
**이 절은 2026-09-11(8차)에 다시 썼다.** A 갈래가 닫히면서 남은 것이 셋으로
줄었다 — 끝난 일의 서술은 §2 에 있고 여기서 되풀이하지 않는다.

```
프로젝트: /Users/dykim/personal/dongminal

M2 를 이어서 진행한다. **P0 5건·P1 11건이 끝났고, 사용자 보고 23건 중 20건이
닫혔다. 값·결정 판정(A 갈래)도 전부 들어갔다** (§2.21).

**남은 것은 여섯 갈래이며, 아래 순서가 곧 우선순위다:**

  A-0  `U-22` — **스펙이 이미 서 있다. 바로 착수할 수 있다**
  A    omp 실행 확인 넷 + `U-1` — **사용자만 답할 수 있고, 이것이 M2 를 닫는다**
  A-1  원인 없는 흔들림 둘 — 추적 대상
  B    `U-3`·`U-17` — 재현 대기
  C    이월 (M4·M5·M8)
  D    마무리 — 전량 e2e 한 번 → 커밋
...
- docs/internal/production/M2_PROGRESS.md
  ...
  · §2.22 기준선 실패는 셋이 아니라 **여덟**이었고 여섯이 한 뿌리였다
  · §2.23·§2.25 CI 를 초록으로 되돌린 길 (govulncheck 판 고정 · Go 툴체인
    1.25.13 · Windows 경로·MIME 의 호스트 의존)
  · §2.24 알람이 되풀이 울던 자리 — **배경 알림이 사용자 턴으로 읽혔다**
  · §3.3 DoD — **전부 닫혔다**. §3.3a 가 마일스톤 문서의 **DoD 원본을 한 줄씩**
    대조한 결과이고, §3.3b 가 **P2 18건 전수**다 (전부 닫힘/판정). 이 둘이
    "M2 가 정말 끝났는가" 의 답이다 — 남은 것은 A 의 실행 확인뿐이다
  · §3.4 사용자 보고 표 — 남은 셋은 `U-22`(스펙 있음)·`U-3`·`U-17`
  · §6   e2e 현황

- docs/internal/EDITOR_EXTERNAL_CHANGE_SRS.md — `U-22` 의 스펙. **A-0 이 그것이다**
...
## A-0. 바로 착수할 것 — `U-22` (스펙이 이미 서 있다)

**편집기로 연 파일이 밖에서 바뀌었을 때** 무엇을 보일지가 접수됐고 판정까지 끝났다.
읽을 것은 하나다: `docs/internal/EDITOR_EXTERNAL_CHANGE_SRS.md`

  · 판정 — dirty 면 **놔둔다** · 저장 시도 때 **경합** 검사 · **폴링 없음**
  · 조사로 확정한 사실 넷이 §2 에 있다. 그중 하나는 **지금도 재현되는 결함**이다:
    `refresh()` 가 dirty 를 보지 않아 **탭을 다시 열기만 해도 편집본이 덮인다**
    (`FR-EXC-4`). 자동 감지를 붙이지 않더라도 이 자리는 고쳐야 한다
  · **착수 전에 물을 것 셋**이 §6 에 있다 (경합 화면의 모양 · 외부 삭제 ·
    표식을 mtime 으로 할지 해시로 할지)
  · 검증은 §4 에 여섯이 적혀 있다 (e2e 셋 · Go 셋)

**`U-10` 을 기억하라** — 이 제품에서 가장 비싼 실패가 편집을 확인 없이 잃는 것이다
(`FR-RTU-103`). 그래서 `FR-EXC-3`(dirty 면 건드리지 않는다)이 이 문서의 중심이다.
...
## A-1. 원인이 아직 없는 흔들림 — 하나 남았다

  · `git-file-actions` F6 — 전량에서만 빈 배열, 단독으로는 통과

**추적의 방법은 이미 입증됐다.** `ui-layout-defaults` 가 같은 모양이었고 원인은
**부하가 아니라 설정 오염**이었다 (§6.3) — `tab-width` 가 `tabFixedWidth` 를 서버
설정에 남기고, 그것이 워커를 넘어 다음 스펙의 화면을 바꿨다.

  1. 의심되는 선행 스펙과 **둘만** `--workers=1` 로 연달아 돌려 재현한다
  2. 재현되면 부하가 아니라 **앞선 스펙이 남긴 상태**다
  3. 고치는 자리는 **검사가 자기 전제를 스스로 세우는 것**이다 — 남이 치우기를
     기대하면 다음 스펙이 또 걸린다
...
## C. 이월 — M2 가 닫지 않는다
  ...
  · **알람 재무장의 범위** (§2.24) — `waitingSignaled` 를 내리는 것이 `working`
    보고인데 그것을 보내는 훅이 다섯이고 `toolId` 는 탭 단위다. 병렬 도구가 도는
    동안 대기 표시가 남의 일로 내려갈 수 있다. **실측 근거가 없어 열어 두었다** —
    고치려면 "사용자가 새 일을 시켰다" 의 정의를 다시 세우고, 터미널에서 직접
    승인하는 경로(`FR-ATN-16`)를 함께 봐야 한다
  · **샌드박스 상한값을 `features.md` 용량 표에 싣는 것** → **M5** (`G9-1`).
    M2 DoD 에 그 문장이 있으나 `docs/features.md` 가 **아직 없다** — 그 문서 자체가
    M5 의 산출물이다. 값은 정해졌다(2cpu·2g·512, `FR-SBX-45`) — 옮겨 적기만 하면
    된다 (§3.3a)
...
- **"부하성 흔들림" 이라고 부르기 전에 순서를 의심하라.** `ui-layout-defaults` 가
  그것이었다 — 전량에서만 났으므로 부하로 보였지만, 실제로는 **앞선 스펙이 서버
  설정에 남긴 값**이었다. 의심되는 둘만 `--workers=1` 로 돌리면 20분 안에 갈린다.
  **설정·워크스페이스처럼 서버에 남는 상태는 워커를 넘어 전파된다.**
- **낙관적으로 그리는 UI 뒤에서 디스크를 곧바로 단정하지 마라.** 탐색기의 조작은
  응답 전에 행을 그리고 실패하면 되돌린다 (`_optimAdd`/`_restore`, `FR-EXR-21`).
  **행이 보이는 것은 디스크에 생겼다는 뜻이 아니다** — darwin·linux 에서는 응답이
  빨라 우연히 통과하고 **Windows 러너에서 ENOENT 로 드러난다** (2026-09-11).
  같은 함정이 `explorer-copy`·`file-transfer` 에도 남아 있다.
...
> | 검사 | 사실 | 남은 물음 |
> |---|---|---|
> | `git-file-actions` F6 | 전량에서만 빈 배열. **단독으로는 통과** | 같은 부류(앞선 스펙이 남긴 상태)인지, 정말 부하인지 |
>

...
func (f *detailFake) run(_ context.Context, _ string, args []string) (core.Output, error) {
	f.argvs = append(f.argvs, append([]string(nil), args...))
	if f.exit != 0 {
		return core.Output{ExitCode: f.exit, Stderr: f.stderr}, nil
	}
	if args[0] == "log" {
...
func TestCommitDetail_RealGit(t *testing.T) {
	...
	cs, err := Log(s, ctx, LogQuery{Repo: repo, Order: LogOrderTopo})
	if err != nil {
		t.Fatalf("Log: %v", err)
	}
	...
	d0, err := CommitDetailOf(s, ctx, repo, merge.Oid, 0)
	if err != nil {
	...
	d1, err := CommitDetailOf(s, ctx, repo, merge.Oid, 1)
	if err != nil {
	...
	ren, err := CommitDetailOf(s, ctx, repo, bySubject["rename with space"].Oid, 0)
	if err != nil {
	...
	root, err := CommitDetailOf(s, ctx, repo, bySubject["init"].Oid, 0)
	if err != nil {
	...
	second, err := CommitDetailOf(s, ctx, repo, bySubject["두 번째 · 유니코드"].Oid, 0)
	if err != nil {

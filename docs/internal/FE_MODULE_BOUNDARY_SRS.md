...
| N3 | ~~`ui/renderer.js` 분할~~ → **개정 2026-09-21** | 착수 시의 근거는 *"응집도가 있다"* (`02-fe-arch` A-6)였고 그것은 **그때 옳았다.** 다만 근거가 줄 수가 아니라 응집도였으므로 **줄이 늘어도 판단이 자동으로 재검토되지 않았다** — 1,336 → 1,586(+19%). 실측으로 이 파일은 지금 **넷을 한다**: 골격 캐시·배치 유틸 · 스크롤 갈무리·복원 · 사이드바·탑바 그리기 · 레이아웃·pane·탭 조립 (`AUDIT-fe-ui.md` M-9). `STRUCTURE_CLEANUP_SRS` FR-STR-43·44 가 **증강 분할**로 가른다 (묶음 B·C 와 같은 절차 — `Object.assign(Renderer.prototype, …)` 이므로 계약은 한 글자도 바뀌지 않는다) |
...
#### 7.1a 모듈 크기 기준선 — 게이트가 읽는 줄 (2026-09-21)

> **개정 (`STRUCTURE_CLEANUP_SRS` 묶음 D · D-STR-5).** §4.3 이 이 수치를 DoD 로
> 올렸는데 **재는 것이 없었다.** 이 저장소의 다른 43종 게이트가 전부 "두 곳이
> 같은가" 를 재는 동안, 유일하게 수치 DoD 만 문서에 적히고 집행되지 않아
> **9일 만에** 22 → 26 · 1,336 → 1,586 으로 되돌아왔다. §4.3 이 수치를 요구한
> 이유(`FR-FMB-43`: *"값을 옮기는 것보다 다시 흩어지지 않게 하는 것이 본체다"*)가
> 자기 자신에게 적용되지 않은 것이다.
>
> `scripts/check-file-size.mjs` 가 아래 줄을 읽고 **나빠지면 실패한다.**
> 기준선을 22 / 1,336 으로 되돌리는 것은 네 파일을 추가로 500 아래로 가르는
> 일이라 이 묶음의 범위를 넘는다 — 감사 D2 가 권한 순서대로 **게이트를 먼저
> 세우고 기준선을 정한다.** 재는 것이 없는 상태가 수치보다 나쁘다.

세는 대상은 `web/js` 이며 **`vendor/`·`test/`·`i18n/` 을 뺀다** — 앞의 둘은 우리
...
|---|---:|---:|---:|---:|
| 500줄 초과 파일 (i18n 제외) | 27 | 22 | 24 | **23** |
| 최대 파일 | 1,336 | 1,336 | 1,586 (`ui/renderer.js`) | **1,174** (`ui/term-pane.js`) |

> 24 → 23 (2026-09-21, `UIUX_OVERHAUL_SRS` FR-ACT-1·2). `app-statusbar.js` 가
> 백그라운드 모달을 잃고(모달이 구역이 됐다) 접힘을 `app-statusbar-fold.js` 로
> 내보내면서 500 아래로 내려갔다. **가르려고 가른 것이 아니라 표면이 줄어서
> 줄었다** — 그래서 이 수는 되돌아가지 않는다.
>
> 1,178 → 1,174 (2026-09-21, UX_BATCH10_SRS FR-UXB-41). `file-editor.js` 의 Monaco
> 옵션 덩이가 `edTextOptions()` 로 나가면서 네 줄이 줄었고, 최대 파일의 이름도
> `ui/file-editor.js` 에서 `ui/term-pane.js` 로 옮겼다. 같은 변경이 더한
> `git/diff-view.js` 는 500 아래로 가른 뒤에야 들어왔다 (`diff-view-parity.js`).

`renderer.js` 를 다섯으로 가른 결과다 (FR-STR-44) — 1,586 → 135 · 307 · 314 · 370 · 585.
**500줄 초과 수는 줄지 않았다**: 조각 하나(`renderer-pane.js` 585)가 아직 경계 위다.
그것을 더 가르는 일과 나머지 파일들의 분할은 이 묶음이 아니며, 게이트가 섰으므로
**악화는 커밋에서 막힌다** (D-STR-5).

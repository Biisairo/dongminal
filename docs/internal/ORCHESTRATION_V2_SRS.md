...
> **정정 (M8 P1 ③ `GO-12` · FBE-01 서버측, 2026-09-13)** — 2 의 기다림은 **요청이
> 끊기면 그 자리에서 끝나고 새 Member 를 만들지 않는다.**
>
>   이전 동작: `time.Sleep` 폴링이라 dmctl 의 시한(10초)이 먼저 끊겨도 서버는
>             상한(기본 180초)까지 기다린 뒤 이었다 — 실패로 보고된 승계가 실제로는
>             성공해 있고, 재시도가 멤버·도구를 이중으로 만들었다 (`07 FBE-01`)
>   새  동작: 대기는 `r.Context()` 로 끊긴다. 끊기면 `--headless` 로 만든 도구를
>             거두고 아무것도 잇지 않는다 — 재시도가 처음부터 다시 한다
>   이유:     묻는 쪽이 사라진 요청의 부작용은 그 요청의 것이 아니다. dmctl 쪽의
>             예산(서브커맨드별 시한)은 P6 ⑤ 에서
...
> **개정 2026-09-22 (`UIUX_OVERHAUL_SRS` FR-CHR-14·15, D-7).** 이 줄에서 **`Runs`
> 버튼이 없어졌다.** 분할 둘은 이미 pane 탭줄로 갔고(FR-CHR-10), 남은 `Runs` 와
> `Agents` 는 **같은 패널**을 열고 있었다 — 문이 셋이면 방도 셋으로 읽힌다.
> 상단바는 `알림 · 칸 ± · Activity` 셋이 된다.
>
> **Run 에 닿는 길은 줄지 않았다**: `Ctrl+Shift+O` 가 패널을 열어 **Run 구역으로
> 직행**하고(FR-CHR-18 · `PANEL_SHORTCUTS_SRS` FR-PSC-2), 모바일은 서랍의
> `Activity` 가 진다. 잃은 것은 *버튼*이지 *진입점*이 아니다.

**FR-RVZ-2** `Runs` 클릭 시 **중앙 모달**이 열린다. 백그라운드 도구 모달(`bg-modal`)과
같은 상호작용 규약을 쓴다 — 배경 클릭·`Esc` 로 닫히고, 오버레이 자신이 대상일 때만 닫힌다.

> **개정 2026-09-21 (`UIUX_OVERHAUL_SRS` FR-ACT-1·2).** 중앙 모달이 **활동 패널의
> Run 구역**으로 바뀌었다. (**2026-09-22 / D-7**: 진입점 `#runs-btn` 은 그 뒤에
> 없어졌다 — FR-RVZ-1 의 개정을 본다.) 행의 마크업·행 클릭이 대시보드
> 탭을 여는 흐름은 그대로이고, **표면만** 바뀐다 — 되돌릴 것이 없는 조회가
> 백드롭으로 앱 전체를 막을 이유가 없다 (그 문서 §3.3 원칙 4).
>
> 함께 바뀐 것: 행을 눌러도 목록이 닫히지 않는다 (조회는 남의 동작에 닫히지
> 않는다). 백그라운드 모달과 *"같은 상호작용 규약"* 이라는 이 요구의 문장은
> 그대로 성립한다 — 그쪽도 같은 개정으로 구역이 됐다.

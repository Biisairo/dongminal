...
## 1.5 진행 상황 (이 파일은 매 세션 끝에 갱신한다)

**M1~M4 완료. Go 측(M1~M5 전 서버 API) 완료.** **M1·M2·M3·M4 완료. M5 는 18·19 완료.** 남은 것은 20단계(다이얼로그 규약, 진행) · 21단계(수동 검증) 뿐이다. 기준선 — `go test ./... -race` 전부 통과 ·
gofmt clean · Playwright **187 → 340**.

| 단계 | 내용 | 상태 | 커밋 |
|:--:|---|---|---|
| — | 21단계 설계 계약 `./design/` + 색인 | ✅ | `67bc325` `b8a288a` |
| — | SRS §7 결정 확정 (O1~O14) + §7.1 해석 (I1~I6) | ✅ | `6157476` |
| — | 검증용 저장소 픽스처 10종 (`scripts/git_fixture.sh`) | ✅ | `ddb4307` |
| — | 수동 검증 체크리스트 (V14·V60) | ✅ | `cece980` |
| — | 테스트 더블 데이터 레이스 2건 — `-race` 게이트 복구 | ✅ | `a2c3abe` |
| **M1** | | | |
| 1 | 묶음 A `internal/git` — git 실행 단일 지점 | ✅ | `f455137` |
| 2 | 묶음 B·C 서버 — 리포 해석·status·signature | ✅ | `cc62d09` |
| 3 | 묶음 D Git 창 골격 | ✅ | `8ed0396` |
| 4 | 묶음 B 클라 좌측 GIT 섹션 | ✅ | `72bbe00` |
| 5·6 | 묶음 E Changes 읽기 + 묶음 C 클라 폴링 | ✅ | `747db3a` |
| 7 | 묶음 F diff-content + Monaco DiffEditor | ✅ | `a9f244e` `f078fdf` |
| 8 | 묶음 G 상태바 chip | ✅ | `1493a8a` |
| **M2** | | | |
| 9 | 묶음 J 안전 정책 (Go + 클라 2단계 확인) | ✅ | `71ed098` `579a070` |
| 10·11 | 묶음 H 스테이징 · I 커밋 (e2e 274/275) | ✅ | `54d869c` `5d25d65` |
| **M3** | | | |
| 12·13 | job 인프라 + fetch/pull/push (e2e 340) | ✅ | `ef45143` `677b059` |
| **M4** | | | |
| 14 | 로그·커밋 상세·refs 조회 | ✅ | `a036af5` |
| 15 | 레인 알고리즘 (UI 보다 먼저 고정) | ✅ | `6a01d4d` |
| — | diff 축 `commit-parent` (17단계가 딛는다) | ✅ | `90d93a4` `5b649df` |
| 16·17 | History 탭 + 커밋 상세 + 컨텍스트 메뉴 프레임워크 (e2e 308) | ✅ | `a25abd0` |
| **M5** | | | |
| 18·19 | Branches · Stash (e2e 331) | ✅ | `c9f8134` `7dd4130` |
| 20 | 다이얼로그 공통 규약 (기존 확인창 통합·감사) | 진행 | |
| 21 | MVP 수동 검증 — 체크리스트는 `../GIT_MANUAL_CHECKLIST.md` | 대기 | |

**단계별 지시는 `./design/GIT_M*_STEP*_CONTRACT.md` 에 있다.** 새 세션은 SRS 를 다시
해석하지 말고 해당 계약 문서를 읽고 그대로 구현한다. 계약과 다르게 가야 할 근거가
생기면 계약 문서를 먼저 고친다.
...
- **Playwright 는 동시에 두 번 돌 수 없다** — `playwright.config.ts` 가 고정 포트
  58147 + `reuseExistingServer:false` 다. e2e 실행 권한을 한 번에 하나에게만 준다.

...
- **M4 가 끝나기 전까지 제품의 기본 사용 형태가 무인증 상태다.** M2 는 브라우저 매개 공격을 닫지만 무인증 LAN 노출은 M4 까지 열려 있다 — M2 섹션의 운영 안내 항목을 보라.
...
| 5 | **M4** 인증·인가·전송 보호 + 제품 경계 | M3·M2 | **없음** | L |
| 6a | **M5** 관측성·에러 규약·설정·문서·릴리스 | M4 | 없음(결정 4 는 진행 중 확정 가능) | L |
| 6b | **M6** git 갱신 계층 재설계 + 렌더 파이프라인 + e2e | M1·M4 | **없음** | XL |
...
> **⚠ 진행 중 (2026-09-10).** **P0 5건이 전부 닫혔고 P1 8건이 끝났다.**
> 남은 것·미검증·사용자 결정 대기 항목은 [`M2_PROGRESS.md`](./M2_PROGRESS.md) 에
> 있다 — **이 섹션보다 그쪽을 먼저 읽어라.** 스펙 2건은 이미 있다:

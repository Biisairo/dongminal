...
> **이 스킬은 오케스트레이션 전용이다 — 인수인계는 `migration` 이다.**
>
> 여럿을 **동시에** 돌리는 일이 여기 있고, 한 세션의 일을 **다음 한 세션에게** 넘기는 일은 `/dongminal:migration` 에 있다. 둘은 모양이 닮았지만 (새 탭 · `wait --for ready` · `dmctl msg`) 목적이 다르다 — 팀은 전용 창에 여럿을 세우고 끝나면 **해체**하며, 인계는 지금 pane 에 하나를 세우고 **자기 탭을 닫는다**. 인계를 팀으로 하면 Run 레코드가 팀원 하나짜리로 남고, 넘기는 쪽이 조정자로 살아 있어야 해서 정작 닫지 못한다.
...
- **답장할 때는 엔벨로프 헤더의 `from=<uuid>` 값을 그대로 `--to` 에 쓴다.** 헤더는 uuid 만 싣는다 — 좌표 라벨은 실리지 않으므로 어느 값을 쓸지 고를 일도, 보낸 사람을 되짚어 찾을 일도 없다

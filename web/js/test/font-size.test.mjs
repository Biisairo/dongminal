...
test('V-FSS-4a: 표가 두 키의 범위와 기본값을 갖는다 (FR-FSS-2·12)', () => {
  ...
  assert.deepEqual(
    { type: by.uiFontScale.type, min: by.uiFontScale.min, max: by.uiFontScale.max, def: by.uiFontScale.def },
    { type: 'int', min: 80, max: 200, def: 100 });
  assert.deepEqual(
...
test('V-FSS-4b: 기본값은 종전 화면을 낸다 (FR-FSS-23)', () => {
  const { SETTINGS_BY_KEY: by } = ctx();
  // 배율 100 = 곱하기 1. 터미널 14 = 종전 `--fs-lg` 의 px 값.
  assert.equal(by.uiFontScale.def, 100);
...
test('V-FSS-4g: settingValue 는 범위 밖을 기본값으로 떨어뜨린다 (FR-CFG-3 · clamp 와 다르다)', () => {
  ...
  // 같은 입력(201)에 두 손이 **다르게** 답한다: 저장값 해석은 100, 입력 clamp 는 200.
  assert.equal(sv(201, by.uiFontScale), 100);
  assert.equal(sv(33, by.termFontSize), 14);
  assert.equal(sv(150, by.uiFontScale), 150);

...
// 격리되지 않는다.
func headRunner(_ context.Context, _ string, _ []string) (core.Output, error) {
...
func TestUnstage_NoHeadUsesRmCached(t *testing.T) {
	...
	if _, err := Unstage(s, ctx, repo, Paths{"a.txt"}); err != nil {
		t.Fatalf("Unstage: %v", err)
	}
...
func TestStagePaths_Rejected(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			deny := func(_ context.Context, _ string, args []string, _ string) (core.Output, error) {
...
func TestStage_SplitsAndReportsPartial(t *testing.T) {
	...
	calls := 0
	fail := func(_ context.Context, _ string, _ []string, _ string) (core.Output, error) {
		calls++
		if calls == 2 {
			return core.Output{ExitCode: 1, Stderr: boom.Error()}, nil
		}
		return core.Output{}, nil
	}
...
func TestDiscard_PartialAcrossBothCommands(t *testing.T) {
	fail := func(_ context.Context, _ string, args []string, _ string) (core.Output, error) {
		if args[0] == "clean" {
			return core.Output{ExitCode: 1, Stderr: "fatal: clean 실패"}, nil
		}
		return core.Output{}, nil
	}

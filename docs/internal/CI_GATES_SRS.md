...
| ~~**`flaky > 0` 을 실패로 올리지 않는다**~~ **철회 (2026-09-12, M6)** | 근거였던 *"제품 쪽 계통 결함이 남아 있다"* 가 해소됐다 — `OPTIMISTIC_LAYOUT_SRS`(탭·창 유실)와 `GIT_REFRESH_LIFECYCLE_SRS`(관측 국소 결함)가 `11 §5` 의 매핑 8건을 닫았다. 이제 `parity-reporter.ts` 가 `flaky > 0` 을 **실행 실패**로 올린다. 판정은 거기 한 자리다 — 워크플로는 수를 요약에 적을 뿐이다. 조사용 손잡이는 `DM_E2E_ALLOW_FLAKY=1` 이고 **CI 는 그것을 주지 않는다** |
| **e2e 샤드 수를 바꾸지 않는다** | §5 |

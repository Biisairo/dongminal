...
### 2.4 코드 전수 감사 — "경로가 OS 마다 다르다" 를 기준으로
...
### 2.6 제품 결함 — 경로 밖에서 나온 것 (D11)

D1~D10 은 경로를 기준으로 훑다 나왔다. D11 은 다르다 — 잔여 실패를 하나까지
좁히다 마지막에 남은 것이며, 경로가 아니라 **수명**의 문제다.

#### D11 · 셸이 스스로 끝나도 탭이 닫히지 않는다

`internal/shared/platform/pty_windows.go`

**ConPTY 는 자식이 끝나도 출력 파이프를 닫지 않는다.** 쓰기 끝을 쥔 것은 자식이
아니라 conhost 이고, conhost 는 `ClosePseudoConsole` 전까지 살아 있다.

그래서 `toolhub.readPTY` 가 EOF 를 보지 못하고, `kill()` 이 불리지 않고,
`done` 채널이 닫히지 않는다. Windows 에서 **`exit` 를 친 탭이 영원히 남는다.**

POSIX 에서는 마지막 slave 가 닫히면 master 가 `EIO` 를 받으므로 이 문제가
없다. 한쪽에서는 커널이 해 주고 다른 쪽에서는 **아무도 해 주지 않는** 차이다 —
이 트랙이 반복해서 만난 모양이다.

조치: 자식을 기다렸다가 의사 콘솔을 닫는 `reap` 을 둔다. 남은 출력은 conhost 가
흘려보내므로 읽는 쪽은 그것을 다 읽은 뒤 EOF 를 본다. 프로세스 핸들의 소유자도
`reap` 하나로 모은다 — 종전에는 `Wait` 가 직접 기다리고 닫았는데, 둘이 같은
핸들을 보면 한쪽이 닫은 것을 다른 쪽이 만진다.

### 2.7 테스트가 딛고 있던 OS 전제 — 경로 밖의 것들

경로만이 아니었다. 잔여를 좁혀 가며 나온 것들을 함께 적는다.

| 전제 | 실제 | 어디서 드러났나 |
|---|---|---|
| Enter 는 `\n` | **ConPTY 는 CR 을 Enter 로 본다.** POSIX 의 pty 는 `ICRNL` 로 둘 다 받아 줘서 `\n` 이 우연히 통했다 | `tool_exit` 이 셸을 못 끝냈다 |
| 셸은 곧 준비된다 | ConPTY 의 인사말 16바이트가 **셸보다 먼저** 온다. 첫 바이트를 준비 신호로 삼으면 그 뒤 입력이 사라진다 | 고정 대기·첫 바이트 둘 다 실패. doctor 가 a36417a 에서 세운 신호(출력이 오고 **조용해지는 것**)를 쓴다 |
| 줄 끝은 `\n` | git 설정·플랫폼에 따라 CRLF 다 | `patch` 검사가 `line10` 을 `line10\r` 로 읽어 전부 어긋났다 |
| 응답이 첫 메시지다 | 도구의 셸이 배너를 내면 **출력 푸시가 먼저** 나간다. pwsh 는 배너를 즉시 낸다 | `paned` 의 hello 검사가 ID 0 을 받았다 |
...
### 빠진 보증 — FR-M10-2 의 스크롤백 다시 그리기 (2026-09-16)

`M10_SRS` FR-M10-2 는 *"소유권 전환으로 폭이 바뀌면 스크롤백이 그 폭으로 다시
선다"* 를 보장한다. **ConPTY 에서는 그 보장이 서지 않는다.**

| 판 | 폭이 168 → 28 로 줄었을 때 그 줄 |
|---|---|
| POSIX PTY | **28열로 다시 파싱된다** — 좌표가 클램프되어 한 줄로 맞는다 |
| ConPTY | **옛 168열 그림이 접혀 남는다** |

까닭은 기록의 성질이다. POSIX PTY 는 앱이 낸 원본 바이트를 흘리므로 `ESC[168G`
같은 절대 좌표가 기록에 남고, 전량 재생이 그것을 새 폭에서 다시 파싱한다. ConPTY
가 주는 것은 그 시점 폭으로 **이미 그려진 화면**이라 좌표가 남아 있지 않다.

**고칠 제품 결함이 아니다.** 되찾은 폭은 PTY 까지 정확히 닿는다(실측
`_ptyCols == 28`). 그런데도 ConPTY 는 이미 그려진 줄을 다시 내보내지 않으며,
그렇게 해 달라고 요청할 API 도 없다 — 공개 표면은 `CreatePseudoConsole` ·
`ResizePseudoConsole` · `ClosePseudoConsole` 셋뿐이고, 위로 밀려난 줄은 conhost
관점에서 이미 내보낸 뒤라 존재하지 않는다.

Windows 쪽 동작은 **네이티브 터미널이 하는 것과 같다.** POSIX 쪽이 원본을 들고
있어 더 잘하는 것이지 Windows 가 깨진 것이 아니다.

**무엇이 빠지고 무엇이 남는가** (FR-WTP-32):

- 빠짐: `term-reclaim` TC-M10-2 의 **둘째** 단언(줄이 새 폭으로 선다). 능력 질의
  `osenv.ptyRecordsRawOutput` 이 가르고, 건너뛴 사실은 `platform-gap` 주석으로
  실행 출력에 남는다
- 남음: 같은 검사의 **첫째** 단언(되찾은 폭이 PTY 까지 닿는다)과 TC-M10-1 —
  Windows 에서도 그대로 잰다
...
run 33263400171 (`e242765`) 에서 **잡 넷이 모두 통과**한다.

| 잡 | 결과 |
|---|---|
| `windows-runtime` (doctor + 종단간) | ✅ |
| `linux-runtime` | ✅ |
| `test (windows-latest)` | ✅ |
| `test (ubuntu-latest)` | ✅ |

Windows 단위테스트의 실패 줄 추이다.

    356 → 79 → 62 → 48 → 6 → 1 → 0

NFR-WTP-2(잡이 초록)와 NFR-WTP-3(타임아웃 소멸)이 함께 충족됐다. 600초 타임아웃
두 건은 원인 ①의 파생이었고, ①이 사라지자 함께 사라졌다.

### 10.2 찾은 제품 결함 11건

이 트랙이 실제로 한 일은 테스트를 고친 것이 아니라, **테스트가 이미 잡고 있었
는데 아무도 읽지 않던 결함을 꺼낸 것**이다.

| # | 결함 | Windows 에서의 결과 |
|---|---|---|
| D1 | `migrate` 가 `os.FindProcess`+`Signal(0)` 로 생존 판정 | 데몬이 도는 중에 마이그레이션이 진행돼 상태를 덮어씀 |
| D2 | `keep` 은 `/`, walk 는 `\` | 설치할 때마다 생성된 `hooks.json` 을 삭제 |
| D3 | `parseWorktreeList` 가 git 형태 경로를 그대로 반환 | worktree 소유가 전부 `outside`, 산 것을 죽었다고 판정 |
| D4 | `check-seams.sh` 의 패턴 구멍 | D1 이 그 구멍으로 들어옴 |
| D5 | `RepoRoot`·`Resolve` 도 같은 결함 | 캐시 키·핀 비교·API 응답이 두 형태로 갈림 |
| D6 | 훅이 확장자 없는 헬퍼 경로를 가리킴 | PATHEXT 해석이라는 우연에 의존 |
| D7 | `safeResolve("/", …)` 의 `Rel` 이 볼륨 불일치 | **파일 전송 전체가 403** |
| D8 | 경로 가드가 `/` 만 구분자로 봄 | `src\..\x` 가 부모 참조 검사를 통과 |
| D9 | 테스트가 `HOME`, Windows 는 `USERPROFILE` | 임시 홈이 안 먹고 진짜 사용자 홈이 샘 |
| D10 | `GIT_CONFIG_GLOBAL=/dev/null` | 전역 설정이 안 끊겨 hunk 대조가 어긋남 |
| D11 | ConPTY 가 자식 종료 시 파이프를 안 닫음 | **`exit` 를 친 탭이 영원히 남음** |

D7·D11 이 사용자가 실제로 겪을 결함이고, D1 은 데이터를 잃을 수 있는 결함이다.

### 10.3 곁들여 고친 것 — CI 플레이크

`test (ubuntu-latest)` 가 실행마다 **다른** 테스트에서 떨어졌다. 원인은 둘이었다.

1. **`SaveAll` 의 비동기 쓰기.** `go m.SaveAll()` 이 요청 경로를 막지 않으려
   저장을 떨어뜨리는데, 그것이 끝났는지 아무도 알 수 없었다. 테스트의
   `t.TempDir` 정리와 부딪혀 `directory not empty` 가 났다.
   `StopSaving()` — **문을 닫고 기다린다.** 기다리기만 하면 기다림이 끝난 뒤
   도구가 죽으며 `onExit → Delete` 가 저장을 다시 떨어뜨린다(실제로 한 번 더
   깨졌다).
2. **고정 대기 뒤 입력.** §2.7 참조.

### 10.4 남은 위험

**R-5(경로 동일성)와 R-6(예약 파일명)은 그대로 열려 있다.** 증거가 나오지 않아
손대지 않았다 — 추측으로 추상화를 세우지 않는다는 것이 이 트랙의 방침이다.

`test (windows-latest)` 가 초록으로 유지되는 한 다음 회귀는 **즉시 보인다.**
그것이 이 트랙이 그 잡 하나에 집착한 이유다 (NFR-WTP-5).

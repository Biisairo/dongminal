...
### 8. 세션, 탭 종료 시 해당 터미널에 실행 중인 프로세스가 있는 경우 물어보기 기능 추가
  - 세션, 탭 종료 시 종료되는 터미널 중 프로세스가 실행중인 경우 질문 후 허가 시 닫기, 허가하지 않을 시 아무 창도 닫히지 않음

### 9. 세션 리스트, pane, tab 드래그로 이동 기능 추가
  - 세션 리스트 내에서 드래그 드롭으로 순서 변경
  - 탭에서 드래그 드롭으로 순서 변경
  - 탭을 드래그하여 다른 pane 위에 올릴 시 올리는 위치에 따라 해당 탭 분할하여 배치
    - 이때 드롭 전에 시각적으로 어떻게 분할이 될 지 보여주기
  - 탭을 다른 pane 에 탭 안으로 이동 가능

### 10. tab 생성, pane 분할 시 포커스 되어있는 터미널의 경로를 가져는 기능 추가
  - 새 탭, pane 분할 시 포커스 되어있는 터미널의 경로와 같은 경로로 생성하기
  - session 생성 시에는 ~(home) 으로 생성 유지

### 11. 스크롤바 추가
  - terminal 에 스크롤이 되기는 하나 스크롤바가 없어 빠른 스크롤이 불가능함
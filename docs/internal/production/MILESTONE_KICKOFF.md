...
| 7 | **M7** 접근성·디자인 시스템·UX 안전 — **진행 중** | M6 | ~~결정 2~~ **확정(a) AA 전면** | XL |
| 8 | **M9** 국제화(i18n) | M7·M6·M5 | ~~결정 2~~ **확정** (`UX-11` 검증은 FR-A11Y-21) | L |
...
> **미결정 0건** (2026-09-13). 마지막 하나였던 **2(접근성 기준)가 (a) WCAG 2.1 AA 전면으로 확정**됐다 — 4(배포 채널)와 6(버전 정책)은 그 앞에 닫혔다. 로드맵 §1.4 를 보라.
...
> **결정 2 가 확정됐다** (2026-09-13): (a) WCAG 2.1 Level AA 전면, 벤더 표면(xterm 캔버스·Monaco 위젯)은 예외 등록부 두 건. 규칙셋은 `wcag2a`·`wcag2aa`·`wcag21a`·`wcag21aa` 넷이다. 판정 기준의 소유자는 [`ACCESSIBILITY_BASELINE_SRS`](../ACCESSIBILITY_BASELINE_SRS.md) 와 [`DESIGN_TOKENS_SRS`](../DESIGN_TOKENS_SRS.md) 다.
>
> **대비 항목은 실측으로 정정됐다** — 테마는 44종이 아니라 54종이고, DoD 가 인용하던 수치는 한 테마의 표본이었다. 로드맵 §M7 DoD 의 정정 문단을 보라.
...
### 5. 선행 결정 — **확정됨 (2026-09-13)**

**결정 2(접근성 목표 기준) = (a) WCAG 2.1 Level AA 전면.** 착수를 막는 것은 없다.
- (b)(핵심 화면에만)를 택하지 않은 근거: **"핵심 화면" 의 경계가 그 자체로 또 하나의 미결정**이다. 전면 선언은 그 물음을 없애고, 경계를 화면 목록이 아니라 **등록된 예외 목록**이 지게 한다 (현재 두 건: xterm 캔버스 · Monaco 위젯).
- 상세는 로드맵 §1.4·§3-B 와 [`ACCESSIBILITY_BASELINE_SRS`](../ACCESSIBILITY_BASELINE_SRS.md) §1.2.

### 6. SRS 필요 여부 — **둘 다 작성 완료 (승인·구현중)**

1. [`ACCESSIBILITY_BASELINE_SRS.md`](../ACCESSIBILITY_BASELINE_SRS.md) — 목표·범위와 제외·검사 방법·완료 조건·**예외 등록부**. (초안 계획의 이름 `ACCESSIBILITY_SRS.md` 에서 바뀌었다 — 이 문서가 정하는 것은 "기준선" 이지 접근성 구현 전부가 아니다.)
2. [`DESIGN_TOKENS_SRS.md`](../DESIGN_TOKENS_SRS.md) — 토큰 목록·파생 규칙·하드코딩 금지 게이트·**과도기 클래스의 종료 조건**(D-TOK-7 이 D-5 를 닫는다: 끝나는 조건은 날짜가 아니라 **CSS 선언이 0개가 되는 시점**이다).

### 7. 착수 프롬프트 — **낡았다**

> **이 프롬프트는 M7 착수 시점의 것이다.** 선행 SRS 와 대비 축은 이미 닫혔으므로,
> 지금 이어받는 세션은 [`M7_NEXT_SESSION.md`](./M7_NEXT_SESSION.md) 를 쓰고
> 진행의 진실은 [`M7_PROGRESS.md`](./M7_PROGRESS.md) 에서 읽어라.
> 아래는 근거 기록이다.

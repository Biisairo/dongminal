...
| 분할 후 `list_workspace` 에 새 pane 없음 | 분할 미반영 | `workspace_command` 응답 재확인, 필요 시 재시도 |
| 팀원이 반응 없음 | `claude` 실행 실패 (PATH, 권한) 또는 따옴표 이스케이프 실패로 쉘 파싱 에러 | `read_screen` 으로 상태 확인. 쉘 에러면 이스케이프 재검토. `command not found` 면 환경 점검 |
| 팀원끼리 서로를 못 찾음 (`to unknown label` / `unknown uuid`) | 팀원을 **순차** 기동해 먼저 뜬 팀원이 아직 존재하지 않는 동료에게 메시지 시도 | 단일 메시지 병렬 기동으로 해결. 이미 발생 시 `send_agent_message` 로 재지시 |
| 정리 단계에서 엉뚱한 pane 이 닫힘 | 라벨을 식별자로 보관 → 다른 창 닫힘으로 reflow → 보관 라벨이 다른 pane 가리킴 | uuid 를 식별자로 보관. `list_workspace` 의 라인 끝 `uuid=` 필드 사용. 라벨은 사람 가독성 디버그용 |
...
| 팀원 CC 죽음 | claude 프로세스/쉘 종료 | `list_workspace` 재확인. 팀원 재생성. 중간 결과는 `read_output` 으로 구출 |

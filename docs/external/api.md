...
**`/api/file/*` 와 `/api/git/*` 에는 경로 경계가 없습니다** (2026-09-20 ~). 절대경로면
어디든 읽고 씁니다 — 허용 루트 대조·홈 쓰기 금지·git `repo` 검사를 전부 제거했습니다
(사용자 결정, `FILE_API_BOUNDARY_SRS`). 남은 것은 형식 검사(빈 경로·상대경로)와 크기
상한뿐입니다. **접속한 기기는 서버를 띄운 계정의 권한으로 파일시스템 전체를 읽고
씁니다** — 무엇이 열렸는지는 [`SECURITY.md`](../../SECURITY.md) §4-5 에 있습니다.

`/api/fs/*` 는 **다릅니다** — 전부 `root` 를 함께 받아 **그 아래로만** 동작합니다. `root` 는 서버가 신뢰하지 않고 `editors.list`·홈·메모 루트 중 하나에 실재하는지 대조합니다 — 그 셋이 `Roots()` 의 전부이며, 메모장이 파일 조작을 얻는 것도 메모 루트가 그 목록에 들기 때문입니다. 오류는 `{code, message}` 이며 코드는 `bad_request`(400) · `outside_root`·`permission_denied`(403) · `not_found`(404) · `exists`(409) · `too_large`(413) · `io_failed`(500) 입니다.
...
| `0x05` | S→C | 크기 통보: `cols uint16 BE + rows uint16 BE`. 지금 PTY 의 크기다. 접속 직후 재생보다 앞에 한 번, 이후 크기가 바뀔 때마다 그 도구의 모든 클라이언트에게 |
| `0x06` | S→C | 응답 좌석: `owns uint8`(1=주인). **질의의 답을 보낼 자격**이다. 접속 직후 재생보다 앞에 한 번, 연결이 붙거나 떨어질 때마다 다시 |
...
터미널 프로토콜은 양방향이다. 앱은 터미널에게 **질의**를 내고(`ESC]11;?` 로 배경색을
묻는 식), 터미널은 그 답을 **PTY 입력으로** 돌려준다 — 사용자가 친 키와 같은 통로다.

한 도구에 클라이언트가 여럿 붙으면 그 질의가 **모두에게 방송되고 각자 답한다.** 앱은
답을 하나만 먹으므로 나머지는 입력 줄에 남아 프롬프트에 글자로 찍힌다. 그래서 서버가
도구마다 **답할 연결 하나**를 정하고 `0x06` 으로 알린다.

- 주인은 그 도구에 **가장 먼저 붙은 연결**이다. 포커스와는 무관하다 — 포커스로 가르면
  브라우저가 백그라운드일 때 아무도 답하지 않는다
- 연결이 붙거나 떨어질 때마다 그 도구의 **모든 연결**에 다시 보낸다. 승계가 곧 그
  통보다
- 클라이언트는 `owns=0` 이면 **질의의 답을 보내지 않는다.** 보내지 않을 것은 DA·DSR/
  CPR·DECRQM 응답·OSC 색 보고·DECRQSS 응답이다
- **포커스 보고(`ESC[I`·`ESC[O`)는 좌석과 무관하다** — 질의의 답이 아니라 그 창 고유의
  사실이므로 창마다 보낸다
- **사용자의 키는 좌석과 무관하다.** 어느 창에서 타이핑하든 같은 터미널이 움직인다

`0x06` 을 모르는 클라이언트는 그 프레임을 무시하면 된다. 그때는 자기가 주인이라고 보고
종전대로 답한다 — 모르는 것이 오류가 되지 않는다.

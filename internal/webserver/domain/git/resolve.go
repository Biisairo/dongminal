...
func (s *Service) Resolve(ctx context.Context, repo, side string, paths Paths) (Output, error) {
	if side != ResolveOurs && side != ResolveTheirs {
		return denied(), fmt.Errorf("%w: 알 수 없는 side: %q", ErrUnsafeArgument, side)
	}
	...
	out, applied, err := s.execPaths(ctx, repo, cp, true, len(cp), 0, "checkout", "--"+side)
	if err != nil {
		return out, err
	}
	// 받아 오지 못한 것을 해결됨으로 표시하면 충돌이 조용히 사라진다 — 위에서
	// 실패하면 여기 오지 않는다.
	out, _, err = s.execPaths(ctx, repo, cp, false, len(cp), applied-len(cp), "add")
	return out, err
}
...
func resolveHint(repo, side string, paths Paths) Hint {
	return Hint{
		Repo:    repo,
		Action:  ActionResolveSide,

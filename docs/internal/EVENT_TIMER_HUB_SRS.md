...
**범위 밖**: 서버(Go). 파일 감시(`fsnotify`) 도입으로 폴링을 푸시로 바꾸는 일은
별개의 SRS 다. 다만 이 구조가 그 이관의 **선행 조건**이다 — 상태별 갱신 경로가
...
| # | 항목 | 처리 |
|---|---|---|
| O-1 | 서버 `fsnotify` 도입으로 git 폴링을 푸시로 전환 | **별도 SRS.** 이 구조가 선행 조건이라는 것만 §1.3 에 적는다. 여기서 인터페이스를 예약하면 쓰이지 않을 추상화를 짓는 것이다 |

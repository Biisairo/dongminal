...
## A-1. 원인이 아직 없는 흔들림 둘 — 추적 대상

**전량에서만** 나고 단독으로는 통과한다 (§6.3). 사용자 지시는 "전부 추적" 이었고,
8차는 재현 조건을 좁히지 못한 채 남겼다.

  · `ui-layout-defaults` V-LAY-1 — 계산값 18곳이 어긋난다 (`overflowX
    visible → hidden` 계열). 부하 때 무엇이 달라지는지가 물음이다
  · `git-file-actions` F6 — 전량에서만 빈 배열

  추적의 첫 걸음은 **전량 실행에서 그 스펙만 격리**하는 것이다 — 워커 수를 줄여
  (`--workers=1`) 부하를 낮췄을 때도 나는지 보면 "부하" 인지 "동시 실행의 간섭"
  인지가 갈린다.
...
>
> **지금 남은 흔들림은 둘이며, 원인이 아직 규명되지 않았다:**
>
> | 검사 | 사실 | 남은 물음 |
> |---|---|---|
> | `ui-layout-defaults` V-LAY-1 | 전량에서만 계산값 18곳이 어긋난다(`overflowX visible → hidden` 계열). **단독으로는 통과** | 부하 때 무엇이 달라지는가 — 워커가 겹칠 때의 탭 구성인가, 스크롤바인가 |
> | `git-file-actions` F6 | 전량에서만 빈 배열. **단독으로는 통과** | 같은 물음 |
>

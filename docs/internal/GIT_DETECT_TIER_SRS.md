...
> **문서 상태**: 승인·구현완료
>
> 근거(2026-09-15 전수조사): 요구는 전부 섰다 — 회차 병렬은 `hub/gitwatch.go:444`
...
**FR-GDT-21a** 그 문구는 **회차마다 다시 쓴다** (2026-09-21, CI windows 가 잡았다).
`_paintRows` 가 빈 안내를 **만들 때만** 문구를 넣고 있었다. `initial` 은 서버
응답이 와야 서므로, 첫 그리기가 응답보다 빠르면 `GIT_HIST_EMPTY`(*"커밋이
없습니다"* — 필터에 걸리는 것이 없다는 뜻)가 박히고 **그 뒤로 영원히 남는다.**
응답이 온 회차는 `_ver` 가 올라 다시 그리지만, 요소가 이미 있어 문구 대입을
건너뛰었다.

- **이전**: `if(showEmpty && !empty){ … textContent=… }` — 생성 시 1회
- **새**: `if(showEmpty){ if(!empty){…} textContent=… }` — 회차마다
- **이유**: 두 문구는 **다른 사실**인데 순서가 어느 쪽인지를 정하고 있었다.
  darwin·ubuntu 는 응답이 먼저 와서 늘 초록이었고, 느린 러너(windows)에서만
  졌다 — 20초 동안 23회 같은 값으로 resolve 됐다

> §3-4 의 모양이다: **하네스가 한 판에서만 재고 있었다.** 이 코드는 리팩토링이
> 클래스 이름 한 곳만 건드렸고(`da7ff39d`), 결함은 그 전부터 있었다.

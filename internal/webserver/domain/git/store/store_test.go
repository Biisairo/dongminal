...
// porcelain v2 픽스처. 레코드는 NUL 로 끝난다 — 헤더(`# ...`)도 마찬가지다.
// core 의 파서 테스트와 같은 모양을 store 에서도 써야 하므로 여기에 둔다.
...
func (g *fakeGit) runner(_ context.Context, dir string, args []string) (core.Output, error) {
	key := args[0]
	...
	case "status":
		...
		return core.Output{Stdout: g.statusOut}, nil
	}

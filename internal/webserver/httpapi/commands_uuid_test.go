...
func TestHandleCommandPost_TranslatesUUIDLocation(t *testing.T) {
	...
	body := `{"action":"focus","args":{"location":` + jsonQ(uuid) + `}}`
...
func TestHandleCommandPost_FullStackUUID_ReflowSafety(t *testing.T) {
	...
	blob1 := `{"activeWindow":"sa","schemaVersion": 2, "windows":[
		{"id":"sa","name":"A","focusedPane":"ra","layout":{"type":"pane","id":"ra","activeTab":` + jsonQ(tabA) + `,"tabs":[{"id":` + jsonQ(tabA) + `,"name":"a","toolId":"10"}]}},
		{"id":"sb","name":"B","focusedPane":"rb","layout":{"type":"pane","id":"rb","activeTab":` + jsonQ(tabB) + `,"tabs":[{"id":` + jsonQ(tabB) + `,"name":"b","toolId":"20"}]}}
	]}`
	...
	payload := post(`{"action":"focus","args":{"location":` + jsonQ(tabB) + `}}`)
...
func TestHandleCommandPost_ResponseCoordinateRejected(t *testing.T) {
	...
	for _, badLoc := range []string{"W4.P1.T1", "4.1.1", "303"} {
		body := `{"action":"focus","args":{"location":` + jsonQ(badLoc) + `}}`
		resp, err := http.Post(ts.URL+"/api/commands", "application/json", strings.NewReader(body))
		if err != nil {

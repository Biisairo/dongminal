...
type Adapter struct {
	...
	// ParseHistory 는 전사본 **한 줄**의 뜻이다 (M9_SRS FR-M9-41 / M9-B23).
	//
	// `ParseUsage` 의 **옆자리**이고 규약도 같다 (FR-AAC-11): 파일을 열고·꼬리를
	// 자르고·줄을 가르는 일은 호출자의 몫이며, 어댑터가 아는 것은 한 줄이 어떤
	// 이벤트가 되는가뿐이다. 한 줄이 이벤트 여럿이 될 수 있어 배열을 돌려준다
	// (도구 결과가 여럿 담긴 user 줄).
	//
	// 이것이 있어야 하는 이유: **에이전트는 재개할 때 과거를 재생하지 않는다**
	// (실측 2026-09-14 — `--resume` 은 컨텍스트만 잇고 스트림에는 새 턴만 온다).
	// 그래서 올린 세션의 화면을 채우려면 기록을 우리가 읽어야 하고, 기록을 어디에
	// 어떤 형식으로 쌓는지 아는 자리는 어댑터다 (사용자 지적 2026-09-14).
	//
	// `nil` 이면 이 에이전트의 기록은 읽지 않는다. 그때 화면은 **빈 채로 열되 그
	// 사실을 문장으로 말한다** (FR-APS-4 — 부재가 뜻이다). 조용히 비면 사용자는
	// 세션이 이어지지 않은 줄 안다.
	ParseHistory func(line string) ([]Event, bool)

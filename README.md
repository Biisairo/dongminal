...
> ### ⚠ 노출해서 쓸 때 — 지금은 이렇게 하세요
>
> **아직 인증이 없습니다.** 허용 목록은 "어느 기기" 를 가르지만 "누구" 는 묻지
> 않습니다. 인증은 준비 중이고, 그때까지 이 셋을 지켜 주세요.
>
> 1. **`--expose` 에는 허용 목록을 켠다.** 켜지 않으면 서버가 뜨지 않습니다 —
>    되돌리는 길은 `--insecure-no-acl` 하나이고 그 이름이 곧 경고입니다.
> 2. **가능하면 Tailscale 같은 오버레이 망 뒤에서만 연다.** 평문 HTTP 이므로
>    같은 네트워크에서 트래픽이 읽힙니다.

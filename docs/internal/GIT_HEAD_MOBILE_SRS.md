...
**폐기됨** (2026-09-04, REPO_TAB_UNIFY_SRS D-RTU-30).

> 이 요구의 근거는 탭이 **고정**이라 사용자가 줄일 수 없다는 것이었다 — 들어가지
> 않으면 닿을 방법이 없었다. 통합이 뷰를 **필요할 때 열고 닫는 보통 탭**으로 만들면서
> (FR-RTU-30·33) 그 전제가 사라졌다. 남는 계약은 **닿을 수 있는가**이며
> `.pn-tabs{overflow-x:auto}` 가 그것을 준다 — 390px 에서 여섯 탭은 447px 을 쓴다.
>
> 그리고 `Changes` 는 본문 탭이 아니라 창의 **사이드**이며(FR-RTU-32), 모바일에서
> 사이드와 본문은 순회의 **자리 하나**씩이다 (FR-RTU-80 / D-RTU-29).
...
| V13 개정 | 390px 폭·모바일에서 뷰 탭이 여섯이고 탭 바가 가로로 스크롤되어 마지막 탭에 닿을 수 있다 (D-RTU-30) |
...
| 좁은 폭의 머리 | 잘린다 | 줄바꿈한다 |
| 모바일 Git 탭 | 일곱 번째가 화면 밖 | ~~일곱 개가 들어간다~~ → 여섯이 되고 탭 바가 스크롤된다 (D-RTU-30) |

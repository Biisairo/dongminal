...
| ~~**A**~~ | ~~`authGate` **자리 예약**~~ — **철회 (2026-09-11, 로드맵 결정 9)**. 인증을 도입하지 않으므로 자리를 제거했다 | ⊘ |
...
**FR-RQG-1 (개정 2026-09-11)** 체인은 `logging → accessGate(기기) → requestGate(출처)
→ recover → mux` 가 된다. **원문은 `requestGate` 와 `recover` 사이에 `authGate`(빈 자리)를
두었다** — 로드맵 결정 9(인증 미도입)로 그 자리를 제거했다(§3.6). `requestGate` 는 mux 바깥의 한 겹이며 판정 함수는 **한 곳**에
...
> **개정 — 항목 4 (2026-09-10, U-18).** 원문은 항목 4 가 "ACL 의 호스트명 항목" 이었다.
> 그 편의 규칙(`hasHostname`)은 **제거됐다** — 축①(출발지 IP)에 적은 *타 기기* 이름이
> 축②(이 서버가 불리는 이름)로 조용히 승격되는 자리였고, 그래서 사용자가 축②에 무엇을
> 적어야 하는지 알 방법이 없었다(MagicDNS 이름 접속이 421). 근거와 동작 변경 기록은
> `ACCESS_ALLOWLIST_SRS` FR-ACL-31·32 에 있다. **항목 1·2·3·5 는 그대로다.**

1. `localhost` · `127.0.0.1` · `::1`
2. 실제 바인드 주소
3. `accessStore.self` — 이 기계의 인터페이스 주소
4. **`access.json` 의 `hosts` 목록 — 활성 항목** (`ACCESS_ALLOWLIST_SRS` 묶음 H)
...
**Host 거절 본문은 고칠 수 있는 자리를 가리킨다** (2026-09-10, U-18): 원문은
`--allowed-host` 를 안내했는데 **그 플래그는 CLI 에 배선돼 있지 않아 사용자가 쓸 수 없는
수단이다.** 설정 ▸ 접속의 "이 컴퓨터의 별명" 을 가리킨다 (`ACCESS_ALLOWLIST_SRS`
FR-ACL-37). 목록 내용을 싣지 않는 규약은 그대로다.
...
**FR-RQG-20** ⊘ **철회** (M9, 2026-09-14 — `M9_SRS` FR-M9-1 / D-M9-1, 사용자 결정)

종전: `--expose`/`DONGMINAL_HOST` 로 노출하면서 ACL 이 꺼져 있으면 **기동을 거부한다.**
되돌리는 길은 명시 플래그 하나(`--insecure-no-acl`)이며 그 이름이 곧 경고다.

  이전 동작: 허용 목록이 없거나·꺼져 있거나·비어 있으면 사유를 찍고 exit 1
  새  동작: `--expose` 는 허용 목록의 상태와 무관하게 뜬다. `--insecure-no-acl` 도
            함께 사라졌다 — 게이트가 없으면 그것을 되돌리는 플래그도 뜻이 없다
  이유:     사용자 결정. 접수한 말은 "IP 필터가 꺼져 있는데도 IP 등록을 요구한다"
            이고, 그 읽기에서 꺼진 필터는 곧 제한 없음이다

**남은 위험은 사라지지 않았다.** 인증이 아직 없으므로, 허용 목록을 켜지 않고 노출하면
같은 망의 누구나 이 서버의 셸과 파일에 닿는다 — 이 조항이 막던 것이 바로 그것이다.
방어는 사용자의 손(`Settings ▸ Access`)으로 옮겼고, 화면의 노출 표시(`FR-RQG-24`,
`dmenv.ExposureLabel`)와 `start` 헬프가 그 사실을 말한다. 되돌리려면 D-M9-1 을 다시 연다.

`FR-RQG-21`(노출 모드의 fail-closed)과 `FR-RQG-24`(노출 판정 한 벌)는 **그대로다** —
둘은 게이트가 아니라 요청 처리와 표시의 조항이다.

**FR-RQG-21** 노출 모드에서 `access.json` 이 깨지면 **fail-closed**(loopback 만)다.
로컬 모드에서는 종전대로 fail-open 이다(`FR-ACL-2` 의 의도를 로컬에 한정한다).

**FR-RQG-24** (M5 `TLS-2`, 2026-09-12) **"노출인가" 의 판정은 한 벌이다** —
`dmenv.IsExposedHost`. **loopback 만 로컬이고 나머지는 전부 노출이다.**

판정이 두 벌이던 것이 결함이었다. 철회된 `FR-RQG-20` 의 게이트는 `host != DefaultHost`
로 보아 `::1`·`localhost` 바인드까지 허용 목록을 요구했고(밖에서 닿지 않는
주소인데 문을 잠그라고 했다), 기동 표시는 `0.0.0.0`/`::` 만 보아
`DONGMINAL_HOST=192.168.1.5` 를 `local-only` 라고 **거짓으로** 찍었다. 두 오차가
한 조합에서 만난다 — **게이트는 물면서 화면은 안전하다고 말한다.**

| 입력 | 판정 |
|---|---|
| `127.0.0.1` · `::1` · `localhost` · 빈 값 | 로컬 |
| `0.0.0.0` · `::` · `192.168.x` · `10.x` · 그 밖 | 노출 |

해석되지 않는 이름은 **노출로 본다.** 판정 자리에서 DNS 를 두드릴 수 없고, 틀릴
때의 대가가 한쪽으로 기운다 — 로컬을 노출로 보면 ACL 을 한 번 더 요구할 뿐이지만
노출을 로컬로 보면 무단 기기에 문이 열린다.

**두드릴 주소(`dmenv.DialHost`)는 이 판정과 기준이 다르다.** 미지정 주소
(`0.0.0.0`·`::`)만 `DefaultHost` 로 바꾼다 — `192.168.1.5` 는 노출이지만 그
주소로 두드리면 된다. 둘을 한 함수로 합치면 `--expose` 의 준비 확인이 깨진다.

**여기에 TLS 는 없다.** 이 항목의 ID 에 `TLS` 가 들어 있는 것은 감사 리포트의
번호일 뿐이고, 조치는 판정을 모으는 것 하나다. 기밀성은 제품이 제공하지 않고
오버레이 망에 위임한다(결정 9).

검사: `TC-RQG-34` 표 기반(`dmenv/host_test.go`) · `TC-RQG-35` 게이트와 표시가
같은 판정을 딛는가(`expose_gate_test.go`).

### 3.6 묶음 A — `authGate` 자리 예약 — ⊘ **철회 (2026-09-11)**

> **FR-RQG-22·23 은 철회됐다.** 로드맵 **결정 9**(2026-09-11): *"인증은 하지 않는다.
> 지금의 tailscale + ip 인증방식으로 갈무리한다."* 이어서 TLS 도 기각됐고, 인증
> 관련을 **완전히 뺀다**는 재확인이 있었다.
>
> **그래서 빈 `authGate` 를 체인에서 제거했다** (`reqgate.go`·`server.go`).
> 이 묶음이 세운 근거("빈 게이트가 있는 것이 미들웨어 자리를 다시 여는 것보다
> 싸다")는 **채울 M4 가 있을 때만 성립한다.** M4 를 영구 폐기했으므로 남은 것은
> 이득 없는 자리이고, **영구 폐기된 기능의 자리를 남겨 두면 그것이 곧 "예정" 으로
> 읽힌다.**
>
> 이 마일스톤이 배운 것은 그대로 남는다 — **체인의 순서가 계약이다.** 게이트는
> 이제 둘(`accessGate` → `requestGate`)이고 그 순서는 `FR-RQG-1` 이 정한다.
> "누구인가" 를 묻는 게이트는 **없으며, 생길 예정도 없다.**
>
> 아래는 철회된 원문이며 근거 기록이다.
...
| ID | 확인 |
|---|---|
| TC-RQG-22 | ⊘ 철회 (FR-RQG-20). 대신 `M9_SRS` V-M9-1 이 **기동한다**를 잰다 |
| TC-RQG-23 | ⊘ 철회 — `--insecure-no-acl` 이 사라졌다 |
| TC-RQG-24 | 노출 모드에서 `access.json` 손상 → loopback 만 통과 |
| TC-RQG-25 | 로컬 모드에서 손상 → 종전대로 fail-open |
| TC-RQG-34 | 노출 판정 표 — loopback 셋은 로컬, 미지정·사설·미해석 이름은 노출 (`FR-RQG-24`) |
| TC-RQG-35 | 노출 판정과 기동 표시가 **같은 자리**를 딛는다 — `::1` 은 둘 다 로컬, `192.168.1.5` 는 둘 다 노출 |
| TC-RQG-36 | `DialHost` 는 미지정 주소만 바꾼다 — `192.168.1.5` 는 그대로 두드린다 |
...
1. **인증.** ~~이 SRS 는 `authGate` 의 **자리와 계약**만 정한다.~~
   > **개정 (2026-09-11, 로드맵 결정 9)**: 인증은 이 SRS 의 비목표를 넘어 **제품의
   > 영구 비목표**가 됐다. 자리도 계약도 없다 — `authGate` 는 제거됐다(§3.6).
   > 원문은 아래와 같다. 세션·토큰·로그인 화면은
   M4 다. **그러므로 `SEC-3`(무인증 LAN 노출)은 이 마일스톤 뒤에도 열려 있다** — 운영
   안내를 문서에 임시로 둔다(FR-RQG-20 이 그 절반을 강제했다 — M9 에서 철회).
2. **TLS.** **영구 비목표다** (로드맵 결정 9, 2026-09-11 — *"tls 인증은 하지
   않을거야"*). 확정 결정 8(축소안)은 **철회**됐고 저장소에 `crypto/tls` 를 부르는
   코드를 넣지 않는다. 기밀성은 오버레이 망(Tailscale·WireGuard)에 위임한다.

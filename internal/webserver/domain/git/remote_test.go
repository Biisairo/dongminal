...
func TestJob_SanitizesCredentialsEverywhere(t *testing.T) {
	...
	jb, err := j.Start(jobRepo, "push", WriteSpec{Argv: []string{"push", "--progress", secretURL, "main"}})
	if err != nil {
...
func TestPushSpec_PublishSetsUpstream(t *testing.T) {
	...
	spec, plan, err := svc.PushSpec(context.Background(), jobRepo, PushOpts{Publish: true})
	if err != nil {
...
func TestPushSpec_PublishNeedsAcknowledgement(t *testing.T) {
	svc := remoteSvc(remoteFake{branch: "no-upstream", config: "remote.origin.url=/tmp/remote.git\n"})
	_, plan, err := svc.PushSpec(context.Background(), jobRepo, PushOpts{})
...
func TestPushSpec_WithUpstreamIsPlain(t *testing.T) {
	...
	spec, plan, err := svc.PushSpec(context.Background(), jobRepo, PushOpts{})
	if err != nil {
		t.Fatalf("PushSpec: %v", err)
	}
...
func TestPushSpec_DetachedIsRejected(t *testing.T) {
	...
	if _, _, err := svc.PushSpec(context.Background(), jobRepo, PushOpts{Publish: true}); !errors.Is(err, ErrDetachedPush) {
...
func TestPushSpec_ForceDefaults(t *testing.T) {
	...
	base, _, err := svc.PushSpec(ctx, jobRepo, PushOpts{})
	if err != nil {
		t.Fatalf("기본: %v", err)
	}
	...
	lease, plan, err := svc.PushSpec(ctx, jobRepo, PushOpts{Force: PushLease})
	if err != nil {
	...
	if _, _, err := svc.PushSpec(ctx, jobRepo, PushOpts{Force: PushForce}); !errors.Is(err, ErrForceConfirm) {
		t.Fatalf("확인 없는 --force err = %v, want ErrForceConfirm", err)
	}

	forced, _, err := svc.PushSpec(ctx, jobRepo, PushOpts{Force: PushForce, Confirm: true})
	if err != nil {
	...
	if _, _, err := svc.PushSpec(ctx, jobRepo, PushOpts{Force: "hard"}); !errors.Is(err, ErrPushForce) {
...
func TestRemoteSpecs_PassWriteGuard(t *testing.T) {
	...
	push, _, err := svc.PushSpec(context.Background(), jobRepo, PushOpts{Publish: true, Force: PushLease})
	if err != nil {
		t.Fatalf("PushSpec: %v", err)
	}

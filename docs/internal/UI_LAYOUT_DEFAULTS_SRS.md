...
#### 세 번째 회차 — `D-6` 이 상단바를 되살렸다 (2026-09-22)

크롬의 축이 **빈도**에서 **관여 범위**로 바뀌면서 DOM 이 다시 크게 움직였다
(`UIUX_OVERHAUL_SRS` §4.2.1). 상단바가 서고, pane 메뉴 `⋯` 와 상태바의 `⚡` 가
사라졌다.

**되돌림이 기준선을 조용히 되돌렸다.** `cad8b43d` 를 `git revert` 하자 그 커밋이
지웠던 `linux`·`win32` 가 되살아나고 **`darwin` 이 M6 이전(2211키)으로 덮였다** —
자동 병합이라 충돌로 뜨지 않았다. 되돌림으로 기준선을 다루면 이 일이 난다:
기준선은 **코드의 산출물**이지 코드가 아니다.

    옛 darwin(되돌려진 것) 2211 → 새 darwin 2219
    pn-acts 9 · pn-tabs-scroll 9 · pn-menu 0 · sb-act 0 · 상단바 버튼 69

**뜨는 회차를 골랐다.** 첫 write 가 `git:history` 행을 못 잡아 `지금에만 12` 로
흔들렸다 — 2026-09-21 의 그 교훈(*"조건부 요소를 품은 쪽이 안정적이다"*)대로 행이
잡힐 때까지 다시 떴고, 그 뒤 **다섯 회차 연속 재시도 없이 초록**이다.

**CI 가 두 판을 건넸다** (run `35656184446`, 샤드 7 · 두 OS — 나머지 열넷은 초록):

    darwin 2219 · linux 2219 · win32 2219
    linux vs win32    linux에만 0 · win32에만 0 · 값이 다른 자리 11
    darwin vs linux   darwin에만 0 · linux에만 0

**세 판의 키가 한 자리도 다르지 않다** — 정정(*"키 집합은 판을 타지 않는다"*)이 네
번째 회차에서도 선다. 드리프트 수는 그대로 `40` 이다.

...
| **V-M12-29** | Go 단위 | claude 의 종료 어휘 아홉이 `completed`·`stopped`·`error` 로 갈린다 — **`aborted_tools` 는 오류가 아니다** (FR-M12-23 의 RED). 사유 문자열은 `Text` 에 그대로 남는다 |
| **V-M12-30** | Go 단위 | codex 의 `turn.status` 셋이 같은 어휘로 온다 — 종류는 **상태**에서 나온다 (`failed` 의 `Text` 는 오류 문장이라 문장을 보면 갈래가 틀린다) |
| **V-M12-31** | Go 단위 | omp 의 `agent_end` 는 `completed` 다 — 중단은 `stopReason` 으로 따로 오고 그것은 이미 `EvError` 다 |
| **V-M12-32** | Go 단위 | **흉내를 원본에 맞췄다** — fake 가 거절된 턴을 `aborted_tools` 로 끝낸다. 종전에는 `completed` 를 내어 그 갈래가 재어지지 않았다 |
| **V-M12-33** | e2e | 도구를 거절한 턴이 **중단으로** 적힌다 — 오류 빨강도, 경과(`0.1초 걸렸습니다`)도 서지 않는다 |
| **V-M12-34** | Go 단위 | claude 의 기동 argv 가 **선언한 플래그 전부**(`PolicyInjection.Flags`)를 싣는다 · **깔려 있지 않으면 싣지 않는다** — 셸 래퍼의 `[[ -f ]]`·`[[ -d ]]` 와 같은 규약 (FR-M12-21) |
| **V-M12-35** | Go 단위 | 기동 argv 를 만드는 문이 **하나다** (`agentLaunchArgv`) — 새 세션과 재개가 같은 문을 지난다. 선언은 종전에도 있었고 **읽는 쪽이 터미널 표면뿐**이었다 |
| **V-M12-36** | e2e | GUI 에이전트의 슬래시 목록에 **`dongminal:migration` 이 선다** — 접수의 그 명령이다. **흉내를 원본에 맞췄다**: fake 가 `--plugin-dir` 의 `commands/*.md`·`skills/*/SKILL.md` 를 실제로 읽어 명령 목록에 올린다 |
...
| **codex·omp 의 프로토콜 기동에는 정책을 싣지 않는다** | FR-M12-21 의 접수는 claude 의 것이고 실측도 claude 뿐이다. omp 의 `--hook`·`--plugin-dir` 가 `--mode rpc-ui` 에서 같게 받아들여지는지, codex 의 `-c notify=…` 가 프로토콜 표면에서 뜻이 있는지를 **재지 않았다** — 재지 않은 플래그를 기동줄에 실으면 그 에이전트가 뜨지 않는다 (D-M11-4) |

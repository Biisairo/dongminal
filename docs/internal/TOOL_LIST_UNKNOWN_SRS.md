...
> **정정 (M8 P1 ④ `GO-46`, 2026-09-13)** — 타입 단언 규약은 폐기됐다. `ListOK`·
> `Connected`·`Daemon` 이 `ToolHub` 인터페이스의 메서드이고, 직접 모드는 언제나
> `(list, true)`·`true`·`nil` 을 답한다. fake 가 따라온 것이 그 대가였고, 그것이 곧
> "구현마다 이 물음에 답한다" 는 계약이다. `List()` 는 `[]ToolInfo`(`GO-13`)다.

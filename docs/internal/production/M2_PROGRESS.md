...
| # | 증상 | 첫 조사 지점 |
|---|---|---|
| U-1 | **LSP 로 파일이 연결되지 않는다** — **2026-09-11 증언 추가**: "파일 연결이 **전혀** 안 되어 있다. **import 한 객체를 커맨드클릭해도 이동하지 않는다**" | **실측이 증언을 둘로 가른다 (2026-09-11).** ① **커맨드클릭은 애초에 배선이 없다** — `EDITOR_LSP_SRS:159` 가 정의로 이동을 `monaco.languages.registerDefinitionProvider` 로 규정했는데 `web/js` 전체에 그 등록이 **한 자리도 없다**(있는 것은 `registerHoverProvider` 하나, `app-lsp.js:274`). Monaco 는 provider 가 없으면 Cmd/Ctrl+클릭 링크를 만들지 않는다 → **요구 미달**이다. ② **F12 경로는 있다** — `_lspGotoDef`(`app-lsp.js:180`)와 종단 `/api/lsp/definition`. 그러므로 **F12 가 되는가**가 첫 갈림길이다: 되면 결함은 ①뿐이고, 안 되면 세션·서버 기동이 따로 있다. ③ **어떤 검사도 실제 언어 서버를 지나지 않는다** — e2e 는 서버 응답을 **stub 하고**(`editor-lsp-nav.spec.ts` 머리), Go 는 fake 서버로 프로토콜만 잰다. "초록인데 안 된다" 의 이유가 그것이다 |
...
| U-19 | **하단 status bar 의 "Git 원격 작업 진행" 이 안 보인다. 뭔지도 모르겠다** (2026-09-10 접수) | ⚠ **절반 닫혔다.** ②(뭔지 모르겠다)를 닫았다 — 라벨이 `'Git 원격 작업 (진행 중일 때만)'` 이 되어 **순간 표시임을 말한다**(`GIT_FOLLOW_REMOVAL_SRS` `FR-FLW-12` 개정, 검증 `git-statusbar` B2). ①(작업이 없을 때도 무언가 보이게)은 **사용자 판정 대기** — 그것은 브랜치 chip 을 일부러 없앤 결정(`FR-FLW-12`)과 `FR-GIT-112` 의 개정을 동반하므로 판정 없이 채택하지 않는다. — 아래는 착수 전 조사다. **불만이 둘이다.** ① 안 보인다 — `_gitJobChip`(`app-git.js:852`)이 `if(!jobs.length) return null` 이라 **진행 중인 원격 작업이 실제로 있을 때만** 뜬다. fetch/pull/push 가 도는 몇 초뿐이므로 사실상 늘 없다(설정 기본값은 `def:true` 로 켜져 있다 — `helpers.js:353`). ② 뭔지도 모르겠다 — **설정의 이름이 그 사실을 말하지 않는다.** `'Git 원격 작업 진행'`(`STATUS_ITEMS.git`)은 상주 지표처럼 읽히는데 실제로는 순간 표시다. `FR-GIT-112` 는 "Git 창을 보지 않아도 알 수 있어야 한다" 이므로 **요구 자체는 그 동작이 맞다** — 고칠 것은 이름과, 그것이 무엇인지 알 방법이다. 먼저 판정할 것: 사용자가 원하는 것이 (가)이름을 알아볼 수 있게 하는 것인가 (나)작업이 없을 때도 무언가 보이는 것인가 — 후자면 `FR-GIT-112`·`FR-FLW-12`(브랜치 chip 을 **일부러 없앤** 결정) 개정을 동반한다 |
| U-20 | **편집기가 스크롤 값을 저장하지 않는다** — 다른 화면을 갔다 오면 스크롤이 **맨 아래**로 가 있다. **터미널에서도 기본은 스크롤 저장이 맞다.** 맨 위로 가는 현상(`U-3`)을 막는 것도 **스크롤 저장의 일부다** (2026-09-11 접수) | **착수 전 조사 완료 — 아래 상세.** 요체: 스크롤을 지키는 선례가 이 저장소에 **넷**이나 있는데(탐색기 `_scrollY` · 터미널 `_captureScrollOf` · git History `_top` · 사이드바 `__sbTop`) **편집기만 없다** — `file-editor.js` 에 `saveViewState`/`restoreViewState` 가 한 자리도 없다. 그리고 터미널의 복원은 **DOM 이 옮겨질 때만**(FR-PDR-10~12) 돈다 |
...
신설과 `hasHostname` 제거) · **U-9**(hunk 툴바를 커서로) · **U-19 ②**(상태바 항목의
이름) · **DoD 헤드리스 로그**를 닫았다.
남은 것은 **DoD 4항목(전부 값 판정 대기) · P2 · 사용자 보고 6건**, 그리고 기존
흔들림이다.

**첫 일은 U-20 이다 — 스크롤 저장.** 2026-09-11 에 접수됐고 **착수 전 조사가 끝나
있다**(§3.4 의 U-20 상세). 편집기가 스크롤을 저장하지 않아 다른 화면을 갔다 오면
맨 아래로 가 있고, 사용자는 **터미널도 기본이 저장**이며 **U-3(최상단으로 붙는다)을
막는 것도 이 일의 일부**라고 못 박았다.

요체 한 줄: **스크롤을 지키는 선례가 이 저장소에 넷이나 있는데 편집기만 없다** —
`file-editor.js` 에 `saveViewState`/`restoreViewState` 가 한 자리도 없다.
...
- docs/internal/production/M2_PROGRESS.md   ← 무엇이 끝났고 무엇이 남았는지
  ...
  · §3.4 사용자 보고(근거·함정·충돌) — 닫힌 열은 ✅ 로 표시돼 있다.
    **U-20 상세를 그대로 딛는다** — 선례 넷의 표, 두 갈래로 갈리는 물음,
    U-3 과의 관계, 규모·스펙 판단까지 적혀 있다. **다시 조사하지 마라**
  · U-1 상세도 실측으로 갈려 있다 — **커맨드클릭은 배선이 없고**(provider 미등록)
    F12 경로는 있다. **F12 가 되는가**가 첫 갈림길이다
...
이 스펙들의 범위 안이면 새로 쓰지 마라. **다만 "전부 구현돼 있다" 는 참이 아니다** —
아래 ⚠ 를 보라 (6차 세션이 그 하나를 실측으로 잡았다):
...
- docs/internal/FILE_API_BOUNDARY_SRS.md     파일 경계 계약
                                             ⚠ **FR-FAB-8·9(읽기 크기 상한)는 미구현**
                                             이다. apiFileRead 가 io.Copy 로 전량을
                                             싣고 413 이 없다 (GO-38 과 한 지점)
...
- docs/internal/EXPLORER_ROOT_KEYS_SRS.md    탐색기의 루트·생성·키보드 (5차)
                                             ← **§7.1 이 이 저장소에서 포커스·스크롤이
                                             왜 안 남는지를 적는다. U-20 이 여기 닿는다**
- docs/internal/POPUP_DEFAULT_ACTION_SRS.md  팝업의 기본 포커스 (5차)
- docs/internal/UI_LAYOUT_DEFAULTS_SRS.md    UI 레이아웃 기본값·게이트 셋 (5차)
                                             ← 게이트 `check-scroll` 이 스크롤 소유권을
                                             지킨다. **U-20 이 여기 닿는다**
- docs/internal/EDITOR_TAB_SRS.md            편집기 탭 (FR-EDT-66·68) ← **U-20**
- docs/internal/PANE_DOM_RECONCILE_SRS.md    DOM 재조정과 시선 (FR-PDR-10~12) ← **U-20**
- docs/internal/EDITOR_LSP_SRS.md            LSP — :159 가 정의로 이동을 Monaco
                                             provider 로 규정한다 ← **U-1**
- docs/internal/DIFF_HUNK_BAR_SRS.md         hunk 툴바 — U-9 로 **커서 트리거**가 됐다
...
**1) U-20 — 스크롤 저장 (편집기·터미널) ← 여기서 시작**

**사용자 요구가 셋이다.** ① 편집기의 스크롤을 저장·복원한다 ② **터미널도 기본이
저장**이다 ③ U-3(최상단으로 붙는다)을 막는 것도 **이 일의 일부**다.

§3.4 의 U-20 상세를 그대로 딛는다. 그 안에 있는 것:

  · 선례 넷의 표 — 탐색기 `_scrollY`(FR-EDT-68) · 터미널 `_captureScrollOf`
    (FR-PDR-10~12) · git History `_top` · 사이드바 `__sbTop`. **편집기만 없다**
  · 두 갈래로 갈리는 물음 — (i) 편집기 인스턴스가 사는데도 잃는다면 원인은 **DOM
    이동**이다. **U-14 의 선례를 먼저 읽어라**(EXPLORER_ROOT_KEYS_SRS §7.1):
    render 가 `.ed-win` 을 떼면 포커스는 빼앗기는 것이 아니라 **잃는다** — 스크롤도
    같은 자리에서 같은 방식으로 사라진다 (ii) 터미널 복원은 **DOM 이 옮겨질 때만**
    돈다(`renderer.js:438` 의 `moved`) — 창을 숨겼다 보이는 경로가 그 목록에 드는가
  · U-3 과 방향이 반대인데(맨 위 vs 맨 아래) 뿌리가 같을 수 있다는 근거 —
    `_restoreScrollOf` 가 `scrollToTop()` 을 지나는 갈래(`renderer.js:126-128`)

**중규모이고 스펙 개정을 동반한다.** 스크롤 소유권이 네 자리에 흩어져 있다. 새로
쓰기 전에 EDITOR_TAB_SRS(FR-EDT-66·68) · PANE_DOM_RECONCILE_SRS(FR-PDR-10~12) ·
UI_LAYOUT_DEFAULTS_SRS(게이트 `check-scroll`)를 **먼저 읽어라** — 다섯 번째 어휘를
만들면 게이트가 먼저 말을 건다.

**U-3·U-17 은 사용자가 재현을 진행 중이다.** 재현 조건이 오면 그것을 딛고, 오기
전에는 **①(편집기)부터** 한다 — 그쪽은 증상도 원인도 확정적이다.

**2) U-1 — LSP 로 파일이 연결되지 않는다**

**2026-09-11 증언**: *"파일 연결이 전혀 안 되어 있다. import 한 객체를 커맨드클릭
해도 이동하지 않는다."* 실측이 그것을 둘로 갈랐다 (§3.4 U-1 열).

  · **커맨드클릭은 애초에 배선이 없다.** EDITOR_LSP_SRS:159 가 정의로 이동을
    `monaco.languages.registerDefinitionProvider` 로 규정했는데 `web/js` 전체에 그
    등록이 **한 자리도 없다** (있는 것은 `registerHoverProvider` 하나,
    `app-lsp.js:274`). Monaco 는 provider 가 없으면 Cmd/Ctrl+클릭 링크를 만들지
    않는다 → **요구 미달**이다
  · **F12 경로는 있다** — `_lspGotoDef`(`app-lsp.js:180`) · `/api/lsp/definition`.
    그러므로 **F12 가 되는가가 첫 갈림길**이다: 되면 고칠 것은 provider 등록뿐이고,
    안 되면 세션·서버 기동이 따로 있다. **거기서부터 재현하라**
  · **어떤 검사도 실제 언어 서버를 지나지 않는다** — e2e 는 서버 응답을 **stub**
    하고(`editor-lsp-nav.spec.ts` 머리) Go 는 fake 서버로 프로토콜만 잰다.
    "초록인데 안 된다" 의 이유가 그것이며, 고친 뒤 **그 사실 자체를 검증으로
    남길지**를 판단하라

**3) 남은 결함 U-2·U-5·U-6, 그리고 DoD·P2**
...
DIFF_HUNK_BAR_SRS FR-DHB-11·13·14 와 V-DHB-10 을 함께 고친다.
  ...
  U-3  claude code·omp 에서 스크롤이 **최상단으로** 붙는다 — ⏳ 사용자가 재현 중.
       **U-20 의 일부다**(사용자가 묶었다)
  U-17 가끔 스크롤이 안 된다. 새로고침하면 풀린다 — ⏳ 사용자가 재현 중.
       **U-8 의 정적 결함과 성질이 다르다**(명시도 문제는 새로고침으로 안 풀린다)

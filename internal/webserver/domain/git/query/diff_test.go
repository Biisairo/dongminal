...
func (f *diffFake) run(_ context.Context, _ string, args []string) (core.Output, error) {
	f.argvs = append(f.argvs, append([]string(nil), args...))
	rev := args[len(args)-1]
	body, ok := f.blobs[rev]
	if !ok {
		p := strings.TrimPrefix(strings.TrimPrefix(rev, "HEAD:"), ":")
		return core.Output{ExitCode: 128, Stderr: "fatal: path '" + p + "' does not exist in 'HEAD'\n"}, nil
	}
	if args[0] == "cat-file" {
		if n, ok := f.sizes[rev]; ok {
			return core.Output{Stdout: strconv.FormatInt(n, 10) + "\n"}, nil
		}
...
func TestDiffContent_AxisArgv(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.axis, func(t *testing.T) {
			...
			dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, tc.axis, "p.txt", "")
			if err != nil {
...
func TestDiffContent_AxisSides(t *testing.T) {
	...
	for _, tc := range []struct{ axis, orig, mod string }{
		...
		dc, err := DiffContentOf(core.New(core.WithRunner(newFake().run)), context.Background(), repo, tc.axis, "p.txt", "")
		if err != nil {
...
func TestDiffContent_AddedFile(t *testing.T) {
	...
	f := newDiffFake()
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "new.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_UntrackedIsAbsent(t *testing.T) {
	...
	run := func(_ context.Context, _ string, _ []string) (core.Output, error) {
		return core.Output{ExitCode: 128, Stderr: "fatal: path 'u.txt' exists on disk, but not in the index\n"}, nil
	}
	dc, err := DiffContentOf(core.New(core.WithRunner(run)), context.Background(), repo, AxisWorktreeIndex, "u.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_InitialRepoHeadIsAbsent(t *testing.T) {
	...
	run := func(_ context.Context, _ string, _ []string) (core.Output, error) {
		return core.Output{ExitCode: 128, Stderr: "fatal: invalid object name 'HEAD'.\n"}, nil
	}
	dc, err := DiffContentOf(core.New(core.WithRunner(run)), context.Background(), repo, AxisWorktreeHead, "a.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_OtherFatalIsError(t *testing.T) {
	repo := diffRepo(t)
	run := func(_ context.Context, _ string, _ []string) (core.Output, error) {
		return core.Output{ExitCode: 128, Stderr: "fatal: unable to read object\n"}, nil
	}
	if _, err := DiffContentOf(core.New(core.WithRunner(run)), context.Background(), repo, AxisIndexHead, "a.txt", ""); err == nil {
...
func TestDiffContent_DeletedFile(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "gone.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_WorktreeDirIsAbsent(t *testing.T) {
	...
	f := newDiffFake()
	f.blobs[":d"] = "x\n"
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "d", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_Binary(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "b.bin", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_NULBeyondSniffWindowIsText(t *testing.T) {
	...
	f := newDiffFake()
	f.blobs[":late.txt"] = "x\n"
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "late.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_LFSPointer(t *testing.T) {
	...
	f := newDiffFake()
	f.blobs[":big.psd"] = ptr
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "big.psd", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
	for name, side := range map[string]DiffSide{"original": dc.Original, "modified": dc.Modified} {
...
func TestDiffContent_LFSPrefixTooLargeIsText(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "notlfs.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
	if dc.Modified.Kind != DiffKindText {
...
func TestDiffContent_TooLargeSkipsRead(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "huge.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_SizeAtLimitIsText(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "at.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_RenameUsesOrigPath(t *testing.T) {
	...
	f.blobs[":new.txt"] = "new\n"
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisIndexHead, "new.txt", "old.txt")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
...
func TestDiffContent_RejectsPathEscape(t *testing.T) {
	...
	for _, tc := range []struct{ name, path, orig string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := DiffContentOf(svc, context.Background(), repo, AxisWorktreeIndex, tc.path, tc.orig)
...
func TestDiffContent_MissingPathIsAbsentNotRejected(t *testing.T) {
	...
	dc, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "deep/gone.txt", "")
	if err != nil {
		t.Fatalf("DiffContent: %v", err)
	}
	if dc.Modified.Kind != DiffKindAbsent {
...
func TestDiffContent_BothAbsentIsError(t *testing.T) {
	repo := diffRepo(t)
	f := newDiffFake()
	_, err := DiffContentOf(core.New(core.WithRunner(f.run)), context.Background(), repo, AxisWorktreeIndex, "nowhere.txt", "")
...
func TestDiffContent_RejectsUnknownAxis(t *testing.T) {
	...
	for _, axis := range []string{"", "commit-parent", "worktree", "INDEX-HEAD"} {
		if _, err := DiffContentOf(svc, context.Background(), repo, axis, "a.txt", ""); !errors.Is(err, ErrDiffAxis) {
...
func TestDiffContent_RealGit(t *testing.T) {
	...
	dc, err := DiffContentOf(svc, ctx, repo, AxisWorktreeIndex, "added.txt", "")
	if err != nil {
		t.Fatalf("추가: %v", err)
	}
	if dc.Original.Kind != DiffKindAbsent || dc.Modified.Kind != DiffKindText {
	...
	dc, err = DiffContentOf(svc, ctx, repo, AxisWorktreeHead, "README.md", "")
	if err != nil {
		t.Fatalf("삭제: %v", err)
	}
	...
	dc, err = DiffContentOf(svc, ctx, repo, AxisIndexHead, "moved.txt", "README.md")
	if err != nil {
	...
	dc, err = DiffContentOf(svc, ctx, initial, AxisWorktreeHead, "only.txt", "")
	if err != nil {

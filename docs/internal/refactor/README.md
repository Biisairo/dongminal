...
| 묶음 | 내용 | 근거 | 위험 |
|---|---|---|---|
| **B0** | **게이트를 먼저 넓힌다** — 지금 새고 있는 네 통로를 막고, 새 검사 둘을 세운다. 탐침으로 검출을 확인하고 지운다 (규약 3-3) | 패턴 B | LOW |
| **B1** | **안전·정확성** — §3 의 1·2·3·8·9·10 | 패턴 C + 계약 누락 | MED |
| **B2** | **킷을 끝까지 적용한다** — `.ui-scroll` 33곳, 버튼 98개, 모달 골격, 포커스 링, `tabindex` | 패턴 A (UI) | MED |
| **B3** | **킷에 없는 컴포넌트 8종을 세우고 옮긴다** — `.ui-switch`·`.ui-empty`·`.ui-section-head`·`.ui-segment`·`.ui-split`·`.ui-notice`·`.ui-key`·배지 | uiux §1·§4 | MED |
| **B4** | **말을 고친다** — 낱말 통일, 영문 리터럴 113개, 조사 헬퍼, 존댓말 등급, 색 위반 25건 | uiux §2 · design | LOW |
| **B5** | **구조 정리** — 패턴 A 의 Go 쪽 잔여, 과대 함수·모듈 분할, `internal/shared` 응집도 | 각 보고서 | MED |

...
> **개정 (2026-09-14) — `M9_SRS` FR-M9-41 / D-M9-25.** **경로는 보낸다.**
>
>   이전 동작: 숫자와 세션 id 뿐. 경로조차 보내지 않았고 근거는 *"서버는 그 파일을
>             열 이유가 없다"* 였다
>   새  동작: `transcriptPath` 를 함께 보낸다. 서버는 그것을 메모리(`AgentSessionInfo`)
>             에만 들고, **올린 세션의 기록을 그릴 때** 로컬에서 연다
>   이유:     이유가 생겼다. 에이전트는 `--resume` 에서 과거를 스트림에 재생하지
>             않으므로(실측), 올린 세션의 화면을 채우려면 기록을 우리가 읽어야 한다.
>             그 파일이 어디 있는지 아는 것은 훅뿐이다
>
> **본문은 그대로 가지 않는다.** 서버가 **로컬** 파일을 여는 것과 훅이 내용을 실어
> 보내는 것은 다른 일이며, 잠금장치 둘은 대상이 좁혀졌을 뿐 살아 있다:
> `dmctl_activity_context_test.go` 의 카나리아(훅이 보낸 **모든 바이트**에 본문이
> 없다)와 `handlers_runs_context_test.go` 의 카나리아(`runs.json` 에 본문도 경로도
> 적히지 않는다). 경로는 `ContextObservation` 에 들지 않으므로 기록으로 가는 길이
> 없고, `AgentSessionInfo.TranscriptPath` 는 `json:"-"` 라 응답으로도 나가지 않는다.

...
| 9 | `Changes`·`Untracked` 의 Discard All (2026-09-05 접수) | ⬜ 스펙부터 (§5) |
...
| `Changes`·`Untracked` 의 **Discard All** | 세 가지. ① **그룹 머리에 버튼이 둘이 된다** — `GIT_GROUP_BULK` 가 그룹당 동작 하나만 담는 map 이라 자료구조부터 바뀌고, **좁은 폭에서 그룹 이름이 눌리지 않아야 한다**(NFR-RTU-6 의 첫 시험대다). ② **두 그룹의 discard 는 같은 명령이 아니다** — tracked 는 `checkout -q`, untracked 는 `clean -q -f`(파일 삭제)이므로 라벨을 가를지 정해야 한다. ③ 확인창이 밝힐 것 — 서버의 `discardHint` 가 "되살릴 값이 없다" 를 이미 적고 있으니 그것을 화면으로 끌어올릴지. 행 동작의 기존 원칙("파괴적인 것이 손에서 가장 멀다", `constants-git.js`)이 그룹 머리에도 적용된다 |

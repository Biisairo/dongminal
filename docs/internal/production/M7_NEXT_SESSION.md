...
> **이 문서의 일은 끝났다.** M8·M9·M10 은 **M8 통합** 하나가 됐고(`M8_UNIFIED_SRS`,
> 2026-09-13) P0 스파이크가 끝났다. **다음 세션이 읽을 것은
> [`M8_NEXT_SESSION.md`](./M8_NEXT_SESSION.md)(P1)** 이고 진행의 진실은
> [`M8_PROGRESS.md`](./M8_PROGRESS.md) 다. 아래는 M7 종료 시점의 근거 기록이다 —
> "변하지 않는 규약" 은 M8 인계서가 그대로 물려받았다.
...
M7 은 끝났다 (M7_PROGRESS §6). 다음은 **M8 통합**(옛 M8·M9·M10 을 하나로 —
`docs/internal/M8_UNIFIED_SRS.md`, 2026-09-13 초안)이다. **첫 단계는 P0 스파이크**
(그 문서 §4·§9.1): U-1~U-10 실측 · 기능 대조표 · 가짜 에이전트 판정 · 어댑터
프로토콜 필드 시안. 스펙은 아직 `초안` — 착수 전에 사용자 승인으로 `승인·구현중`.

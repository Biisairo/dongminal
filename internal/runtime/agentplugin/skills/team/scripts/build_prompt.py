...
`claude --model X "..."` 형태의 명령 문자열을 만든다. 이 문자열을
`dmctl send-input --at <팀원uuid> --execute -` 의 stdin 으로 넘긴다.

- 첫 작업 지시는 포함하지 않는다 (Barrier 뒤 Kickoff 단계에서 dmctl msg 로 전달).
- 답장 경로를 Bash + `dmctl msg` 로 못박고, 내장 SendMessage 오용 경고를 항상 포함.
- 따옴표 이스케이프 (`"` → `\\"`) 를 자동 처리해 쉘 파싱 안전.

식별자 — UUID 사용:
  `--my-label` / `--boss` / `--teammate <id>:<role>` 의 식별자 값은
  dmctl list-workspace / who-am-i 의 `uuid=<36자>` 필드를 그대로 넣는다.
...
출력은 단일 줄의 명령 문자열 (claude --model ... "..." 형태).
"""
...
def build(model, my_label, boss, role, teammates, process, reply_to, extra):
    ...
    body_parts += [
        "",
        "[답장 규칙]",
        "  반드시 Bash 로 dmctl msg 를 호출한다. 발신자는 자동으로 채워지므로",
        "  --from 을 넘기지 말고, who-am-i 도 부를 필요 없다.",
        "  아래 명령을 그대로 쓴다 — MSG 종료자는 반드시 줄 맨 앞(열 0)이어야 한다.",
        "",
        f"dmctl msg --to {reply_target} - <<'MSG'",
        ...
        "MSG",
        "",
        "  ※ 내장 SendMessage / SendUserMessage 는 완전히 다른 기능이다. 그걸 호출하면",
        "    메시지가 dongminal 채널에 도달하지 않는데 화면엔 '전송 완료' 로 보인다.",
        "    답장은 반드시 위 Bash 명령으로 한다.",
        f"  기본 답장 대상은 {reply_target} 이며, 프로세스에 따라 달라질 수 있다.",
        "",
...
def main():
    ...
    if args.json:
        json.dump({"send_input_text": text, "execute": True, "how": "dmctl send-input --at <uuid> --execute -"}, sys.stdout, ensure_ascii=False)
        print()
    else:
        print(text)

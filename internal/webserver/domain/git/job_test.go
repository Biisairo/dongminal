...
func jobSvc() *Service {
	return New(
		WithRunner(func(context.Context, string, []string) (Output, error) { return Output{}, nil }),
...
func jobFetchSpec() WriteSpec { return WriteSpec{Argv: []string{"fetch", "--progress"}} }
...
func TestJobStart_SameRepoIsBusy(t *testing.T) {
	...
	<-started
	if _, err := j.Start(jobRepo, "push", WriteSpec{Argv: []string{"push", "--progress"}}); !errors.Is(err, ErrJobBusy) {
...
func TestJobStart_RejectsNonRemoteKind(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if _, err := j.Start(c.repo, c.kind, WriteSpec{Argv: c.argv}); err == nil {
				t.Fatal("거부되지 않았다")
			}
		})
	}
...
func TestJob_AuthRequiredFromStderr(t *testing.T) {
	...
	jb, _ := j.Start(jobRepo, "push", WriteSpec{Argv: []string{"push", "--progress"}})
	done := jobWait(t, j, jb.ID, 3*time.Second)

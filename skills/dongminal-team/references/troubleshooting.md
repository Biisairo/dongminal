...
| 분할 후 `list_workspace` 에 새 도구 없음 | 분할 미반영 | `workspace_command` 응답 재확인, 필요 시 재시도 |
...
| 정리 단계에서 엉뚱한 탭이 닫힘 | 라벨을 식별자로 보관 → 다른 창 닫힘으로 reflow → 보관 라벨이 다른 탭 가리킴 | uuid 를 식별자로 보관. `list_workspace` 의 라인 끝 `uuid=` 필드 사용. 라벨은 사람 가독성 디버그용 |

...
- **D-6 뜬 자리가 처음 값이다.** 첫 프롬프트가 돌기 전까지 서버가 아는 사실은
  "어디서 띄웠는가" 하나뿐이고, 그것은 정확하다 — 승계(`cwdTool=…`)가 그 창을 딛는다.
...
| FR-WTC-6 | 도구가 **뜬 자리**(`StartTool` 의 `startDir`)를 처음 값으로 기록한다. 첫 프롬프트가 돌기 전까지는 그것이 서버가 아는 유일한 사실이며, 그 사이에 이 도구를 기준으로 새 도구를 만들면(`cwdTool=…`) 승계가 홈으로 떨어진다(러너 실측). 훅 보고가 오면 그것이 덮는다. | 필수 |
...
| V-WTC-6 | 갓 뜬 도구가 — 아직 어떤 출력도 오기 전에 — 자기 시작 자리를 답한다 (단위) |

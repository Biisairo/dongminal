...
- [x] **F1 프런트 If-Match 연동 완료 (2026-04-21)**: `static/app.js` `App` 에 `this.wsETag` 필드 추가, `init()` 에서 `GET /api/state` 응답의 `ETag` 헤더 저장. `_save()` 가 `If-Match: this.wsETag` 헤더와 함께 `PUT /api/workspace` 요청을 보내고, 200 응답의 `ETag` 로 `wsETag` 갱신. 409 수신 시 `console.warn('[workspace] stale revision; reloading')` 후 `GET /api/workspace` 로 최신 ETag 만 재동기화 — 로컬 편집 상태는 보존하고 자동 재시도 없음. 프런트 전용 변경, 백엔드 회귀 없음.
...
- [x] **H1 dropped_bytes 핫픽스 완료** (2026-04-21): `internal/outbuf/stream.go` Feed 의 `else if len(s.buf) > s.max` 분기 삭제 — `totalDrop` 은 실제 2*max 초과 compaction 시에만 누적. 기존 테스트 4개 + 신규 `TestNoPhantomDrops` 통과(`go build/vet/test ./internal/outbuf/...` OK).
- [x] **H2 list_panes 좀비 필터 완료** (2026-04-21): `internal/mcptool/tools/listpanes.go` 가 `WS.Entries()` 반환분 중 `PaneReader.Has(id)` 가 false 인 엔트리를 응답에서 제외. 응답 포맷·설명 문구는 변경 없음. `seen` 은 원본 entries 기준으로 구성해 orphan 판정 로직은 동일. `read_pane_*`/`send_input`/`send_agent_message` 는 `wsMgr.Resolve`→`IsLive` 경로로 이미 dead pane 에 error 반환(확인). `registry_test.go` 에 `TestListPanesFiltersDeadEntries` 추가, `go build/vet/test ./internal/mcptool/...` OK.
- [x] **F2 ToolDispatcher Stage 2 완료** (2026-04-21): `internal/mcptool/registry.go` 에 제네릭 `Register[A any](r, name, spec, fn)` + 내부 `genericTool[A]` + `Textf(format, a...)` 헬퍼 추가. `genericTool.Call` 은 `json.Unmarshal` 실패 시 `ErrorResult("잘못된 인자: ...")` 를 반환하고 `error` 는 nil (Stage 1 tool 들의 err→isError 매핑과 정합). 스키마는 별도 인자로 전달해 `tools/list` 응답 포맷 동일성 유지. `registry_test.go` 에 `TestRegisterGeneric`/`TestGenericInvalidJSON`/`TestTextf` 3개 추가. 샘플 마이그레이션: `tools/whoami.go` 의 `WhoAmI` struct 제거 → `WhoAmIName`/`WhoAmISpec`/`WhoAmIArgs`/`WhoAmIDeps`/`WhoAmIHandler` 로 분해 + `main.go` 의 등록을 `mcptool.Register(toolRegistry, tools.WhoAmIName, tools.WhoAmISpec, tools.WhoAmIHandler(...))` 로 교체. 기존 struct-based 6개 tool 은 그대로. LOC: whoami.go 49→46 (-3, Spec/Call receiver 제거 + `Textf` 로 `TextResult(fmt.Sprintf(...))` 2건 축약). `go build/vet ./...` + `go test ./internal/mcptool/...` 통과.
- [x] **F4 Server DI Stage 5c 완료** (2026-04-21): `internal/server/deps.go` 신설 — `PaneHub`/`CodeServerHost`/`WorkspaceStore`/`ToolDispatcher`/`CommandBroker`/`SettingsStore` 6개 인터페이스 + `Deps` 구조체 consolidate. `Server` 필드(`Panes`/`CS`/`Work`/`Tools`/`Commands`/`Settings`)를 구체 타입 포인터에서 인터페이스 타입으로 교체. 기존 구체 타입(`*PaneManager`/`*CodeServerManager`/`*workspace.Manager`/`*mcptool.Registry`/`*CommandHub`/`*settingsStore`)은 인터페이스를 자연스럽게 만족 — 어댑터 불필요. `main.go` `buildDeps` 는 `builtDeps{deps, pm, csm, wsMgr}` 를 반환해 shutdown 시 `pm.SaveAll`/`csm.StopAll`/`csm.Watchdog` 등 `Server` 표면 밖 메서드를 구체 타입으로 유지. YAGNI: 각 인터페이스는 `handlers_api.go`/`handlers_ws.go`/`mcp.go`/`commands.go` 실제 사용 메서드만 포함(`PaneHub`=4/`CodeServerHost`=5/`WorkspaceStore`=7/`ToolDispatcher`=2/`CommandBroker`=3/`SettingsStore`=3). `CommandBroker`/`SettingsStore` 는 패키지 내부 전용이라 unexported 메서드 유지. `internal/server/fakes_test.go` 신설 — 6종 fake(`fakePaneHub`/`fakeCodeServerHost`/`fakeWorkspaceStore`/`fakeToolDispatcher`/`fakeCommandBroker`/`fakeSettingsStore`). `internal/server/di_test.go` 신설 — `TestHandlerPanesGetUsesFake`(GET /api/state), `TestHandlerWorkspacePutIfMatch`(ErrStale→409+ETag), `TestMCPDispatchUsesFakeTools`(POST /mcp/message → fake Dispatch 호출 인자·세션 채널 응답 검증). `go build/vet/test ./...` 전부 통과, 기존 테스트 회귀 0.
- [x] **H3 file drop 프롬프트 복구 완료** (2026-04-21): `static/app.js` `_uploadFiles` 의 `uploadNext` 종료 분기(`i>=files.length`)에서 `OP.INPUT(0)+0x0d` 2바이트를 `this._send` 로 전송 — 업로드 성공·실패 경로 모두 재귀적으로 이 분기에 도달하므로 최종 1회만 CR 전송되어 쉘이 새 프롬프트를 자동 출력. 프런트 전용 변경, `go build/vet/test ./...` 통과.
- [x] **H4 pane 제거 시 포커스 이동 버그 수정 완료** (2026-04-21): `static/app.js` `closeTab` 의 빈-탭 분기에서 `closestRg(s.layout, rid)` 호출을 `doRemove` 이전으로 이동 → `prevClosestId` 로 보관 후 재검증. 기존엔 `doRemove` 이후 rid 가 트리에 없어 `findPath`→null → `closestRg` 가 `firstRg` 로 폴백, 항상 P1 로 포커스가 튀는 문제. 활성 세션(`isActive && this.focused===rid`) 과 비활성 세션(`s.focusedRegion===rid`) 양쪽 모두 `prevClosestId` 를 1차 타겟으로 사용하고, `findRg` 재검증 실패 시에만 `firstRg` 폴백 유지. `closeTab` 외 함수·`closestRg`/`firstRg`/`doRemove` 구현 불변. 프런트 전용, `go build/vet/test ./...` 통과.
- [x] **H6 code-server 안정화 완료** (2026-05-11, `CODESERVER_STABILITY_SRS.md`): (A/FR-A1) `internal/server/codeserver.go` 에 `stripOSC777` + `osc777Pattern` 추가, `internal/server/handlers_ws.go` 스냅샷 송출 직전(`pane.stream.Snapshot()` 직후) 1회 strip 적용 — WS 재연결/페이지 리로드 시 OSC 777(`OpenCodeServer`/`Download`/`CodeServerList`/`Cwd`) 가 재실행되어 의도치 않은 창이 다시 열리는 P1 해결. 라이브 `Pane.broadcast` 경로는 무변경. (B/FR-B1) `web/app.js` 의 `window.addEventListener('beforeunload', ...)` 일괄 stop 블록 제거 — 터미널 탭 새로고침이 다른 창의 code-server 인스턴스를 죽이는 P3 해결. (C/FR-C1) `CodeServerManager.Start` 가 `filepath.Abs` 정규화 후 `m.insts` 순회로 동일 folder 가진 인스턴스 발견 시 `LastPing` 갱신 후 즉시 반환 — `code-server` 바이너리 spawn 없음. P4 인스턴스 누적 해결. (D/FR-D1) `watchdogStale` 패키지 변수 도입(30s→90s), 프론트 `document.addEventListener('visibilitychange', ...)` 추가하여 가시 전환 시 활성 watcher 마다 hb 1회 즉시 송신 — 백그라운드 탭 `setInterval` throttling 으로 인한 P5 disconnect 해결. (E/FR-E1·E2) `codeServerTrack` 멱등화: 동일 id 재호출 시 prev.win===win → 즉시 return, prev 가 살아있고 새 win 도 살아있으면 새 창을 닫고 추적 유지, prev 가 이미 닫혔을 때만 교체. `_openCodeServer` 첫머리에서 살아있는 동일 id 창 있으면 `focus()` 만 호출하고 신규 open 생략. 테스트 6개 신규(`TestStripOSC777_NoSequence`/`_RemovesOpenCodeServer`/`_PreservesAnsiEscape`/`_IncompleteSequenceUnchanged`, `TestCodeServerManager_Start_ReusesFolder`, `TestWatchdogThreshold`). 기존 `TestCodeServerManager_Watchdog` 의 인라인 `30*time.Second` 를 `watchdogStale` 참조로 갱신. `go build/vet ./...` + `go test ./... -count=1 -race -vet=off` 모두 그린.
- [x] **H5 창 분할/삭제 지연 수정 완료** (2026-04-21): 원인은 `workspace.Manager.Save` 가 HTTP 핸들러 hot-path 에서 `os.WriteFile` 를 동기 호출(fsync 지연이 keystroke latency 로 누출)하고, `loggingMiddleware` 가 `/api/workspace` PUT·`/api/panes` DELETE 를 초당 수십 건 로깅하며 CPU/I/O 를 점유. Track B — `workspace/manager.go` 에 `writeCh chan []byte`(size 1) + writer goroutine + latest-wins coalescing(`enqueueWrite`: non-blocking send, 큐가 차면 이전 blob drop 후 재시도) 도입. `Save` 는 `buildIndex` → atomic swap(`raw`/`idx`/`rev`) → `enqueueWrite` 후 즉시 리턴. 리턴 시점에 in-memory state/rev 는 확정되어 기존 UX semantics 불변. `Close() error` 추가 — `done` 채널 close 후 writer goroutine 이 pending blob 을 flush 하고 exit(`sync.Once` 로 multi-call 안전). `main.go` shutdown 경로에 `bd.wsMgr.Close()` 호출 추가. Track A — `internal/server/server.go` `loggingMiddleware` 를 `shouldLogRequest(path, status)` 로 분리: `status>=400` 항상 로그, `/api/ping`·`/api/stats`·`/api/workspace*`·`/api/panes*` 는 성공 시 스킵. 측정(임시 계측 후 제거): slow Persister(100ms delay) 로 연속 5 회 Save. before=101.7 ms/call (buildIndex ~0.1ms + storeWrite ~100ms + swap ~µs) / after=18 µs/call (buildIndex ~13µs + swap ~0.3µs + enqueue ~0.3µs). 테스트 3개 추가(`TestSaveIsNonBlocking`: 100ms slow persister 로 Save <20ms 확인, `TestSaveCoalescing`: gated persister 로 N=10 Save 시 실제 write ≤2 확인, `TestCloseFlushesPending`: Close 후 최신 blob 이 disk 에 기록됨 확인). 기존 `TestSaveRevIncrement` 는 Close 후 검증으로 조정. `go build/vet/test ./...` 전부 통과.
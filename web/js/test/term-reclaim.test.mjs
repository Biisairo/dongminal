...
/**
 * OWNER_TRANSFER_REPLAY_SRS FR-OTR-1 이 **V-M10-3 의 이 절반을 개정했다.**
 *
 *   이전 동작: 따라가다 폭이 바뀌면 그 자리에서 전량 재생을 요청했다
 *   새  동작: 빚으로 적어 두고 **되찾는 자리**(`ptySize`)에서 갚는다
 *   이유:     소유권이 한 번 움직일 때마다 되찾는 쪽과 잃는 쪽에서 1 MiB 재생이
 *             두 벌 돌았다 — 접수된 "0.7~0.8초" 의 몸통이다. 잃는 쪽의 것은
 *             아무도 기다리지 않는다: 그 화면은 dim 이고, 원 설계 결정 E-7 이
 *             *"off-focus 시 dim 이 되므로 정상 동작"* 으로 이미 못박았다
 *
 * **조건은 개정되지 않았다** (FR-M10-2 그대로). 되찾은 화면은 반드시 제 폭의
 * 그림을 받는다 — 빚의 전 생애는 `term-transfer-replay.test.mjs` 가 잰다.
 */
test('따라가 폭이 바뀐 쪽은 재생을 미룬다 — 갚는 자리는 되찾기다 (FR-OTR-1·2)', () => {
  const h = pane({ owner: true });
  h.p.doFit();
  h.release();
  const c = countRefresh(h.p);
  ...
  h.claim();
  h.p.ptySize();
  assert.equal(c.n, 1, '되찾았는데 미뤄 둔 재생이 사라졌다 — 미루기는 없애기가 아니다');
});

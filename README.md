...
**Settings ▸ Access** 에서 접속을 허용할 출발지를 지정합니다. IP
(`100.117.248.111`) · 대역(`192.168.0.0/24`) · 이름(`macmini`) 을 넣으면 그
출발지만 들어옵니다. 서버가 도는 컴퓨터에서는 목록과 무관하게 언제나 접속되므로
잘못 설정해도 되돌릴 수 있습니다.

> ### ⚠ 노출해서 쓸 때
>
> **이 제품은 인증을 지원하지 않습니다.** 허용 목록은 "어느 기기" 를 가르고,
> "누구" 는 묻지 않습니다. 준비 중인 기능이 아니라 **확정된 경계**입니다.
>
> **그리고 접속은 곧 전권입니다.** 들어온 기기는 서버를 띄운 계정의 권한으로
> **파일시스템 전체를 읽고 씁니다** — 편집기와 git 종단에 경로 경계가 없습니다
> (2026-09-20 ~, 사용자 결정). `~/.dongminal` 아래도 예외가 아니어서 허용 목록
> 파일 자체를 덮을 수 있습니다.
>
> 그래서 아래 셋은 "지켜 주세요" 가 아니라 **필수**입니다.
>
> 1. **Tailscale 같은 오버레이 망 뒤에서만 연다.** 권고가 아니라 **전제**입니다 —
>    평문 HTTP 이므로 그 밖에서는 같은 네트워크에서 트래픽이 읽힙니다.
> 2. **`--expose` 에는 허용 목록을 켠다.** 켜지 않으면 서버가 뜨지 않습니다.
>    되돌리는 길은 `--insecure-no-acl` 하나이고 그 이름이 곧 경고입니다.
> 3. **공용 Wi-Fi 에서는 `--expose` 를 쓰지 않는다.** `DONGMINAL_HOST` 를 셸
>    프로필에 넣어 두었다면 그것도 같습니다 — 환경변수 하나로 노출됩니다.
>
> **무엇을 보증하고 무엇을 보증하지 않는지는 [`SECURITY.md`](./SECURITY.md) 에
> 전부 적혀 있습니다** — 배포 형태별 보증, 허용 목록의 한계 넷, 그리고 우리가
> 고치지 않기로 한 잔여 위험 넷까지. 노출해서 쓰실 거라면 한 번 읽어 주세요.
...
**창을 한 화면에 여럿 놓습니다.** 상단 `− Slot +` 로 칸을 넷까지 늘리고
(`Ctrl+Shift+S` · `Ctrl+Shift+X`), 칸마다 다른 창을 놓습니다 — 왼쪽에 터미널,
오른쪽에 Git 같은 배치입니다. 브라우저 창을 여럿 띄우던 자리를 대신합니다. 칸
사이 이동은 분할 칸 이동 키(`Ctrl+Shift+←→`)가 그대로 이어받으므로 외울 키가
늘지 않고, 칸 배치는 브라우저 탭마다 따로 기억되어 데스크톱은 세 칸, 휴대폰은
한 칸으로 볼 수 있습니다.
...
**탐색기는 바깥의 변경을 스스로 따라갑니다.** 터미널에서 파일을 만들거나
`git checkout` 이 파일을 더하고 지워도 새로고침을 누를 필요가 없습니다. git 이
없는 경로에서도 같습니다.

### 메모장

`Editor` 탭 맨 아래 **메모장** 행이 `~` 위에 고정으로 있습니다. 누르면 메모 전용
창이 서고, 그 안에서는 여느 편집기와 똑같이 씁니다 — 탐색기의 `새 파일` 로 메모를
늘리고, 분할해서 둘을 나란히 놓고, `Cmd+S` 로 저장합니다.

메모는 **서버의 `~/.dongminal/notes/` 에 그냥 파일로** 삽니다. 브라우저를 닫아도,
다른 기기에서 접속해도 그대로 있고, 터미널에서 `grep` 으로 뒤지거나 git 으로
버전을 매길 수도 있습니다.
...
테마 54종(다크 43 · 라이트 11)과 커스텀 편집기가 있습니다. 단축키는 전부 바꿀 수
있고, 하단 상태 표시줄에 무엇을 띄울지도 고릅니다. 브라우저 탭 이름을 바꿔 두면
여러 대를 띄웠을 때 어느 것이 어느 기계인지 탭 줄에서 갈립니다.

**글자 크기는 두 자리입니다.** *Display ▸ UI 글자 크기*(80~200%, 기본 100%)가
사이드바·탭·설정 같은 앱 화면을, *Terminal ▸ 터미널 글자 크기*(8~32px, 기본 14px)가
터미널 안을 정합니다. 서로 영향을 주지 않고, 터미널 쪽은 **열려 있는 터미널에 곧바로**
적용되며 칸 수를 다시 계산해 셸에 알립니다. *Terminal ▸ Claude Code 스크롤 속도*
(1~20줄, 기본 3줄)로 휠 한 번에 넘어갈 줄 수도 정합니다 — 이것은 **새로 여는
터미널부터** 적용됩니다.

**설정 → Backup** 으로 이 창에서 설정할 수 있는 값 전부를 JSON 파일 하나로 내보내고
그 파일로 되돌립니다. 새 기계를 세울 때 테마·단축키·상태바·프리셋을 손으로 다시
짜지 않아도 됩니다.

> 가져오기는 **전체 교체**입니다 — 파일에 없는 설정은 기본값으로 돌아가고 되돌릴 수
> 없습니다. 무엇이 덮이는지 보이고 한 번 확인을 받습니다.

### 언어

UI 는 **한국어(`ko`)와 영어(`en`)** 로 나오고 기본은 한국어입니다. 브라우저 언어를
읽어 자동으로 고르지 않습니다 — **설정 → Display → 언어**에서 고른 값이 유일한
기준이고, 바꾸면 페이지가 다시 열립니다. 번역이 빠진 문장은 한국어로 나오고 브라우저
콘솔에 경고가 남습니다.

범위 밖은 셋입니다 — `dongminal`·`dmctl` 의 **터미널 출력**, 서버 **로그**, 서버
**오류 응답의 본문**(코드 `X-Error-Code` 로 화면 문장을 고르므로 본문은 그대로입니다).
근거와 규약은 [통합 일정 SRS](docs/internal/M8_UNIFIED_SRS.md) §3.3 에 있습니다.

### 접근성

이 앱의 목표 기준은 **WCAG 2.1 Level AA** 입니다. 테마 54종 전부에서 글자 대비가
기준을 넘도록 색을 **파생**하고(팔레트 원본은 그대로 둡니다), 포커스 표시·움직임
줄이기(`prefers-reduced-motion`)·모바일 터치 타겟을 검사로 지킵니다.

범위 밖은 둘입니다 — **터미널 화면(xterm)의 안쪽**과 **편집기 위젯(Monaco)의
안쪽**. 그 둘은 각자의 접근성 모드를 갖고 있고 그쪽이 소유합니다. 무시하는 것이
아니라 **어디까지가 우리 몫인지**를 적어 둔 것이고, 근거와 재검토 조건은
[접근성 기준 문서](docs/internal/ACCESSIBILITY_BASELINE_SRS.md) §6 에 있습니다.
...
| 창 슬롯 더하기 · 빼기 | `Ctrl+Shift+S` · `Ctrl+Shift+X` |
| 사이드바 탭 (Windows · Git · Editor) | `Ctrl+Shift+1` · `2` · `3` |

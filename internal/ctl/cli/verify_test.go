...
var verifyGolden = []string{
	...
	"경계|읽기 상한 초과 413",
	"경계|정적 응답의 보안 헤더",
	// "경계|노출 ACL 게이트" 는 M9 가 지웠다 (FR-M9-1 / D-M9-1) — 게이트가 없으니
	// 물을 사실도 없다. "경계|허용 루트 밖 파일 읽기 403" 도 같은 경위로 빠졌다 —
	// `FILE_API_BOUNDARY_SRS` 묶음 B·W·G·O 의 폐기(2026-09-20)가 그 경계를 없앴다.
	// 이 표는 **골든**이므로 빠진 것도 늘어난 것도 여기서 걸린다.
}

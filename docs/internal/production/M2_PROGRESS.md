...
**P0 5건·P1 11건이 끝났고, 사용자 보고 21건 중 19건이 닫혔다** (8차 세션이 `U-2` 를
더했다). **A 갈래(값·결정 판정)가 전부 닫혔다** — §2.21.
...
| 갈래 | 내용 |
|---|---|
| ~~**A. 판정 대기**~~ | ✅ **닫혔다 (2026-09-11, §2.21)** — 값 다섯 · 결정 셋 · `U-2` 를 한 번에 받아 스펙→테스트→구현으로 넣었다 |
| **B. 사용자 환경 확인** | `U-21` 실행 확인 **셋**(`V-OMP-20` 은 사용자가 확인했다) · `U-1` 의 언어 서버 기동. **서버 재시작이 먼저다** |
| ~~**C. 재현 대기**~~ | ✅ **`U-3`·`U-17` 닫혔다** (2026-09-11 2차, M4 착수 전). 둘은 **한 결함**이었고 원인이 실측으로 확정됐다 — `VIEW_SCROLL_RESTORE_SRS` §2.8 · `FR-VSR-24·25`. 9차의 `FR-VSR-22` 는 불변식으로 남되 결함에는 닿지 않았다 |
...
### 2.21 값 판정 아홉을 받아 한 번에 넣었다 (8차 세션, 2026-09-11)

**A 갈래가 닫혔다.** 코드는 준비돼 있고 값·결정만 없던 자리들이며, 사용자 판정을
한 번에 받아 스펙 → 테스트(RED) → 구현으로 넣었다.

| 자리 | 값·결정 | 들어간 곳 |
|---|---|---|
| 읽기 크기 상한 | **10 MiB** | `FR-FAB-8`(서버 413 + `LimitReader`) · `FR-FAB-9`(클라 게이트) |
| `wait` 동시 수 | **32**, 초과 429 | `FR-STA-9` 신설 (`handlers_status.go`) |
| diag 임계 | `goroutines>500`·`allocMB>256`·`ws>100`·`hold>32` | `FR-CNR-13` 신설 |
| 샌드박스 자원 | `cpus=2`·`memory=2g`·`pids=512` | `FR-SBX-45` 신설 (비목표에서 승격) |
| zip 동시 | **2**, 초과 429 | `FR-ETR-45` 신설 |
| `verify` 항목 | 경계 넷 + 노출 ACL 게이트 | `verify_boundary.go` 신설 (검사 22→27) |
| `SEC-10` | **조치 불요** | 게이트는 이미 환경변수 경로에도 걸린다 (아래) |
| `SEC-15` | git `repo` 도 같은 경계로 | `FR-FAB-14·15` 신설 (`RepoGuard` 주입) |
| `SEC-16` | 홈 안 쓰기는 **노트만** | `FR-FAB-12·13` 신설 (묶음 W) |
| `U-2` | 미리보기 버튼 좌상단 → **좌하단** + 강조색 | `FR-DRB-3`·`3a` 개정 |

**`SEC-19`·`FUI-06`·`GO-38` 이 값 하나로 함께 닫혔다** — 예고된 대로다.

#### 값을 두 벌로 두지 않았다

`FR-FAB-9` 는 "서버와 같은 값을 쓴다" 를 요구했다. 상수를 양쪽에 적는 대신
**`/api/file/probe` 가 `maxBytes` 를 실어 보내고** 편집기가 그것으로 판정한다.
클라이언트에 숫자가 없으므로 한쪽만 고쳐질 자리가 없다. 옛 서버에 붙어 `maxBytes`
가 없으면 게이트를 걸지 않는다 (`FR-EVW-8` 과 같은 관대함).

#### `SEC-10` 은 조사로 닫혔다 — 차단은 이미 돌고 있었다

`start.go:52` 의 `host != DefaultHost && !o.InsecureNoACL` 게이트는 `--expose` 뿐
아니라 **`DONGMINAL_HOST` 경로에도 그대로 걸린다.** 감사 항목이 가리킨 "조용한
노출" 중 실제로 남은 것은 **표기**였다 — `start.go:154`·`280` 이 `0.0.0.0`·`::` 만
"LAN 노출" 로 부르므로 `DONGMINAL_HOST=192.168.0.5` 같은 구체 LAN IP 는
`local-only` 로 출력된다. 사용자 판정은 **조치 불요**다.

> 이것이 §"비싸게 배운 것" 의 첫 줄을 또 한 번 확인해 준다 — **감사 항목을 결함이라
> 단정하지 마라.** `SEC-14`·`SEC-12`·`FE-15`·`SEC-18` 에 이어 다섯 번째다.

#### 조사 중 나온 것 — 홈 전체가 파일 API 루트다

`fileRoots` 는 `Entries.Roots()` 를 **서브트리 prefix** 로 쓰는데,
`wsentry.Store.Roots()[0]` 은 `os.UserHomeDir()` 이다. 그래서
`file_boundary.go` 의 *"홈 디렉터리 전체는 루트가 아니다"* 라는 주석은 **사실이
아니었다.** 실측: 실제 인스턴스에서 `/api/file/read?path=~/.ssh/known_hosts` 가
**200**. 단위 테스트가 가짜 `HomeFn` 을 쓰는 탓에 드러나지 않았다.

**고친 것은 코드가 아니라 문서다** (사용자 판정). 확인해 보니 `/api/fs/*` 도
`root=$HOME` 으로 홈 아래 전부를 조회·다운로드한다 — 홈이 루트인 것은 탐색기의
설계다 (`FR-EDT-113`·`FR-EDT-42`). 파일 API 만 좁히면 접근 경로는 그대로 남고
**편집기로 여는 흐름만** 끊긴다. 그것은 보호가 아니라 기능 손실이다. 남는 위험은
인증의 부재이며 `SEC-3` 과 같은 자리 → **M4**.

#### `SEC-16` 은 실재하는 구멍이었다

같은 사실의 다른 면이다. 홈이 루트이므로 **웹의 파일 쓰기 API 가 홈 전체에 닿았고**,
그 아래에는 읽히기만 하는 값이 아니라 **집행되는 선언**이 산다:

| 자리 | 무엇이 일어나는가 |
|---|---|
| `ext/plugins/<id>/dongminal-ext.json` | 여기 적힌 설치 명령이 실제로 **실행**된다 (`FR-EXT-17`) |
| `access.json` | 접속 허용 목록이다 — 경계 자체가 바뀐다 |
| `settings.json` | `fileApiUnrestricted` 가 여기 있다 — 경계를 끄는 스위치다 |

즉 쓰기 요청 **하나**로 임의 명령 실행과 ACL 무력화에 닿았다. 이 마일스톤이
막으려던 브라우저 매개 공격의 정의 그대로다. 사용자 판정: **"파일 쓰기 API 는 홈
안에서 노트를 제외하고는 쓰면 안 된다. 노트를 제외한 파일들은 본인들의 전용 API 로만
수정된다."** 판정이 `fileApiUnrestricted` 보다 **앞**에 선다 — 그 설정은 "내 작업
파일을 어디서든 열겠다" 이지 "내 서버의 집행 선언을 웹으로 덮겠다" 가 아니다.

**읽기는 막지 않았다.** 설정을 확인하려고 여는 정상 흐름이 있고 그 자리에 비밀이
있지 않다.

#### `U-2` — "미리보기" 가 무엇인지부터 물었다

접수된 말은 *"미리보기를 좌하단으로 옮기고, 색을 바꿔 잘 보이게"* 뿐이었고 저장소에서
그 이름을 쓰는 자리가 셋이었다. 사용자가 **문서 렌더 보기의 진입 버튼**임을
확인했다. 좌상단 → 좌하단, 기본 상태에서 강조색이다.

**딸려 온 정리 하나**: 위쪽 띠는 찾기 패널·알림 줄·언어 서버 제안이 나눠 쓰고,
그래서 제안 띠가 서면 이 버튼을 밀어내는 규칙(`fe-offered`)이 있었다. 아래로
내리면서 **그 규칙과 표식이 함께 사라졌다** — `FR-LSP-44a` 의 목록에서 이 손잡이를
뺐다.

#### `SEC-15` 는 전량 e2e 가 두 번 고쳐 세웠다

경계를 세우자 **git e2e 가 대거 실패했다.** 세 가지가 겹쳐 있었고, 셋 다 "감사
항목이 제품 모델을 모른 채 쓰였다" 의 다른 얼굴이다.

| 어긋난 것 | 무엇이 사실이었나 | 조치 |
|---|---|---|
| 루트 목록에 **핀이 없다** | `wsentry.Roots()` 는 `Editors` 만 담는다. 그런데 git 에서 "등록" 이란 곧 **핀**이다 | `FR-FAB-14a` — 핀 목록을 더한다 |
| 방향이 **하나뿐이었다** | 저장소 **안의 하위 폴더**를 Editor 루트로 삼는 것이 정상이고(`FR-DIR-40`·`FR-EDT-69`), 그때 다룰 저장소는 그 루트의 **조상**이다 | `FR-FAB-14b` — 조상도 허용한다 |
| 검사가 계약을 안 따랐다 | 제품의 UI 흐름은 `openGitWindow` 가 **열기 전에 등록**한다 (`FR-RTU-72`). e2e·`verify` 만 그 걸음을 건너뛰고 API 를 직접 불렀다 | `FR-FAB-14c` — 검사가 대상을 먼저 등록한다 |

**원인을 추론으로 적지 않았다.** 주입만 잠시 끄고 같은 spec 을 돌려 통과하는 것을
보고서야 가드가 원인임을 확정했다 (§"부재는 사실이고 인과는 추론이다").

> **등록을 훅에 두었다가 흔들림을 하나 만들었다.** `describe` 의 `beforeAll` 에
> 넣으면 픽스처를 만드는 바깥 훅과 **순서로 엮인다** — 첫 회만 403 이 나고 재시도에서
> 통과하는 모양이었다. 등록은 멱등이므로 **쓰는 자리에서** 하는 것이 순서에 기대지
> 않는 유일한 형태다. 재시도를 끄고(`--retries=0`) 두 번 연속 통과로 확인했다.
>
> 그 과정에서 실패 메시지도 고쳤다 — `expect(r.ok()).toBeTruthy()` 는 `false` 만
> 남겨 "경계에 걸린 것" 과 "git 이 답을 못 준 것" 을 가르지 못한다. 응답 본문을
> 싣게 하자 원인이 한 줄로 나왔다.

> **`verify` 도 같은 함정에 있었다 — 밀기 전에 찾았다.** 그 검사는 격리 홈을 새로
> 잡고 대상 저장소는 체크아웃 자리에 있다. 리눅스 러너는 둘 다 `/home/runner`
> 아래라 **우연히** 통과하지만, Windows 러너는 체크아웃이 `D:\` 이고 홈이 `C:\` 다
> — 한쪽 러너에서만 빨개졌을 것이다. `registerRepo` 검사를 git 섹션 맨 앞에 세웠다.

### 2.22 기준선이 셋이 아니라 여덟이었고, 여섯은 한 뿌리였다 (2026-09-11)

착수 문서는 **"기존 흔들림 셋"**(`terminal` 검색 둘 · `editor-tab` E17 ·
`git-remote` R19)을 적어 두고 *"쫓지 마라 — 기준선에서도 실패한다"* 고 했다. 그 말이
조사를 막고 있었다.

**기준선을 실제로 쟀다.** 이전 커밋으로 되돌려 같은 spec 을 돌리자 darwin 에서
**여덟**이 실패했다. 그리고 그중 **여섯이 같은 원인**이었다:

> `FR-LAY-3` 이 숨김 어휘를 `.gone`·`.git-hidden` → **`[hidden]`** 으로 옮겼는데,
> **검사가 따라오지 않았다.** 제품은 속성으로 숨기고 검사는 클래스를 재니 영영
> 맞지 않는다 — 흔들린 적이 없다. **결정론적으로 틀린 검사**였다.

| 검사 | 무엇을 재고 있었나 |
|---|---|
| `terminal` 검색 둘 · `focus` | `#search-bar` 의 `.hidden` **클래스** (제품은 `el.hidden` 속성) |
| `editor-tab` E17 | `#split-h`·`#split-v` 의 `.git-hidden` |
| `git-remote` R19 | 충돌 그룹의 `.gone` |
| `git-ui-revision` V89·V93 · `git-discard-all` D1b | 숨은 `Conflicts` 그룹을 **보이는 것으로 세었다** (`[0,36,36]` 의 0 이 그 머리다) |

나머지 둘은 각자 다른 이유였다:

- `history-branch-button` — **U-6 가 계약을 폐기했는데 검사가 따라오지 않았다.**
  `FR-GHM-3a` 가 History 머리에서 원격 버튼을 뺐으므로 "두 머리의 버튼 수가 같다"
  는 더는 참이 아니다. 이제 **가름이 실제로 일어났는가**를 잰다.
- `ui-layout-defaults`·`git-file-actions` — 전량 실행에서만 나는 **부하성**이다.
  단독으로는 통과한다.

#### 재발은 게이트가 막는다

`check-visibility.sh` 는 `web/` 만 보았고 **`e2e/` 는 범위 밖**이었다 — 그 빈
자리가 이 여섯을 만들었다. 검사 (4)를 더해 e2e 가 숨김을 클래스로 재면 걸린다.
탐침으로 검출을 확인하고 지웠다.

### 2.23 CI 를 초록으로 되돌리는 길 (2026-09-11)

**CI 는 이번 작업 이전부터 두 워크플로 모두 빨갰다.** 원인이 셋이었다.

| 자리 | 무엇이었나 | 조치 |
|---|---|---|
| `verify` / gates | `govulncheck@latest` 가 `go >= 1.26` 을 요구하며 **설치 단계에서 죽었다** | 판 고정(`v1.1.4`) — 툴체인은 `go.mod` 이 정하는데 도구만 떠 있으면 반드시 어긋난다 |
| 〃 | 설치가 되자 그다음이 드러났다 — **표준 라이브러리 취약점 21건** | `go.mod` 을 **1.25.13** 으로. 워크플로 다섯 자리가 전부 `go-version-file: go.mod` 이라 한 자리가 전부를 옮긴다. **21건 → 0** |
| `verify` / test (windows) | `"/repo"` 리터럴이 Windows 에서 절대경로가 아니다 (11자리) · omp 기동줄의 **혼합 구분자** | `testpath.Abs` 관례로 되돌림(`FR-WTP-10`·`12`) · `FR-OMP-22a` 신설 |

**내 변경분은 CI 에서 통과했다** — 이번 작업이 만든 e2e(`file-size-gate` 등)와
고친 spec 들이 실패 목록에 없다.
...
- Go 전량 통과 (`go test ./...`).
- 새 단위 검사: `TC-FAB-16·18·19·20`(크기) · `TC-FAB-21~26`(홈 쓰기) ·
  `TC-FAB-27~30`(git `repo`) · wait 동시 3건 · zip 동시 3건 · diag 임계 4건 ·
  샌드박스 자원 2건. 각 묶음마다 **기본값을 지키는 검사**를 따로 두었다 — 테스트가
  상한을 낮춰 도는 관례가 값의 회귀를 가릴 수 있기 때문이다.
- 닿는 e2e 단독 통과: `file-size-gate`(신설, **실물 10MiB+1 파일**) ·
  `ux-batch8` · `doc-render` · `view-scroll-restore` · `repo-tab` ·
  `explorer-transfer`.

---

### 2.24 알람이 되풀이 울던 자리 — 배경 알림이 사용자 턴으로 읽혔다 (2026-09-11)

접수한 말: *"waiting 한 번만 울리고 안 울리는 게 안 되는 것 같다. 지금 세션에서도
계속 대기 알람이 나온다."*

**추론으로 적지 않았다.** 순서대로 지웠다.

| 의심 | 어떻게 지웠나 |
|---|---|
| 옛 바이너리 | 도는 서버는 09-11 09:13 빌드, 구현은 09-09 커밋 — **들어 있다** |
| 서버 로직 결함 | `AllowSignal` 의 단위 검증 12건 통과 (`WaitingMarkIsConsumedOnce` 포함) |
| 403 이 아닌 다른 거부 | 서버 로그에 거부가 **한 줄도 없다** |

그래서 **방송을 직접 셌다** (`/api/commands/sse` 구독). 그 순간 답이 나왔다 —
`done` 이 13:19:49 · 13:20:30 · 13:20:53 에 울었고 **그 사이 사람이 친 것은
없었다.**

#### 원인 — `UserPromptSubmit` 은 사람과 배경을 가르지 않는다

백그라운드 작업이 끝나 턴이 깨어날 때도 같은 훅이 오고, `prompt` 에는 그 알림의
본문이 실린다(`<task-notification>…`). 그것을 사용자 턴으로 읽으면 **알림 하나가
`done` 알람 하나를 낳는다.** §2.7 이 *"배경 이벤트로 깨어난 턴의 종료는 사건이
아니다"* 로 막으려던 자리이고, `FR-ATN-3` 의 전제("출처를 말하는 훅은 이것
하나뿐")가 실제와 달랐다.

조치는 `FR-ATN-3a` 다 — 프롬프트가 `<task-notification>`·`<system-reminder>` 로
**시작하면** 사용자 턴 표시를 세우지 않는다. 휴리스틱임을 인정하고, **못 가르면
종전처럼 울린다**(§1.7 과 같은 판단).

> **관측이 대상을 흔들었다.** 처음에는 이 관측을 `Monitor` 로 걸었는데, 그 알림이
> 다시 턴을 만들어 `done` 을 낳았다 — 재는 행위가 재려는 것을 만들고 있었다.
> 파일로만 기록하도록 바꾼 뒤에야 깨끗한 표본이 나왔다.

#### `waiting` 은 되풀이되지 않았다

같은 기록에서 `waiting` 은 **7분 간격 두 번**이다(13:23:02 · 13:30:35). 그 사이
도구가 여럿 돌았으므로 서로 다른 대기이며, `FR-ATN-15` 는 지키고 있다.

**다만 재무장의 범위는 넓은 채로 남는다.** `waitingSignaled` 를 내리는 것은
`working` 보고인데 그것을 보내는 훅이 다섯이다 — 에이전트가 스스로 부르는
`PreToolUse` 도 포함된다. `toolId` 가 탭 단위이므로, 병렬 도구가 도는 동안에는
대기 중인 표시가 남의 일로 내려갈 수 있다. **실측 근거가 없어 열어 둔다** — 고치려면
"사용자가 새 일을 시켰다" 의 정의를 다시 세워야 하고, 그때 터미널에서 직접 승인하는
경로(`FR-ATN-16` 이 지키려던 것)를 함께 봐야 한다.

### 2.25 CI 를 초록으로 — 남은 뿌리 둘 (2026-09-11)

§2.23 뒤로 두 번 더 밀며 남은 것을 걷었다.

| 자리 | 무엇이었나 |
|---|---|
| Windows `test` | **이번 작업이 새로 넣은 검사 셋**이 플랫폼에 기댔다 — omp 기대값이 POSIX 리터럴 · `"/elsewhere/repo"` · Windows 의 `TempDir` 이 홈 **아래**라 "경계 밖" 이 성립하지 않았다. 방금 남을 고친 실수를 그대로 반복했다 |
| Windows `test` | `TestStatic_PrecompressedServedAsGzip` — `.js` 의 형식을 **레지스트리**(`HKCR\.js`)에서 읽어 `application/javascript` 가 나갔다. 같은 코드가 호스트 설정에 따라 다른 형식을 내보내고 있었다 |

`gates` 는 툴체인 상향 뒤로 **success** 다 (취약점 21건 → 0).
...
**전부 닫혔다** (2026-09-11 사용자 판정, §2.21):

| 항목 | 판정 |
|---|---|
| `SEC-19`·`FUI-06`·`GO-38`(나머지) | **10 MiB.** 서버가 413 을 내고 `probe.maxBytes` 로 같은 값을 클라이언트에 준다 |
| `SEC-20` | zip 동시 **2**, 초과 429 (`FR-ETR-45`) |
| `SEC-10` | **조치 불요** — ACL 게이트는 환경변수 경로에도 이미 걸린다. 남은 것은 표기뿐이었다 |
| `SEC-15` | **제한한다** — git `repo` 도 파일 표면과 같은 경계를 지난다 (`FR-FAB-14·15`) |
| `SEC-16` | **홈 안 쓰기는 노트만** (`FR-FAB-12·13`). 나머지는 전용 API 로만 바뀐다 |
| `09` 비목표 3 | `cpus=2`·`memory=2g`·`pids=512` (`FR-SBX-45`) |
...
**남은 넷은 전부 값 판정이 필요했고, 2026-09-11 에 전부 받았다 (§2.21).** 아래는
그 판정과 들어간 자리다 — 상한 숫자는 제품 결정이므로 임의로 박지 않았다.

- ~~`wait` 동시 수 상한~~ **닫힘 — 32** (`FR-STA-9`). 초과는 429 다. 종단이 최대
  30분 붙잡으므로(`handlers_status.go:31`) 상한이 없으면 루프가 잘못 도는
  오케스트레이션 하나가 그 자원을 전부 가져간다.
- ~~diag 스냅샷의 임계 경고~~ **닫힘** (`FR-CNR-13`) —
  `goroutines>500`·`allocMB>256`·`ws>100`·`hold>32`. 값의 근거는 **실측**이다
  (정상 구간 `goroutines 95~112`·`allocMB 2~6`·`ws 21~24`·`hold 0`). `hold` 의 32 는
  대기 상한과 같은 값이며 "거절이 시작됐다" 를 뜻한다.
- ~~편집기의 `probe.size` 상한(`FUI-06`)~~ **닫힘 — 10 MiB.** 서버·클라이언트가
  한 벌로 쓴다 (`probe.maxBytes`).

<details><summary>판정 전의 기술 (보존)</summary>
...
- ~~샌드박스 컨테이너의 cpu·memory·pids 상한~~ **닫힘 — `cpus=2`·`memory=2g`·
  `pids=512`** (`FR-SBX-45`, 비목표에서 요구로 승격). 프로파일을 가리지 않는다 —
  격리 경계인 `scratch` 가 오히려 무제한이면 그 경계는 절반만 있는 것이다.
- ~~`dongminal verify` 에 게이트 항목 추가~~ **닫힘 — 경계 넷 + 노출 ACL 게이트**
  (`verify_boundary.go`, 검사 22→27). §2.12 의 정정을 지킨다: 다섯 전부 **실제로 뜬
  서버**(또는 실제로 돌린 기동)를 두드린다. 노출 게이트 검사는 격리 홈을 하나 더
  잡아 `start --expose` 를 **실제로 돌리고** 거부되는지 본다 — 거부되면 아무것도 뜨지
  않고, 만에 하나 통과하면 실패로 적기 전에 정리부터 한다.

**§3.3 은 전부 닫혔다** (2026-09-11, §2.21).

### 3.3a DoD 전수 확인 — `PRODUCTION_ROADMAP` §M2 의 목록 (2026-09-11)

§3.3 은 이 진행 문서가 **따로 추린** 다섯이었다. 마일스톤 문서의 DoD 원본을 한 줄씩
대조했고, 결과는 아래다.

| DoD 항목 | 상태 |
|---|---|
| `ToolManager.Create` 상한 초과 시 429 | ✅ `ErrToolCap` → 429 (`manager.go:369`) |
| `CommandHub.Add` 구독자 상한 | ✅ `SubCap` → 503 (`commands.go:177`) |
| `wait` 동시 수 상한 | ✅ 32 (`FR-STA-9`, §2.21) |
| `diag` 임계 경고 | ✅ (`FR-CNR-13`, §2.21) |
| 홈 0700 · 소켓·로그 0600 · 로그 자리 | ✅ (04-secops `P1-6`) |
| `http.Error(err.Error())` 0곳 · 에러 분류 `errors.Is` | ✅ (§2.6) |
| 헤드리스 명령 로그가 길이·해시 | ✅ (`redactCmdForLog`) |
| `worktree`·`submodule` 이 `core.Env()` 공유 | ✅ (§2.8) |
| 노출인데 ACL 이 꺼져 있으면 기동 거부 | ✅ (`FR-RQG-20`) |
| **노출 모드에서 `access.json` 손상은 fail-closed** | ✅ **단, 자리가 다르다** — `loadAccess` 는 여전히 열린 채 서지만(`FR-ACL-2`), 노출 경로는 **기동 게이트**가 "허용 목록이 깨졌습니다" 로 막는다. 로컬은 loopback 이므로 뜻이 지켜진다 |
| `_confirmClose` 가 `GitConfirm` 규약으로 | ✅ (`FE-15`) |
| `core/api.js` 경유 · 손수 `fetch` 0 | ✅ (`FE-8`, 전량 확인은 §6.5 에서 닫혔다) |
| `submodule.ExecGit` 이 `ctx` 를 받는다 | ✅ (`FBE-08`, §2.8) |
| 편집기의 `probe.size` 상한 | ✅ 10 MiB (`FR-FAB-9`, §2.21) |
| 샌드박스 cpu·memory·pids | ✅ 2·2g·512 (`FR-SBX-45`, §2.21) |
| **그 상한값이 `features.md` 용량 표에 실린다** | ⏭ **M2 에서 충족할 수 없다** — `docs/features.md` 는 **아직 없다.** 그 문서는 M5 의 산출물이며(`G9-1`) DoD 문장도 그렇게 적고 있다. **M5 로 이월**하고 그 사실을 여기 남긴다 |
| `dongminal verify` 항목 추가 + 3 OS CI | ✅ 검사 28건 (§2.21). CI 는 `verify` success (§2.25) |
| M2~M4 구간 운영 안내가 문서에 | ✅ `README.md:94` (ACL·오버레이 망·공용 Wi-Fi 셋) |

### 3.3b P2 전수 — 18건 (2026-09-11)

`SEC-10`~`21` · `GO-23` · `GO-38` · `GO-39` · `FE-15`~`17` 을 한 줄씩 확인했다.
**전부 닫혔거나 판정으로 종결됐다.**

| 판정으로 종결 | `SEC-10`(게이트가 이미 막는다) · `SEC-14` · `GO-23`(착수 근거 없음) |
| 이번 세션에 닫힘 | `SEC-15` · `SEC-16` · `SEC-19` · `SEC-20` · `GO-38` |
| 이전 세션에 닫힘 | `SEC-11`(위 표) · `SEC-12` · `SEC-17` · `SEC-18` · `SEC-21`(`json.Valid`) · `GO-39` · `FE-15`·`16`·`17` |

**`SEC-7` 만 M8 로 이월된다** — 그것은 P2 목록 밖(`/api/upload`·`/api/download`)이며
`FILE_API_BOUNDARY_SRS` §5 비목표 2 가 그 자리를 적는다.
...
| U-2 | **미리보기를 좌하단으로 옮기고, 색을 바꿔 잘 보이게 한다** | ✅ **닫혔다 — §2.21.** 요구 자체가 미확정이었고(저장소에서 "미리보기" 를 쓰는 자리가 셋) **사용자가 문서 렌더 진입 버튼임을 확인**했다. 좌상단 → 좌하단(`FR-DRB-3` 개정), 기본 상태에서 강조색(`FR-DRB-3a` 신설). 딸려 온 정리: 제안 띠가 이 버튼을 밀어내던 규칙·표식(`fe-offered`)이 **필요 없어져 함께 걷혔다** — `FR-LSP-44a` 의 목록에서 이 손잡이를 뺐다. 검증 `V-DRB-1` 확장(아래쪽 절반에 선다 · 글자와 테두리가 같은 강조색) |
| U-22 | **열기만 하고 수정하기 전인 파일은 외부에서 바뀌면 그대로 반영되어야 한다** | ✅ **닫혔다** (2026-09-11, §2.26). [`EDITOR_EXTERNAL_CHANGE_SRS`](../EDITOR_EXTERNAL_CHANGE_SRS.md) 가 확정됐고 미해결 셋을 사용자 결정으로 닫았다(확인창은 `취소`/`덮어쓰기` 둘 · 외부 삭제는 탭을 두고 새로 만든다 · 표식은 불투명 토큰). `refresh()` 에 dirty 가드, `/api/file/{read,write}` 에 표식·409, 확인창, 그리고 **저장 실패가 닫기를 막는다**(`FR-EXC-12`·`13` — 착수 조사에서 새로 확정한 손실 경로). Go 6 · e2e 6 검증 |
| U-21b | **대기·완료 알람이 되풀이 운다** | ✅ **닫혔다 — §2.24 `FR-ATN-3a`.** 방송을 직접 세어 원인을 잡았다 — 배경 알림이 `UserPromptSubmit` 으로 들어와 **알림 하나가 `done` 알람 하나**를 낳고 있었다. `waiting` 은 되풀이되지 않았다(7분 간격 2회). 재무장 범위는 근거가 없어 열어 두었다 |
| U-3 | **`claude code`·`omp` 에서 스크롤이 위로 붙는 문제가 아직 남아 있다** — **2026-09-10 재확인**: "스크롤이 **최상단으로** 가 있을 때가 있다" | ✅ **닫혔다 (2026-09-11 2차) — 원인은 `VIEW_SCROLL_RESTORE_SRS` §2.8 이 진실의 자리다.** 사용자가 증상을 좁혔고(*"보이기에는 맨 아래가 보이나 실제 스크롤은 맨 위에 가 있다"*) 브라우저 실측이 그것을 확정했다: 요소가 문서에서 떨어질 때 브라우저가 `.xterm-viewport.scrollTop` 을 버리는데 `ydisp` 는 인스턴스에 남아, **캔버스는 맨 아래를 그리고 DOM 은 맨 위에 선다.** xterm 의 `syncScrollArea` 는 캐시(`_lastScrollTop`)로 판정하므로 스스로 회복하지 못한다. 조치는 `FR-VSR-24` — 복원 끝에서 DOM 을 `ydisp` 에 맞춘다(`V-VSR-12·13`). **가설이었던 흔들기 착지 실패는 기각됐다** — 실측 전 구간 `_ignoreNextScrollEvent=false` 였고 결함 갈래는 흔들기를 지나지도 않는다. 아래는 그때까지의 기록이다. 사용자가 이것을 **`U-20`(스크롤 저장)의 일부로 묶었다** — *"맨 위로 가는 현상을 막는 것도 스크롤 저장의 일부다."* 종전 조치가 있었으나 미해결. 재확인된 문구가 증상을 좁힌다 — 붙는 자리가 **최상단**이다. 터미널 스크롤백(xterm)의 자리이며 `U-17`(가끔 스크롤이 안 됨)과 **같은 뿌리일 수 있다** — 둘 다 "높이·위치를 잘못된 순간에 잰다" 로 설명된다. 재현 조건부터 다시 잡고, 둘을 함께 본다 |
...
| U-17 | **가끔 스크롤이 안 될 때가 있다. 새로고침하면 풀린다** (2026-09-10 접수) | ✅ **닫혔다 (2026-09-11 2차) — `U-3` 과 한 뿌리였다** (`VIEW_SCROLL_RESTORE_SRS` §2.8). "가끔"의 정체는 **출력의 유무**다: PTY 가 한 줄만 내보내도 xterm 이 맨 아래를 따라가며 `onScroll` 을 내고 그때 `Viewport` 가 `scrollTop` 을 다시 맞춘다. 그래서 조용한 터미널에서만 어긋남이 살아남는다 — Claude Code 처럼 출력이 멎은 동안 창을 다녀오는 것이 정확히 그 조건이다. "새로고침하면 풀린다" 도 같은 값이 새로 서기 때문이다. 착수 전 조사가 지목한 *"높이가 0 인 순간에 잰 것이 남는"* 부류는 **옳은 방향이었으나 자리가 달랐다** — 잰 값이 아니라 **버려진 값**이 남았다. 아래는 그때까지의 기록이다. **`U-8` 의 정적 결함과 성질이 다르다** — 명시도 문제는 새로고침으로 풀리지 않는다. "새로고침하면 풀린다" 는 **런타임 상태가 굳는다**는 뜻이다. 첫 조사 지점은 flex 자손이 줄어들지 못하는 자리(`min-height:0` 누락)와 **높이가 0 인 순간에 잰 것이 남는** 경로다 — `elFor`(`panel-life.js:81`)의 주석이 이미 그 부류를 적고 있다("루트가 아직 pane 본문에 붙기 전이라 목록의 높이가 0 이다"). `_scrollY` 복원(`file-tree.js mount`)·`_restoreScrollOf`(`FR-PDR-10`)도 같은 자리다. 재현 조건부터 잡는다 |
...
### 2.26 `U-22` — 외부 변경은 덮지 않고 묻는다 (2026-09-11, 9차)

접수한 요구(clean 이면 따라간다)를 구현하면서 **요구 밖의 손실 경로 하나**가 함께
드러났다. `FileEditor.save()` 는 아무것도 반환하지 않고 실패를 `catch` 로 삼켰고,
탭 닫기의 "저장 후 닫기" 는 그것을 기다린 뒤 **무조건 닫았다**. 바로 위 git diff
경로에는 `if(!await this._gitViewSave(...)) return` 이 있는데(주석: *"저장한 줄 알고
닫는 것이 곧 손실이다"*) 편집기 경로에만 그 가드가 없었다.

**경합을 들이는 순간 그것이 손실 경로가 된다** — 경합으로 저장이 막혔는데 탭이
닫힌다. 그래서 `FR-EXC-12`·`13` 을 같은 변경에 넣었다. 요구가 만든 위험은 그 요구와
함께 닫는다.

표식은 **문서**의 것이다. 처음에는 `this._stamp` 로 썼는데 `destroy()` 가
`this._doc` 을 끊으므로, 저장이 날아가 있는 동안 그 칸이 파괴되면 표식이 죽은 필드에
쓰이고 문서의 표식은 낡은 채 남는다 — 그러면 같은 파일을 보던 다른 칸의 다음 저장이
**방금 우리가 만든 변경**에 걸려 409 가 된다. `TC-SVS-53` 이 그것을 잡았다.
`FR-WBR-90~92` 가 `dirty`·`saving` 에서 이미 치른 값이고, 표식만 그 교훈에서 빠져
있었다.

### 2.27 `FR-VSR-22` — `U-3` 의 가설 기전을 실물에서 처음 재현했다 (2026-09-11, 9차)

`U-20` 묶음 C 의 마지막 미구현 조항이다. `VIEW_SCROLL_RESTORE_SRS` §2.7 이 가설로만
적어 두었던 것 — *"흔들기의 두 번째 동작이 듣지 않으면 화면이 최상단에 남는다"* —
을 새 검사 `V-VSR-11` 이 **재현했다**: `scrollToLine` 을 눌러 두고 탭을 왕복하면
`viewportY` 가 `0` 이다.

고친 것은 흔들기의 **출발점**뿐이다. `scrollToTop()` 대신 갈무리한 자리의 이웃 줄에서
출발하므로 같은 실패가 최상단이 아니라 한 줄 차이로 끝난다. `_restoreScrollOf` 의
판정(`FR-PDR-11` bottom-follow)은 그대로다 — `FR-VSR-23` 이 재현 조건 전의 변경을
금지하고 이 변경은 그것을 지킨다.

**`U-3`·`U-17` 의 원인 규명은 여전히 열려 있다.** 이것은 불변식이지 원인 조치가
아니다 (D-6). 왜 듣지 않는 프레임이 있는지는 재현 조건을 기다린다.

### 2.28 omp 의 알람 — 원인은 omp 가 아니라 **배선**이었다 (2026-09-11, 9차)

접수: *"omp 는 agents 에서 상태 바뀌는건 확인했는데 알람이 연결 안됐어."*

활동(`dmctl activity`)과 알람(`dmctl notify`)이 **다른 배선**이었고, 셋 다 손으로
쓰여 있었다 — claude 는 `hooks.json` 에 따로, codex 는 `notify` 안에서 역방향으로,
omp 는 아예 없었다. 판정부(`AgentTurn`)는 이미 에이전트 중립이었으므로, 추상화가 없던
자리는 **"누가 알람을 부르는가" 하나뿐**이었다.

[`AGENT_EVENT_ABSTRACTION_SRS`](../AGENT_EVENT_ABSTRACTION_SRS.md) 가 그것을 닫는다:
`Adapter.Signals` 가 **모든 이벤트를 선언**하고(사용자 결정), 알람은 활동 이벤트에서
파생하며, claude 의 중복 `notify` 배선은 걷혔다.

**선언이 왜 필요한가** — codex 에는 턴 출처 이벤트가 없다. 알람 규칙은 사용자 턴
표시를 보는데, 그 표시를 세울 이벤트가 **없는** 에이전트를 "사용자 턴이 아니었다" 로
읽으면 한 번도 울지 않는다. `Signals.UserTurn=false` 가 그 둘을 가른다 (`FR-CBG-5`).

구현 중 스펙 하나가 틀린 것이 드러났다. 초안은 codex 의 중복 발화를 소비
의미론이 흡수한다고 적었으나, `FR-AEV-12` 의 **무조건 갈래에는 그 의미론이 걸리지
않는다** — 같은 턴이 두 번 운다. 남긴 교훈: **예외 갈래를 더하면 그 갈래에는 본 규칙의
보호가 걸리지 않는다.**

omp 의 한계는 값으로 남겼다 — `Signals.Waiting=false`. 승인 게이트가 omp 훅에
통지되지 않으므로 **완료 알람은 울리고 승인 대기 알람은 없다**.
...
아래 블록을 새 세션에 그대로 붙여넣으면 된다. 사본이
`docs/internal/production/M2_NEXT_SESSION.md` 에 있다 — **고칠 때는 여기를 고치고
그쪽으로 옮긴다.**
...
## A-1. 부하성 흔들림 — **고정 목록으로 보지 마라**

9차에 전량을 **세 번** 돌렸고 **세 번 다 다른 목록**이 나왔다. 겹치는 것이 하나도
없다 (§6.3). 실패로 남은 것은 한 건도 없고 전부 재시도로 통과한다.

  · 물려받은 `git-file-actions` F6 은 **재현되지 않았다** (전량·`--repeat-each=3`
    병렬 모두 통과). 그 자리의 시간 전제만 걷어 두었다
  · 대신 관측된 것: `git-history` H4 · `repo-diff-edit` E1 (한 프로세스 실행) ·
    `git-branches` B1·B11 · `git-observe-revive` TC-GOR-4 ·
    `slot-view-state` TC-SVS-1·22 (8샤드 실행)

**그러므로 다음 세션이 할 일은 "이 목록을 쫓는 것" 이 아니다.** 특정 검사의
결함이라기보다 **부하 민감성**으로 보이고, 그것을 고치려면 개별 검사가 아니라
**시간으로 전제를 세우는 자리**를 걷어야 한다.
...
- **전체 e2e 는 남은 구현이 전부 끝난 뒤 한 번이다.** 그때까지는 **닿는 스펙만** —
  고친 파일의 FR 번호로 `e2e/*.ts` 를 grep 하면 그 변경의 회귀 넷이 나온다.
- **출력을 자르지 마라.** `make e2e ... | tail -N` 으로 돌리면 종료코드가 `tail`
  의 것이 되어 **`exit 0` 이 초록을 뜻하지 않고**, 샤드 요약도 잘린다 (9차 실측).
  전량은 출력을 통째로 남기고 거기서 샤드 8개의 요약을 읽어라.
- **로컬 전량은 `make e2e` 다** (사용자 결정 2026-09-11). CI 와 **같은 8샤드**를
  **병렬**로 돈다. **8.4분**이다 (9차 실측 — 한 프로세스는 20.3분이었다).
  `npx playwright test` 로 한 프로세스로 돌지 마라 — 느릴 뿐 아니라 CI 와
  **상태 누적의 범위**가 달라 답이 갈린다 (`FR-EPL-13`). 그 무거운 쪽이 필요할
  때만 `make e2e-all` 을 **명시로** 쓴다.
- **다른 세션과 겹쳐 돌리지 마라.** 같은 기계에서 함께 돌리면 PTY 가 소진되고
  (`kern.tty.ptmx_max` 기본 511) 실패 목록이 오염된다. **샤드끼리는 안전하다** —
  `make e2e` 가 포트·산출물·청소 범위를 갈라 둔다 (`FR-EPL-14`).
...
> **8차 세션 갱신 (2026-09-11).** 아래 7차 기록의 판단("기존 것이며 무관하다")은
> **절반만 맞았다.** 기준선을 실제로 재자 darwin 에서 **여덟**이 실패했고 그중
> **여섯이 결정론적으로 틀린 검사**였다 — 흔들린 적이 없다 (§2.22). 전부 닫혔다.
>
> **`ui-layout-defaults` 는 원인을 찾아 닫았다 (2026-09-11).** 부하가 아니라
> **설정 오염**이었다 — `tab-width` 가 `tabFixedWidth` 를 **서버 설정에 저장**하고,
> 설정은 워커를 넘어 남는다. 그러면 `body.tabfix` 가 켜진 채로 재게 되고
> `.pn-tab-label` 의 `overflow` 가 `visible`→`hidden` 이 된다(그 18자리다).
>
> **가설을 재현해 확정했다** — `tab-width` → `ui-layout-defaults` 를 `--workers=1`
> 로 연달아 돌리자 그대로 실패했고, 수정 뒤 같은 명령이 통과한다. 고친 것은
> **검사가 자기 전제를 스스로 세우게** 한 것이다(`PUT /api/settings` 는 전체
> 교체이므로 현재 값을 받아 한 자리만 되돌린다).
>
> **9차 세션 갱신 (2026-09-11) — 목록을 물려받지 않고 다시 쟀다. 세 번.**
>
> | 실행 | 방식 | 결과 | 벽시계 | 흔들린 것 |
> |---|---|---|---|---|
> | ① | 한 프로세스 (`make e2e-all` 상당) | 1532 통과 · 실패 0 | 20.3분 | `git-history` H4 · `repo-diff-edit` E1 |
> | ② | 8샤드 병렬 (`make e2e`) | — | — | **판독 불가** (아래) |
> | ③ | 8샤드 병렬 (`make e2e`) | **1530 통과 · 실패 0** | **8.4분** | `git-branches` B1·B11 · `git-observe-revive` TC-GOR-4 · `slot-view-state` TC-SVS-1·22 |
>
> **②는 근거가 되지 못했다.** `make e2e 2>&1 | tail -80` 으로 돌려서 파이프
> 종료코드가 `tail` 의 것이 되었고(`exit 0` 이 초록을 뜻하지 않았다), 동시에
> `tail` 이 샤드 7 말고는 요약을 잘라 버렸다. **전량을 돌릴 때 출력을 자르지
> 마라** — 자른 출력으로는 초록을 말할 수 없다.
>
> **세 번 재서 세 번 다 다른 목록이 나왔다.** 겹치는 것이 하나도 없다. 이것이
> "물려받은 흔들림 목록은 낡는다" 의 결정적 증거이며, 동시에 **개별 검사의 결함이
> 아니라 부하 민감성**이라는 쪽을 가리킨다 — 특정 검사가 아니라 그때그때 밀린
> 것이 흔들린다.
>
> - `git-file-actions` F6 는 **재현되지 않았다.** 전량에서도, `--repeat-each=3`
>   병렬에서도 통과한다(39/39). 물려받은 목록이 낡았다.
> - 그래도 그 자리의 **시간 전제는 걷었다** — 두 번의 ignore 사이에
>   `waitForTimeout(300)` 이 있었고, 그것은 "그 안에 서버가 쓴다" 는 가정이다.
>   기계가 붐비면 깨지고 그때 `.gitignore` 가 비어 **관측된 실패 모양(빈 배열)과
>   정확히 같아진다.** 응답을 기다리도록 바꿨다. **이것은 강화이지 원인 조치가
>   아니다** — RED 를 보지 못했으므로 원인이라고 적지 않는다.
> - 대신 **다른 둘이 한 번씩 흔들렸다**(재시도로 통과): `git-history` H4 ·
>   `repo-diff-edit` E1. 둘 다 **단독으로는 40/40 통과**한다. 실패 출력을 남기지
>   못했고, 그 상태에서 원인을 적는 것은 이 문서가 금지한 일이라 손대지 않았다.
>   다음에 만나면 **그 실행의 trace 를 먼저 보존하라.**
>
> **남은 흔들림 (부하성, 원인 미상).** 고정 목록으로 보지 마라 — 실행마다
> 바뀐다. 아래는 9차에 관측된 전부다:
>
> | 검사 | 관측된 실행 |
> |---|---|
...
> | `repo-diff-edit` E1 | ① |
> | `git-branches` B1 · B11 | ③ |
> | `git-observe-revive` TC-GOR-4 | ③ |
> | `slot-view-state` TC-SVS-1 · TC-SVS-22 | ③ |
>
> 전부 **재시도로 통과**했고 실패로 남은 것은 한 건도 없다.
>
> **CI 에는 플랫폼 전용 실패가 더 있다** — 로컬 darwin 에서 재현되지 않아 이번에
> 손대지 못했다: ubuntu `editor-ops` S3 · windows `editor-ops` O1 ·
> `git-console` K2 · `soft-reload` SR8. 추적하려면 그 OS 의 러너 로그나 실기가
> 필요하다.
>
> 아래는 7차의 기록이며 보존한다.
...
### 6.5 `FE-8` 의 전량 확인 — **닫혔다** (2026-09-11)

> **정정.** 이 절은 *"전량 확인이 남아 있다. 이것이 다음 세션의 첫 일이다"* 로
> 적혀 있었다. **8차 세션이 전체 e2e 를 다섯 번 돌려 그것을 충족했다** — 중단
> 없이 1534~1554 항목이 돌았고, `core/api.js` 를 지나는 경로에서 나온 실패는
> 없다. `NFR-CAPI-3`("아무것도 달라지지 않는다")이 충족됐다고 말할 수 있다.
>
> 아래는 그때의 기록이며 보존한다.
...
~~**남은 것은 전량 한 번이다.**~~ → 위의 정정대로 **닫혔다** (8차, 전량 5회).

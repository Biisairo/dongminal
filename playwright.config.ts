...
export default defineConfig({
  ...
  /**
   * 재시도는 **한 번**이고, 로컬과 CI 가 **같은 조건**이다 (DRIFT_RECLAIM_SRS
   * FR-DRC-17).
   *
   * 종전에는 CI 만 1회였다. 조건이 갈리면 로컬에서 초록인 것이 CI 에서 다르게
   * 보이고, 어느 쪽이 사실인지 말할 수 없다.
   *
   * **상한선 1 은 흔들림을 지우지 않는다 — 한 번까지만 봐준다는 뜻이다.**
   * 두 번째에도 깨지면 실패다. 그리고 한 번에 걸린 것도 `flaky` 로 **기록되고
   * 트레이스가 남는다**(`trace` 참조) — 그것이 이 상한선과 예전 설정의 차이다.
   *
   * 예전 설정이 위험했던 이유는 재시도 자체가 아니라 **증거를 남기지 않은 것**
   * 이었다. `trace: 'on-first-retry'` 는 재시도한 회차만 찍었고, 아무도 flaky
   * 개수를 세지 않았다. 그 사이 제품 결함 셋이 초록 뒤에 있었다 —
   * 메뉴의 `Esc` 죽은 창, History 의 레이아웃 복구, 핀 통지 (§7.5). 사용자는
   * 그것을 "가끔 안 된다" 로 만나고 있었다.
   *
   * **flaky 가 0 이 아니면 그것은 부채다.** 상한선은 그 부채를 안고 진행할지를
   * 정하는 것이지, 부채가 없다고 말하는 것이 아니다.
   */
  retries: 1,
  /**
   * 실패의 상한 (FR-CEM-18).
  ...
  use: {
    ...
    /**
     * **첫 시도가 깨진 순간을 남긴다** (FR-DRC-17).
     *
     * 재시도가 한 번 있으므로 `retain-on-failure` 로는 부족하다 — 그 값은 테스트가
     * **최종적으로** 실패했을 때만 남기고, 재시도에서 통과한 `flaky` 의 트레이스는
     * 버린다. 그러면 상한선 안에 든 흔들림은 다시 보이지 않게 되고, 그것이 예전
     * 설정이 제품 결함 셋을 숨긴 방식이다.
     *
     * `retain-on-first-failure` 는 **첫 시도가 깨지면 그 트레이스를 남긴다** —
     * 최종 실패든 flaky 든. 상한선을 두더라도 원인까지 갈 근거는 남는다.
     */

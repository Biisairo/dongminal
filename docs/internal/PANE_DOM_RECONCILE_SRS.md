...
| FR-PDR-10a | 이 조항의 "라이브 위젯" 은 **터미널만이 아니다.** 편집기·렌더 뷰의 시선도 같은 규약을 받으며, 다만 **갈무리의 자리가 다르다** — xterm 은 붙일 때 재도 값이 옳지만(버퍼가 인스턴스에 남는다) Monaco 는 그렇지 않아 **떼기 전**에 재야 한다. 그 자리와 조항은 `VIEW_SCROLL_RESTORE_SRS` `FR-VSR-2·3` 이다 | 필수 |
| FR-PDR-11 | 사후 처리는 bottom-follow 를 먼저 본다. 이동 직전이 `viewportY >= baseY` 였으면 `scrollToBottom()`, 아니면 라인 기준으로 되돌린다. **판정은 이대로 유지된다** — 뒤의 "~~픽셀 대입은 하지 않는다~~" 만 `FR-PDR-11a` 가 개정했다 | 필수 |
| FR-PDR-11a | **`scrollToBottom()` 만으로는 DOM 이 따라오지 않는다.** `ydisp === ybase` 인 자리에서 그것은 `scrollLines(0)` 이라 xterm 내부에서 즉시 반환하고, `Viewport` 는 깨지 않는다 — 그 사이 `.xterm-viewport.scrollTop` 은 요소가 떼였을 때 브라우저가 버린 `0` 으로 남아 있다. 그래서 사후 처리는 **끝에서 DOM 을 `ydisp` 에 맞춘다** (`VIEW_SCROLL_RESTORE_SRS FR-VSR-24`). `FR-PDR-11` 의 "픽셀 대입 금지" 는 갈무리한 **옛 픽셀**을 되붙이지 말라는 뜻이었고, 그 금지는 유효하다 — 새 조항이 대입하는 것은 옛 값이 아니라 **지금의 `ydisp` 에서 파생한 값**이다 | 필수 |
...
- **D-6a. 판정과 반영을 갈라 둔다.** `FR-PDR-11` 이 정하는 것은 *"어느 줄을 보여야
  하는가"* 이고, `FR-PDR-11a` 가 정하는 것은 *"그 결론을 DOM 에 어떻게 반영하는가"*
  다. 1차 개정(`FR-VSR-22`)이 흔들기의 순서를 고치고도 결함에 닿지 못한 이유가
  이 구분이 없었기 때문이다 — 판정은 내내 옳았고 반영이 빠져 있었다.

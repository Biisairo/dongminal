...
> **갱신 (2026-05-11)**: 사용자 피드백("키바가 안 보여 모바일에서 사용 불가")에 따라 표시 정책을 **모바일 모드 진입 시 상시 표시**로 변경. 위치 보정(키보드 위 sticky)은 유지. 상세 요구·검증은 [MOBILE_KEYBAR_ALWAYS_VISIBLE_SRS.md](MOBILE_KEYBAR_ALWAYS_VISIBLE_SRS.md) 참조.
...
- [x] Ctrl + C, Ctrl + D 등 sticky modifier 조합 동작 (자동화: TC-D1)
- [x] 보조 키바 버튼 누를 때 키보드 사라지지 않음 (focus 보존) (자동화: TC-D2)
- [x] 모바일에서 분할 핸들 / 분할 단축키 모두 비노출 / 비동작 (자동화: TC-D4)
- [x] 단일 pane 세션에서도 `1/1` 인디케이터 표시 (자동화: TC-D3)

...
- **부팅 화면(boot screen)**: `#boot` 오버레이. 로고 · 스피너 · 단계 텍스트로 구성한다.
...
| FR-BTS-6 | `index.html` 은 `#boot` 오버레이를 **정적 마크업**으로 가진다. 로고(인라인 SVG) · 스피너 · 단계 텍스트(`#boot-step`)를 포함한다. | 필수 |
...
| V-4 | FR-BTS-6·7·9 | e2e: `goto` 직후 `#boot` 가 보이고 그 배경이 `--bg` 와 같으며, `#boot-step` 에 문구가 있다. |

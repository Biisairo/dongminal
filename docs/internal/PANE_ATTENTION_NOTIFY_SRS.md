...
| **FR-PAN-10** | 알림 강조는 포커스 강조와 **시각적으로 구분**되어야 한다. 포커스=`--accent`(파랑), 닫기/위험=`--danger`(빨강)와 겹치지 않는 `--attn` 을 쓰되, **테마 팔레트의 색(테마 terminal `yellow`/`brightYellow`)을 `applyThemeObj` 에서 동적 적용**한다(테마별 조화). 가시성을 위해 **굵은 실선 테두리 + 글로우**(box-shadow)로 강조한다(기존 점선 대비 강화). CSS 클래스 `.rt.attn`/`.rg.attn`. `:root` 폴백값 유지(하드코딩 폴백 1회만). | 필수 |
...
| **FR-PAN-13** | 프론트는 페이지가 백그라운드(다른 OS 창/탭)일 때를 대비해 **추가 알림 채널**을 제공한다: (a) 데스크톱 알림(Web Notification API) — 권한 허용 시, (b) 브라우저 탭 제목/파비콘에 주의 pane 개수 배지, (c) 사운드 큐(선택). 각 채널은 설정으로 on/off 한다. 기본값: 시각 강조=항상, 데스크톱 알림=권한 허용 시 on, 제목 배지=on, 사운드=off. | 필수 |
...
| NFR-PAN-10 | **억제는 "실제로 보고 있을 때"만** — 포커스+활성 pane 이라도 페이지가 가려져 있으면(`document.hidden`) 알람을 살린다(자리 비움 중 데스크톱 알림 가치). 페이지가 보이고 그 pane 에 포커스가 있을 때만 즉시 해제. |

...
//
// 이 파일은 이식 가능한 규칙만 담는다. 실제 조회는 fg_posix.go / fg_other.go 가
// build tag 로 갈라 맡는다 (FR-TAN-23/24).
...
// 테스트가 호스트의 PTY·ps 동작에 기대지 않고 결정론적으로 대체할 수 있다.
...
func (m *ToolManager) refreshForeground(now time.Time) {
	...
	for id, p := range m.tools {
		live = append(live, fgRequest{ID: id, PTMX: p.ptmx, ShellPID: p.CmdProcessPID()})
	}
	m.mu.RUnlock()

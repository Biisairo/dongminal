...
| ~~N5~~ | ~~`diag_snapshot_test.go` 의 데이터 경쟁 수정~~ | ⊘ **해소됨** — `CODE_AUDIT_FIXES_SRS` FR-CAF-1 이 닫았다 (`09` 모순 E). 당시 사유: 무관한 기존 결함 (§2.4) |
...
- **`diag_snapshot_test.go` 의 데이터 경쟁** — ✅ **닫혔다** (`CODE_AUDIT_FIXES_SRS`
  FR-CAF-1, `09` 모순 E 로 확인). **아래는 그 당시의 기록이다** — 미결로 읽지 마라.

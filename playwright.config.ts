...
export default defineConfig({
   ...
   *
   * **60 → 90초** (M9_SRS FR-M9-16, 2026-09-14).
   *   이전 동작: 60초. 그런데 `fixtures.ts` 의 첫 관측 대기 하나가 **그 상한과
   *             같은 크기**로 자라 있었다 (20→30→45→60, 매번 전량 회차가 근거).
   *             그러면 그 대기가 상한을 다 쓰는 회차에서 테스트는 대기가 끝나기도
   *             전에 죽고, 실패 메시지는 `Test timeout of 60000ms exceeded` 한 줄로
   *             **무엇을 기다렸는지 말하지 않는다** (`git-head-mobile` V5·V6 실측)
   *   새  동작: 90초. 대기의 상한(60초)보다 크므로 대기가 시한에 걸리면 **그 대기가
   *             자기 이름으로** 실패한다
   *   이유:     상한 둘이 같은 크기면 안쪽 것이 바깥 것을 밀어낸다. 진단이 사라지는
   *             것이 그 대가이고, 그것은 상한을 두는 목적과 반대다
   */
  timeout: isWin ? 150_000 : 90_000,
  use: {

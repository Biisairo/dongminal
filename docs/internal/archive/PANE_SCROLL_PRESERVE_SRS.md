...
> **대체됨 (PANE_DOM_RECONCILE_SRS FR-PDR-22).** 이 문서의 FR-1~FR-4(갈무리 후
> 절대 위치 복원)는 `docs/internal/PANE_DOM_RECONCILE_SRS.md` 가 대신한다.
>
> 여기서 세운 보정은 **버퍼가 정지해 있다**를 전제했고, 출력이 흐르는 TUI 에서
> 그 전제가 깨져 화면이 스스로 맨 위로 올라가는 결함이 됐다. 새 문서는 보정을
> 정교하게 만드는 대신 그 보정이 필요했던 이유 — `render()` 가 레이아웃 DOM 을
> 다시 지으며 살아 있는 위젯을 옮기는 것 — 을 없앤다.

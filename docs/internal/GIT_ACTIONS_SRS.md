...
- **범위 밖(2026-08-27 사용자가 뺀 것)**: 커밋 **mute**(GG2) · 커밋 **author
  ...
  mute 와 author override 는 요구사항에서 **철회**한다 (D7·D10). 283~285 는 이 판이
  **착수하지 않았고**(D9), **2026-09-15 사용자 결정으로 셋 다 폐기됐다** — 아래 D11.
...
| 진행 중 job chip + 취소 | P2 | 취소는 ✅ (FR-GIT-102). **상태바 chip 은 철회** (FR-GIT-112 / U-19 ①) |
...
- **FR-GIT-277** **미커밋 행 메뉴.** `Stash`(생성 다이얼로그 재사용) ·
  ...
  - **Clean 은 확인한 목록만 지운다** (개정 2026-09-24, REPO_FIX 05 §3A-8 사용자 결정).
    - 요청은 `paths`(확인창에 보인 untracked 경로 배열)를 **반드시** 싣는다. 없거나
      비었으면 400 `bad_request` 이며 실행하지 않는다.
    - 서버는 실행 직전에 status 를 새로 찍어 그 untracked 목록(상한 절단 뒤의 것)과
      `paths` 를 **집합으로** 비교한다. 다르면 409 `stale_observation` 이며 실행하지
      않는다 — 확인 뒤 생긴·사라진 파일이 있다는 뜻이다.
    - 같으면 그 경로만 지운다: `git clean -q -f -d -- :(literal)<경로>…`. literal 인
      이유는 `*`·`?` 가 든 파일명이 다른 파일을 glob 으로 잡지 않게 하려는 것이다.
      경로 단위로 지우면 git 이 부모 디렉터리를 남기므로, 지운 경로의 조상 중
      **빈 것만** 저장소 최상위 아래까지 지운다(비지 않았으면 그대로 둔다).
    - 이전 동작: 서버가 목록을 받지 않고 실행 순간의 untracked 전부를 지웠다.
      이유: 확인 뒤~실행 사이에 생긴 파일이 보인 적 없이 지워질 수 있었다.

### 3.7 묶음 G — 부분 스테이징

- **FR-GIT-278** **hunk 단위 stage / unstage.** diff 뷰의 hunk 에 동작을 붙이고
...
> **개정 (2026-09-06, DIFF_HUNK_BAR_SRS I-1·I-2·I-3).** 위 두 요구의 **뜻은
> 그대로이고 표면만 바뀌었다.**
>
> - "hunk **머리에** 동작을 붙인다" 는 하단 목록(`.git-hunks`)이 조각마다 머리를
>   그리던 시절의 말이다. 그 목록은 같은 diff 를 두 자리에 그렸고 세로 공간의
>   42% 를 먹었다(`style-git-views.css:805`) — 사용자가 "공간을 가린다" 고 접수한
>   것이 그것이다. 이제 동작은 모디파이드 에디터 위에 **hover 로 뜨는 content
>   widget** 이다 (FR-DHB-10~22).
> - 줄 범위의 출처가 커스텀 줄 선택 UI(`.git-hunk-line` 클릭·Shift·`Clear`)에서
>   **Monaco 텍스트 선택**으로 바뀌었다 (FR-DHB-30~38). 사용자가 배워야 하던
>   조작이 하나 사라진다.
> - **서버 규약은 한 글자도 바뀌지 않는다** — 클라이언트가 보내는 것은 여전히
>   `(파일, 축, hunk 번호, 줄 범위, 관측 식별자)` 뿐이고 패치는 서버가 만든다
>   (D6). 그래서 V204 도 그대로다.
...
> **V205·V206 의 조작이 바뀌었다 (2026-09-06, DIFF_HUNK_BAR_SRS §7).** 두 항목이
> **무엇을 재는지는 그대로다** — 실제 저장소의 index·워킹 트리에서 확인한다. 다만
> 그 조작이 `.git-hunk-line` 클릭·`.git-hunk-act` 버튼에서 **hover 툴바와 Monaco
> 선택**으로 옮겨갔고, e2e 는 `V-DHB-2~9` 가 이어받는다.
>
> 옛 G8("줄 선택이 조각을 넘지 않는다")은 뜻이 바뀌었다: 선택은 이제 에디터의
> 것이므로 막을 수 없고, 대신 **적용 범위가 hunk 경계에서 잘린다** (FR-DHB-32,
> V-DHB-9). 근거는 서버 `patch` 가 hunk 번호를 하나만 받는다는 것이다.
| **V211** | GIT-276 | 단위·e2e | porcelain 을 줄과 커밋으로 가른다(메타는 커밋당 한 벌). 파일 메뉴의 Blame 이 Diff 탭을 blame 모드로 열고, 미커밋 줄이 구분되며, 실패는 **Diff 탭 안에서** 사유를 보인다. 잘린 blame 은 413 이다 |
...
| **D11** | **283·284·285 를 요구사항에서 폐기한다** | 2026-09-15 사용자 결정. D9 가 남겨 둔 "다음 판" 이 오지 않는다 — 셋 다 대체 경로가 있고 그것으로 충분하다는 판정이다. **283** merge editor: 충돌은 파일 단위 `ours`/`theirs` 로 해결된다. **284** 인터랙티브 rebase: `--continue`/`--abort` 가 있고 todo 편집은 터미널의 일이다. **285** clone: 터미널의 `git clone` 한 줄이며, `init` 절반은 이미 서 있다(REPO_TAB_UNIFY). 되살리려면 이 결정을 먼저 연다 |
...
| I 최상위 난이도 | ~~283 merge editor · 284 인터랙티브 rebase · 285 clone~~ | ⊘ **폐기** (D11, 2026-09-15). `init` 절반은 아래대로 이미 섰다 |
...
**남은 요구사항은 없다.** 접수한 말의 본체(브랜치·태그·커밋의 기본 동작)는 전부 섰고,
최상위 난이도 셋은 **D11 로 폐기됐다**(2026-09-15). 종전에 여기 적혀 있던 "남은 것은 5개"
중 둘은 D7·D10 으로 철회, 셋은 D11 로 폐기다.

**이 판의 범위였던 묶음 H 셋이 끝났다** (2026-08-27): `280 → 282 → 276`.
셋 다 읽기와 화면이며 **새 쓰기 동작은 하나도 열지 않았다** — author override 를
...
|---|---|---|---|
| ~~**276**~~ | Blame | ✅ 완료. `query/blame.go` · `gitapi/handlers_git_blame.go` · `panel.js` 의 blame 모드. `blame` 을 읽기 가드에 더했다 | — |
| ~~**280**~~ | reflog 포함 | ✅ 완료. `log --reflog` 한 인자 — 읽기 가드를 넓히지 않았다 | — |
| ~~**282**~~ | 리포 전환 드롭다운 | ✅ 완료. 헤더의 리포명이 전환 자리다. `GitMenu.openList` 로 동적 항목을 띄운다 | — |
...
| 날짜 | 내용 |
|---|---|
| 2026-08-27 (4) | **FR-GIT-282·276 완료.** 이 판의 범위(묶음 H 셋)가 끝났다. Blame 은 Diff 탭의 모드이고(D8), 검증 항목 **V211** 을 새로 세웠다 |

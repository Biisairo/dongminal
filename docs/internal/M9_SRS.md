...
| M9-B16 | **에이전트 GUI 의 정보가 너무 없다** (사용자, 2026-09-14) — *"context window 사용량, 주간, 5시간 사용량 등 가능한 정보를 전부 표시해 주어야 한다"* | `agent-pane.js` · `ALERT_MOBILE_CONTEXT_SRS`(`ContextTokens`) · **`production/M9_P7_GUI_SURVEY.md` §1·§3** — 컨텍스트 창·비용은 **이미 오고 이미 그린다**, 5시간·주간 한도는 프로토콜이 **주지 않는다** | M |

...
func Stage(s *core.Service, ctx context.Context, repo string, paths Paths) (core.Output, error) {
	clean, err := checkPaths(paths)
	if err != nil {
		return denied(), err
	}
	out, _, err := execPaths(s, ctx, repo, clean, false, len(clean), 0, "add")
	return out, err
}
...
func Unstage(s *core.Service, ctx context.Context, repo string, paths Paths) (core.Output, error) {
	clean, err := checkPaths(paths)
	if err != nil {
		return denied(), err
	}
	...
	out, _, err := execPaths(s, ctx, repo, clean, false, len(clean), 0, lead...)
	return out, err
}
...
func Discard(s *core.Service, ctx context.Context, repo string, tracked, untracked Paths) (core.Output, error) {
	if len(tracked) == 0 && len(untracked) == 0 {
		return denied(), fmt.Errorf("%w: 경로가 없다", core.ErrUnsafeArgument)
	}
	...
	applied := 0
	if len(ct) > 0 {
		out, applied, err = execPaths(s, ctx, repo, ct, true, total, 0, "checkout", "-q")
		if err != nil {
			return out, err
		}
	}
	if len(cu) > 0 {
		out, _, err = execPaths(s, ctx, repo, cu, true, total, applied, "clean", "-q", "-f")
		if err != nil {
			return out, err
		}
	}
...
func discardHint(repo string, tracked, untracked Paths) core.Hint {
	...
	return core.Hint{Repo: repo, Action: core.ActionDiscard, Targets: targets, Note: note}
}
...
// 적용됐는지 말할 수 없게 된다.
func execPaths(s *core.Service, ctx context.Context, repo string, paths Paths, destructive bool, total, offset int, lead ...string) (core.Output, int, error) {
	...
	for len(paths) > 0 {
		...
		out, err := s.ExecWrite(ctx, repo, core.WriteSpec{Argv: argv, Destructive: destructive})
		if err != nil {
...
func checkPaths(paths Paths) (Paths, error) {
	if len(paths) == 0 {
		return nil, fmt.Errorf("%w: 경로가 없다", core.ErrUnsafeArgument)
	}
...
func checkEach(paths Paths) (Paths, error) {
	...
	for _, p := range paths {
		rel, err := core.RelPath(p, core.ErrUnsafeArgument)
		if err != nil {
			return nil, err
		}
		out = append(out, rel)
...
func denied() core.Output { return core.Output{ExitCode: -1} }

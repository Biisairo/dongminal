...
**개정됨 — [`./EDITOR_FIND_PANEL_SRS.md`](./EDITOR_FIND_PANEL_SRS.md) FR-EFP-25.**
파일 내 검색은 **우리 패널을 만들며 Monaco 의 find 위젯을 쓰지 않는다.**

이 조항의 전제("위젯이 이미 그 자리에 있다")가 무너졌기 때문이다. 위젯은 Monaco 가
**우리보다 먼저** 키를 보고 열지만(안쪽 배선이 bubble 이다), 곧이어 우리 `_edFindInFile`
이 `ed.focus()` 로 그 포커스를 본문으로 되돌렸다. 그래서 위젯은 떠 있는데 글자를 받지
못하고 — **타이핑이 문서에 삽입됐다.** 검색하려고 누른 키가 파일을 편집한 것이다
(그 SRS §2.4 의 재현 표).

위젯을 고쳐 쓰는 길(`ed.focus()` 를 지우는 것)도 있었으나, 그러면 껍데기의 생김새·
자리·용어를 Monaco 가 정하고 우리 검색 셋 중 하나만 남의 것이 된다 (그 SRS D-1).

FR-EKB-1·2·4·5·6 은 그대로다 — 키는 여전히 두 자리에 배선되고(다만 안쪽이 **capture**
로 옮겨간다), 설정에서 바꿀 수 있으며, Editor 창이 아니면 삼키지 않는다.
...
| ~~(FR-EKB-3 의 검증은 없었다)~~ | FR-EKB-3 | **그것이 §2.4 결함이 살아남은 이유다** — EDITOR_FIND_PANEL_SRS V-EFP-1~3 이 그 자리를 메운다 |

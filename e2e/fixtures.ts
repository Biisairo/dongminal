...
/**
 * UIUX_OVERHAUL_SRS FR-CHR-2 · FR-CHR-10 (D-6): **분할은 pane 탭줄의 고정 구에
 * 있다.** 상단바가 돌아왔지만 분할은 돌아가지 않았다 — 그것은 **이 pane** 을
 * 쪼개므로 pane 의 자리에 산다. 상단바가 드는 것은 전역 동작뿐이다.
 *
 * 자리가 pane 의 것이므로 **어느 칸을 쪼개는지 셀렉터가 말한다** — 옛 전역
 * id(`#split-h`)는 활성 창의 포커스 칸을 암묵으로 겨눴다. 기본값
 * (`.pn.focused`)이 그 옛 뜻과 같고, 다른 칸을 겨누려면 `paneSel` 을 준다.
 */
...
export async function openGit(page: any, repo: string) {
  ...
  //
  // ── 여기서 멈춘다 (M10_SRS §2.7 · D-M10-6, 2026-09-15) ──────────────────
  //
  // **60초도 넘겼다.** `git-history` H15 가 60.9초로 걸렸다. 위의 이력이 보이는
  // 그대로다 — 20 → 30 → 45 → 60 → 초과. **늘리는 것으로는 끝나지 않는다.**
  //
  // 같은 회차에 동시 워커를 8에서 4로 되돌려(FR-M10-5) 부하성 flaky 가 5 → 0~1 로
  // 줄었다. **빈도는 내려갔지만 이 한 자리의 무게는 그대로다** — 이 대기가 딛는

...
func TestApiRunMember(t *testing.T) {
	...
	code, out := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"writer","agent":"claude","id":"tab-b"}`)
	if code != http.StatusOK {
		t.Fatalf("want 200, got %d (%+v)", code, out)
	}
	...
	if code, _ := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"other","agent":"claude","id":"tool-b"}`); code != http.StatusConflict {
		t.Fatalf("도구 중복 등록 want 409, got %d", code)
	}
	// 없는 도구.
	if code, _ := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"x","agent":"claude","id":"no-such"}`); code != http.StatusNotFound {
...
func TestApiRunReport_AuthorityAndReasons(t *testing.T) {
	...
	_, member := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"writer","agent":"claude","id":"tool-a"}`)
	...
	code, out = postRun(t, s, "/api/runs/report",
		`{"memberId":`+jsonQ(memberID)+`,"outcome":"succeeded","summary":"했다"}`)
	...
	who.toolID = "tool-a"
	code, out = postRun(t, s, "/api/runs/report",
		`{"runId":`+jsonQ(runID)+`,"memberId":`+jsonQ(memberID)+`,"outcome":"succeeded","summary":"했다. 봤다. 남았다.","files":["a.go","b.go"]}`)
	...
	// outcome 누락.
	who.toolID = "tool-b"
	_, _ = postRun(t, s, "/api/runs/members", `{"runId":`+jsonQ(runID)+`,"role":"r2","agent":"claude","id":"tool-b"}`)
...
func TestApiRunReport_FallsBackToClaimedToolID(t *testing.T) {
	...
	runID := startRun(t, s)
	_, _ = postRun(t, s, "/api/runs/members", `{"runId":`+jsonQ(runID)+`,"role":"w","agent":"claude","id":"tool-a"}`)
...
func TestApiRunStatus_DerivesMemberState(t *testing.T) {
	...
	runID := startRun(t, s)
	_, _ = postRun(t, s, "/api/runs/members", `{"runId":`+jsonQ(runID)+`,"role":"a","agent":"claude","id":"tool-a"}`)
	_, _ = postRun(t, s, "/api/runs/members", `{"runId":`+jsonQ(runID)+`,"role":"b","agent":"claude","id":"tool-b"}`)
...
func TestApiRunMember_MarksWorkspaceBestEffort(t *testing.T) {
	...
	if code, out := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"a","agent":"claude","id":"tab-a"}`); code != http.StatusOK {
	...
	// close 는 표식을 되돌린다.
	_, _ = postRun(t, s, "/api/runs/report", `{"outcome":"succeeded","summary":"ok"}`)
	if code, out := postRun(t, s, "/api/runs/close", `{"runId":`+jsonQ(runID)+`}`); code != http.StatusOK {
...
func TestApiRunMember_WorksWithoutWorkspaceStore(t *testing.T) {
	...
	runID := startRun(t, s)
	if code, out := postRun(t, s, "/api/runs/members",
		`{"runId":`+jsonQ(runID)+`,"role":"a","agent":"claude","id":"tool-a"}`); code != http.StatusOK {

...
- **열기**: 상단 툴바의 **🧭 Agents** 버튼(Split V 옆). 우측 접이식 패널이 열리며, 핸들로 너비 조절. 열림/너비는 브라우저별로 저장.
- **카드**: pane 위치(세션·탭), 상태 아이콘(● 작업 중 / ✅ 완료 / ⌨️ 입력 대기 / ⏸️ 멈춤), 툴 라벨, 명령어/파일 **원문**(로컬·본인용이라 마스킹하지 않음). 카드를 클릭하면 그 pane 으로 바로 이동.

...
| M9-B16 | **에이전트 GUI 의 정보가 너무 없다** (사용자, 2026-09-14) — *"context window 사용량, 주간, 5시간 사용량 등 가능한 정보를 전부 표시해 주어야 한다"* | `agent-pane.js` · `ALERT_MOBILE_CONTEXT_SRS`(`ContextTokens`) · 사용량의 출처(프로토콜이 주는 것과 주지 않는 것을 먼저 가른다) | M |

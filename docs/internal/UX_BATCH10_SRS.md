...
| FR-UXB-27a | **소유권 복귀도 같다.** 남이 놓아 돌아온 화면은 render 와 함께 job 도 되살린다 — 아래 개정을 본다. | 필수 |
| FR-UXB-27b | 그 판정은 **소유권 맵**으로 한다. dim 이 걸렸는지(DOM)를 묻지 않는다. | 필수 |
...
> **개정 2026-09-22 (D-UXB-9) — 복귀의 계기가 셋인데 하나만 반쪽이었다.**
>
> 접수: *"포커스를 뺏었다가 돌려오는 동작에서 포커스가 돌아오긴 하나 재렌더가
> 되지 않는다. 창을 클릭하면 재렌더된다."* 조건은 칸 여럿·포커스 칸·상대가 놓아서
> 복귀·OS 포커스 있음이었고, 낡은 채로 남는 것은 터미널만이 아니었다.
>
> FR-UXB-26 은 *"돌아온 화면이 옛 그림인 것은 두 가지가 겹친 결과다 — 렌더가
> 돌지 않고, 폴링도 다시 돌지 않는다. **둘 다 여기서 한 번에 푼다**"* 로 그 둘을
> 묶었다. 그런데 그 "한 자리" 가 **OS 포커스·가시성 경로뿐**이었다:
>
>     _onLifeResume()     render() + TIMERS.revalidate()
>     _onWindowFocus(a)   render()                        ← 되살림이 없다
>
> 소유권으로 돌아오는 길은 **그리기만 하고 데이터는 낡은 채**다. 그래서 다음
> 주기가 올 때까지 옛 값이 남고, 그 사이 사용자가 보는 것은 render 가 지난 뒤의
> **낡은 상태로 그린 새 그림**이다.
>
> **FR-UXB-27 의 근거가 그대로 적용된다** — *"두 창을 오갈 때 `document.hidden`
> 은 거짓이라 아무 job 도 다시 돌지 않는다."* 다른 기기의 화면이 창을 쥐었다
> 놓는 것도 같은 성질이고, 그쪽에는 `visibilitychange` 조차 오지 않는다.
>
> 되살림의 조건은 render 와 같다 — **dim 이 풀린 칸이 있을 때만**이다. 소유권
> 맵은 남의 이동으로도 오므로, 맵이 올 때마다 job 을 쏘면 화면 수만큼 폴링이
> 늘어난다.

> **재개정 2026-09-22 (D-UXB-10) — 판정이 DOM 을 되묻고 있었다.**
>
> 위 개정을 올린 뒤 실배포에서 **여전히 돌아오지 않았다** (접수). TUI 만의 일도
> 아니었다 — 일반 화면도 같았다.
>
> 가드가 `applyFocusOverlay()` 의 반환값, 즉 *"dim 이 **풀렸나**"* 였다. 그 값은
> **DOM 의 `pn-dimmed` 클래스**를 훑어 얻는다:
>
>     if(!this.applyFocusOverlay()) return;   // 여기서 조용히 나갔다
>     this._onLifeResume();
>
> 클래스가 기대대로 붙어 있지 않으면 — 렌더 시점이 어긋났거나, 슬롯 신원이 맞지
> 않았거나 — 소유권이 실제로 돌아왔는데도 거짓이 되고 **아무 일도 일어나지
> 않는다.** 화면에 보이는 사실을 DOM 에 되물어 확인하는 구조라, 그 되물음이
> 틀리는 순간 조용히 넘어간다.
>
> **판정을 맵으로 옮긴다** (FR-UXB-27b). 소유권 맵이 바뀌었으면 돈다 — dim 이
> 걸렸는지는 묻지 않는다. 사용자 제안 그대로다: *"포커스가 돌아오는 건 확인이
> 가능하니 이 시점에서 재렌더를 무조건 하면 되는 것 아닌가."*
>
> 같은 맵의 재전송에는 돌지 않는다. 서버는 **어느 화면의 이동에도 전체 맵**을
> 뿌리므로, 내 쪽이 달라진 것이 없으면 되살릴 이유도 없다. 반대로 맵이 두 번
> 바뀌면 두 번 돈다 — 컨텍스트 종료가 그렇다(반납 한 번, 구독이 끊겨 서버가
> 해제하며 한 번). 횟수를 못박지 않는 이유다.
>
> **`app-focus.js` 가 갈라졌다.** 이 변경이 그 파일을 500줄 위로 밀었고
> (`FE_MODULE_BOUNDARY_SRS` FR-STR-40), 가른 자리는 그 파일의 머리 주석이 이미
> 적어 둔 경계다 — *"Ownership transport lives in `_focusClaim` (out) and the
> `window_focus` SSE branch (in)"*. 소유권 묶음이 `app-focus-owner.js` 로 간다.

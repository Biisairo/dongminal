...
- `inline` (기본): 보스 CC 의 분할 칸 옆을 분할해 팀 배치. 빠르지만 사용자 화면이 좁아진다.
- `dedicated`: 워크플로우 이름의 **전용 창**을 백그라운드로 생성해 그 안에 팀 배치. 사용자 화면 완전 무손상, 사이드바에서 잡 진행을 관전 가능, 같은 워크플로우 여러 개 병렬 실행 가능. 해체 시 팀원 탭이 모두 닫히면 창도 자동 제거.

 ...
 *
 * ## 이 파일은 **테마 밖**이다 (WORDING_COLOR_SRS FR-WRD-33 / D-WRD-8)
 *
 * 아래 주입 스타일은 색 아홉·`z-index:9999`·`font:10px` 로 토큰 체계 셋을
 * 동시에 지난다. **그것이 의도다**: 이 화면은 테마 파생이나 CSS 로드가 깨졌을
 * 때에도 읽혀야 하고, 테마에 기대면 바로 그 순간 못 읽는다. 진단 도구가
 * 진단 대상에 의존하면 도구가 아니다.
 *
 * 그래서 `check-hardcoded-color.mjs` 의 예외 등록부가 이 파일을 **사유와 함께**
 * 지난다 — 조용한 예외는 예외가 아니라 구멍이다. 이 화면이 테마와 무관한 별도
 * 문서로 분리되면 그 예외는 없어진다.
 */
(function(){

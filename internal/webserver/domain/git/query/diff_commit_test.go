...
func TestDiffCommit_RejectsOptionLikeRev(t *testing.T) {
	svc := core.New(core.WithRunner(func(_ context.Context, _ string, _ []string) (core.Output, error) {
...
func TestDiffCommit_RealGit(t *testing.T) {
	...
	run := func(args ...string) {
		t.Helper()
		if _, err := svc.ExecWrite(context.Background(), dir, core.WriteSpec{Argv: args}); err != nil {
	...
	dc, err := DiffCommit(svc, context.Background(), dir, second, first, "README.md", "")
	if err != nil {
		t.Fatalf("DiffCommit: %v", err)
	}
	...
	root, err := DiffCommit(svc, context.Background(), dir, first, "", "README.md", "")
	if err != nil {
...
func headOid(t *testing.T, svc *core.Service, dir string) string {
	t.Helper()

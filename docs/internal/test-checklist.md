...
> 작성일: 2026-05-05 · 갱신: 2026-08-20 (엔티티 모델 v2 어휘 + 제거된 표면 정리)  
> 범위: 백엔드(Go) + 프론트엔드(xterm.js) 전 동작  
> 형식: 각 동작은 하나 이상의 단위/통합 테스트로 커버되어야 함
>
> **제거된 표면**: code-server 통합과 markdown 뷰어는 `8dc0a3f` 에서 내장
> 편집기(Monaco)로 대체되며 사라졌다. 관련 항목(A6·C3·E5 섹션, `/api/code-server*`,
> `/api/md-file`, `MdViewer`)은 이 갱신에서 삭제했다.
...
| A1.8 | `shouldLogRequest` — 핫패스 필터링 | 400+는 항상 로그; /api/ping, /api/stats, /api/workspace/*, /api/tools/*는 200대에서 필터 |

### A2. WebSocket 핸들러 (handleWS)
| # | 동작 | 엣지/실패 케이스 |
|---|------|------------------|
| A2.1 | toolID 쿼리 없음 → 새 도구 생성 | `s.Tools == nil` → 500 |
| A2.2 | toolID 쿼리 있음 → 기존 도구 재연결 | 존재하지 않는 toolID → OpError |
| A2.3 | 기존 도구 재연결 시 snapshot 전송 | snapshot 비어있는 경우; snapshot이 bufMax 초과하지 않는지 |
| A2.4 | restored 도구 재연결 시 reset 시퀀스 전송 | restored=false인 도구에는 전송되지 않음 |
| A2.5 | 재연결 시 resize(cols, rows) 호출 | |
| A2.6 | `OpToolID` 전송으로 클라이언트에 도구 id 전달 | |
| A2.7 | 클라이언트 연결/해제 시 로깅 | panic recover |
| A2.8 | `readWS`: `OpInput` → `ptmx.Write` | ptmx Write 에러 시 종료 |
| A2.9 | `readWS`: `OpResize` → `tool.resize` | 메시지 길이 < 5이면 무시 |
| A2.10 | `readWS`: 빈 메시지 무시 | |
| A2.11 | `readWS`: 정상 Close, GoingAway, NoStatusReceived → 조용히 종료 | 기타 에러는 로그 |
| A2.12 | `pingLoop`: 주기적 Ping 전송 | writePing 실패 시 종료; 도구 done 시 종료 |
| A2.13 | WebSocket 업그레이드 실패 처리 | |

### A3. API 핸들러 (handleAPI)
| # | 동작 | 엣지/실패 케이스 |
|---|------|------------------|
| A3.1 | `GET /api/state` — tools + workspace JSON | `s.Tools == nil` → 500; workspace nil → null; ETag = rev |
| A3.2 | `POST /api/tools` — 새 도구 생성 | `s.Tools == nil` → 500; cwd 쿼리 우선; cwdTool 참조로 CWD 상속; 생성 실패 → 500 |
| A3.3 | `GET /api/tools/{id}/busy` — Busy 상태 | 도구 없음 → `{"busy": false}` |
| A3.4 | `DELETE /api/tools/{id}` — 도구 삭제 | `s.Tools == nil`도 200 반환 (no-op) |
...
| A4.2 | `StartTool` — 환경변수 설정 | TERM, COLORTERM, LANG, PATH(DONGMINAL_HOME/bin 추가), HOME, USER/LOGNAME, SHELL |
| A4.3 | `StartTool` — zsh용 ZDOTDIR 설정 | |
| A4.4 | `StartTool` — bash용 BASH_ENV 설정 | |
| A4.5 | `StartTool` — cwd 결정: 인자 > home > "." | 존재하지 않거나 파일이면 home |
| A4.6 | `StartTool` — PTY 시작 및 프로세스 생성 | pty.StartWithSize 실패 |
| A4.7 | `Tool.IsBusy` — pgrep -P로 자식 프로세스 확인 | cmd/process nil → false; pgrep 에러 → false |
| A4.8 | `Tool.Cwd` — /proc/PID/cwd 읽기 | 실패 시 lsof 폴백; 둘 다 실패 시 os.Getwd |
| A4.9 | `Tool.readPTY` — 출력 읽고 broadcast | EOF → OpExit + kill + onExit; io error → 동일; 기타 에러 → 동일 |
| A4.10 | `Tool.readPTY` — bch 채널 full 시 drop | drop 로그 |
| A4.11 | `Tool.drainBuf` — stream.Feed | panic recover |
| A4.12 | `Tool.broadcast` — 모든 클라이언트에 writeMsg | 개별 실패 시 removeClient + close |
| A4.13 | `Tool.addClient/removeClient` — 동시성 안전 | cmu Lock |
| A4.14 | `Tool.resize` — pty.Setsize | 에러 로그 |
| A4.15 | `Tool.kill` — once.Do로 중복 방지 | SIGTERM → sleep 50ms → SIGKILL → Wait → stream.Close |
...
| A5.3 | `Create` — ID 할당(nextID++) 및 StartTool 호출 | StartTool 실패 시 로그; dirty.Store(true); SaveAll 비동기 |
| A5.4 | `Restore` — 기존 ID로 StartTool, restored=true | nextID 동기화; 실패 시 에러 반환 |
| A5.5 | `Get` — id로 도구 조회 | 없으면 nil |
| A5.6 | `List` — 정렬된 도구 목록 | PID 포함 |
| A5.7 | `Delete` — 도구 map에서 제거 후 kill | nil 도구여도 panic 없음; dirty.Store(true); SaveAll 비동기 |
| A5.8 | `IsLive` — 도구 존재 여부 | |
| A5.9 | `SaveAll` — dirty=false면 스킵 | tools.json 에 ToolState 배열 저장; 정렬. **탭이 참조하는 도구만** 기록(백그라운드 도구 제외, FR-EM-12 / FR-BG-9) |
| A5.10 | `LoadAll` — tools.json 로드 및 Restore | 파일 없음 → 무시; unmarshal 실패 → 로그; 미참조 항목은 복원하지 않음 (FR-EM-14) |
...
| B2.16 | `buildIndex` — 라벨 생성 규칙 | W{window+1}.P{pane+1}.T{tab+1} |
...
| B4.2 | `read_screen` — Snapshot 반환 | 도구 없음; base64 인코딩; bytesDrop 포함 |
| B4.3 | `read_output` — Snapshot 반환 | 도구 없음; lines 파라미터; ansi 필터링 |
| B4.4 | `send_input` — 텍스트 입력 | 도구 없음; submit 시 \r 추가 |
| B4.5 | `send_agent_message` — 에이전트 메시지 전송 | 도구 없음; commandHub Broadcast |
| B4.6 | `who_am_i` — 클라이언트 PID → 도구 매칭 | clientPID 조회 실패; 조상 체인 32단계; 매칭 실패 |
| B4.7 | `workspace_command` — 허용된 액션 브로드캐스트 | unknown action 거부 |

### B5. Adapters (internal/adapters/)
| # | 동작 | 엣지/실패 케이스 |
|---|------|------------------|
| B5.1 | `Tool.List` — mcptool.ToolInfo 변환 | |
| B5.2 | `Tool.Has` — 존재 여부 | |
| B5.3 | `Tool.Snapshot` — 데이터 + bytesDrop | 도구/stream nil → false |
| B5.4 | `Tool.Size` — pty.Getsize | 도구/ptmx nil → "?"; 에러 → "?" |
| B5.5 | `Tool.SendPaste` — bracketed paste | 도구 없음; submit 시 120ms sleep + \r |
...
| C1.6 | `connect` — WebSocket 연결 | toolID 포함; binaryType='arraybuffer'; onopen 시 resize 재전송 |
...
| C4.2 | `applyThemeObj` — 터미널 테마 동기화 | 모든 도구의 term.options.theme 갱신 |
...
| C7.2 | 창 삭제 — `closeWindow` | 마지막 창이면 새 창 자동 생성 |
| C7.3 | 창 전환 — `windowNext/Prev` | 순환 |
| C7.4 | 탭 생성 — `newTab` | 현재 분할 칸에 추가; 새 도구 API 호출 |
| C7.5 | 탭 삭제 — `closeTab` | 마지막 탭이면 분할 칸 제거; 도구 delete API 호출 |
| C7.6 | 탭 전환 — `tabNext/Prev` | 순환 |
| C7.7 | Split H/V — 현재 탭을 분할 | 새 도구 API 호출; layout 트리 업데이트 |
| C7.8 | 분할 칸 포커스 이동 — `paneUp/Down/Left/Right` | 인접 분할 칸으로 포커스 |
| C7.9 | 레이아웃 트리 — split/pane 노드 | direction(h/v), children, tabs, activeTab |
| C7.10 | 드래그 앤 드롭 — 탭 이동 | 분할 칸 간 이동; 순서 변경 |
| C7.11 | 프리셋 저장 — 현재 layout 트리 직렬화 | name 입력; layoutPresets 배열에 추가 |
| C7.12 | 프리셋 로드 — 저장된 트리 복원 | 도구 생성 필요 시 API 호출 |
...
| C10.2 | 창 클릭 — 전환 | |
| C10.3 | `+ New` 버튼 — 새 창 | |
| C10.4 | `★ Preset` 버튼 — 프리셋으로 새 창 | defaultPreset |
...
> 새탭/새 분할 칸/새 창/분할/닫기/전환 시 `app.focused`, `window.focusedPane`, `activeWindow`, `pane.activeTab`, DOM, 상태바, 검색, 모바일 인덱스가 일관되게 업데이트되는지 검증.
> **코드상 버그로 인해 `window.focusedPane`이 동기화되지 않는 경우가 다수 있음** — 테스트에서 반드시 감지해야 함. (E2E `regression-focus.spec.ts` 가 이 불변식을 검증한다)
...
| # | 시나리오 | 검증 포인트 |
|---|----------|-------------|
| D1.1 | 서버 시작 → 브라우저 접속 → 새 창 → 명령 실행 | WS 연결, 출력 수신, 상태바 갱신 |
| D1.2 | Pane 분할 → 탭 이동 → Pane 닫기 | layout 트리 일관성, API 호출 |
| D1.3 | 브라우저 새로고침 → 기존 도구 재연결 | snapshot 수신, 출력 연속성 |
...
| # | 테스트 | 검증 방법 |
|---|--------|-----------|
| E1.1 | 서버 기동 → `/` 접속 → 초기 창 1개 확인 | `#area .pn` 존재, `.pn-tabs .pn-tab` 1개, `.pn.focused` 존재 |
...
| E1.4 | 페이지 새로고침 → 기존 도구 재연결 | 창 이름 유지, WS 메시지에서 snapshot 수신 (Network/WS 로그) |

### E2. 포커스 이동 (사용자 요구 핵심)
| # | 테스트 | 검증 방법 |
|---|--------|-----------|
| E2.1 | **새 창 생성** → 포커스 이동 | `+ New` 클릭 → `.pn.focused` 개수=1, 활성 창 이름 변경, 상태바 CWD 초기화, **`window.focusedPane` 누락 여부** (API PUT body에서 검증) |
| E2.2 | **새 탭 생성** → 포커스 이동 | 분할 칸 내 `+` 버튼 클릭 → `.pn-tab.active` = 새 탭, `.pn.focused` 유지, 이전 탭 비활성화 |
| E2.3 | **Split H** → 포커스 우측 이동 / **Split V** → 포커스 하단 이동 | `Split H` → `.pn` 개수=2, `.pn.focused` = 우측(신규); `Split V` → `.pn.focused` = 하단(신규) |
| E2.4 | **Split (keepFocus)** → 포커스 유지 | dmctl/SSE로 `splitV` + keepFocus → `.pn.focused` = 기존 분할 칸 유지, API PUT body에서 `focusedPane` 변경 없음 확인 |
| E2.5 | **탭 닫기** — 마지막 탭 아님 | `×` 클릭 → `.pn-tab.active` = 남은 탭, `.pn.focused` 유지 |
| E2.6 | **탭 닫기** — 마지막 탭 (분할 칸 제거) | 유일한 탭 `×` 클릭 → `.pn` 개수 감소, `.pn.focused` = fallback 분할 칸, activeWindow 유지, **새 activeWindow의 `focusedPane` 설정 여부** 확인 |
| E2.7 | **창 전환** → 포커스 복원 | 사이드바 창 클릭 → `.si.active` 변경, `#area` 재렌더, `.pn.focused` = 해당 창의 focusedPane |
...
| E6.2 | dmctl → `splitH` 브로드캐스트 수신 → 실행 | 외부 HTTP POST → 브라우저 SSE 수신 → DOM에 새 `.pn` 생성 |
...
분할 칸의 활성 탭" 이 터미널이 아니게 되고 대기가 타임아웃한다.

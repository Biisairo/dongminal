...
### 3.5 묶음 M — 구 식별자 재작성 (마이그레이션)

묶음 U 까지는 **구 id 보존**이 규약이었고(FR-WID-2·FR-UNI-9), §5 는 마이그레이션을
비목표로 두었다. 본 묶음이 그 비목표를 **개정**한다 — 근거는 §6 의 결정 기록
(2026-08-25). 보존 규약 자체는 유지된다: 묶음 M 은 **사용자가 명시적으로 실행하는
1회성 변환**이지, 런타임이 자동으로 수행하는 동작이 아니다.
...
| 파일 | 재작성 대상 | 갱신할 참조 |
|---|---|---|
| `workspace.json` | `windows[].id`, 레이아웃 노드 `id`(재귀), `tabs[].id`, `tabs[].toolId` | `activeWindow`, 창별 `activeTab`, `focusedPane`, `agentsOrder[]` |
| `tools.json` | `[].id` | — (`workspace.json` 의 `tabs[].toolId` 가 참조자다) |

**FR-MGU-2** 아래는 **비대상**이다. 근거는 각 행에 적는다.
...
| `bin/agent-hooks/*.json` | `dmctl` 커맨드 문자열만 담고 정체는 런타임에 조회한다 |

#### M-2. 판별과 멱등성

**FR-MGU-3** canonical uuid 형태(8-4-4-4-12)인 id 는 **재발급하지 않는다.** 그 외
식별자를 재작성 대상으로 본다. 따라서 변환은 멱등하고, uuid 와 구 id 가 혼재하는
현재 상태(FR-UNI-9)를 입력으로 받아 구 id 만 좁혀 고친다.

**FR-MGU-4** 새 id 는 `internal/uuid`(v7)로 발급한다 — FR-UNI-6 의 서버측 단일
생성기와 같다. 결정적 테스트를 위해 생성기는 주입 가능해야 한다.

#### M-3. 참조 무결성

**FR-MGU-5** 재작성은 **매핑을 먼저 완성한 뒤 일괄 치환**한다. 같은 구 id 는 파일
경계를 넘어 항상 같은 새 id 로 간다 (`tools.json` 의 `id` 와 `workspace.json` 의
`tabs[].toolId` 가 같은 매핑을 공유한다).

**FR-MGU-6** 매핑에 없는 참조(끊어진 참조)는 **재작성하지 않고 값을 보존**하며
리포트한다. 기존 `BrokenRefs` 와 같은 철학이다 — 참조 정리는 런타임 무결성
검사(FR-EM-14)의 책임이지 마이그레이션의 책임이 아니다.

**FR-MGU-7** 변환 산출물에는 구 형식 id 가 남지 않는다. 단, FR-MGU-6 이 보존한
끊어진 참조는 예외다.

#### M-4. 백업과 스키마 버전

**FR-MGU-8** 재작성 직전 상태를 `*.preuuid.bak` 으로 백업한다. 기존 `*.v1.bak` 과
**별도 접미사**여야 한다 — 이미 v2 인 사용자는 `.v1.bak` 이 이미 존재하고
`backupOnce` 가 무동작이므로, 같은 접미사를 쓰면 재작성 직전 상태가 어디에도 남지
않는다.

**FR-MGU-9** `schemaVersion` 은 **2를 유지한다.** 구조가 아니라 값만 바뀌고,
`web/js/app.js` 가 저장 시 `schemaVersion=2` 를 하드코딩하므로(`app.js:1600`) 3으로
올리면 브라우저의 첫 저장에 되돌아간다. 멱등성은 FR-MGU-3 의 형태 검사가 담보하므로
버전 표식이 필요 없다.

#### M-5. 리포트

**FR-MGU-10** 리포트는 재작성 건수를 종류별(창·분할 칸·탭·도구)로, 그리고
FR-MGU-6 이 보존한 끊어진 참조를 보고한다. `--dry-run` 은 이 리포트만 내고 파일을
건드리지 않는다.
...
> `LoadAll` 이 **새 id 로 셸을 respawn** 한다(`tool.go:933`·`954`). 즉 셸은 새
> 프로세스이며 `cwd` 만 보존된다. 이는 마이그레이션 고유의 손실이 아니다 — 마이그레이션은
> 데몬 정지 상태에서만 실행되고(FR-MGU-11), 정지 시점에 이미 모든 PTY 가 죽는다.
> 백그라운드 도구가 재기동을 넘지 못하는 것(FR-BG-9)도 같은 이유로 마이그레이션과
> 무관하다.
...
| TC-MGU-1 | FR-MGU-1/5/7 | 구 id 만 든 `workspace.json`+`tools.json` 으로 `Apply` | 모든 id 가 uuid. `activeWindow`·`activeTab`·`focusedPane`·`agentsOrder`·`tabs[].toolId` 가 새 id 를 정확히 가리킨다 |
| TC-MGU-2 | FR-MGU-3 | `Apply` 를 두 번 연속 실행 | 2회차는 어떤 id 도 바꾸지 않는다 (멱등) |
| TC-MGU-3 | FR-MGU-3 | uuid 와 구 id 가 혼재한 입력 | uuid 는 값 그대로, 구 id 만 재작성 |
| TC-MGU-4 | FR-MGU-6 | 도구 컬렉션에 없는 `toolId` 를 참조하는 탭 | 값 보존 + 리포트에 등재. 변환은 실패하지 않는다 |
| TC-MGU-5 | FR-MGU-8 | `.v1.bak` 이 이미 있는 v2 홈에서 실행 | `.preuuid.bak` 이 재작성 **직전** 내용으로 생성. `.v1.bak` 은 불변 |
| TC-MGU-6 | FR-MGU-1 | v1 입력으로 1회 실행 | v1→v2 변환과 id 재작성이 한 번에 끝난다 |
| TC-MGU-7 | FR-MGU-10 | `--dry-run` | 리포트에 건수가 나오고 파일은 무변경 |
| TC-MGU-8 | FR-MGU-4 | 생성기 주입 | 산출 id 가 결정적이고 v7 형식 |
| TC-MGU-9 | FR-MGU-9 | 변환 후 `schemaVersion` | 2 (올라가지 않는다) |
...
| ~~구 식별자 마이그레이션~~ | **개정됨 (2026-08-25).** 혼재가 무해하다는 판단은 유효하지만, 사용자가 1회성 정리를 선택했다. 묶음 M(§3.5)이 이를 덮는다. **런타임 보존 규약(FR-UNI-9·FR-WID-2)은 불변** — 재작성은 사용자가 명시적으로 실행하는 마이그레이션 경로에서만 일어난다 |
...
**결정 기록 (사용자, 2026-08-25) — 묶음 M**

| 결정 | 선택 | 대안과 근거 |
|---|---|---|
| 구 식별자를 남길 것인가 | **1회성 마이그레이션으로 정리** | 보존(기존 §5 비목표)은 혼재를 영구화한다. 사용자가 정리를 택했다. 런타임 보존 규약은 그대로 두고 마이그레이션 경로에서만 재작성한다 |
| 진입점 | **기존 `scripts/migrate.sh` 에 편입** | 별도 스크립트는 v1 사용자가 두 번 실행해야 하고 순서 의존이 생긴다. 편입하면 v1 사용자는 1회 실행으로 스키마와 id 를 함께 정리하고, 이미 v2 인 사용자는 id 만 정리된다 |
| `schemaVersion` 인상 | **하지 않는다 (2 유지)** | 구조가 아니라 값만 바뀐다. `app.js:1600` 이 저장 시 `schemaVersion=2` 를 하드코딩하므로 인상하면 브라우저 첫 저장에 되돌아간다. 멱등성은 uuid 형태 검사로 충분 |
| 백업 접미사 | **`.preuuid.bak` (신규)** | `.v1.bak` 재사용은 `backupOnce` 의 "이미 있으면 무동작" 때문에 v2 사용자의 직전 상태를 남기지 못한다 |
| 끊어진 참조 | **보존 + 리포트** | 마이그레이션이 참조를 지우면 런타임 무결성 검사(FR-EM-14)와 책임이 겹치고, 사용자가 손실을 알아챌 기회가 사라진다 |
...
| 이전 동작 | 새 동작 | 이유 |
|---|---|---|
| `dongminal migrate` 는 v1→v2 스키마만 변환 | 스키마 변환에 이어 구 식별자를 uuid 로 재작성 | 묶음 U 이후에도 기존 엔터티는 `s{n}`/`r{n}`/`t{n}`·정수 `toolId` 로 남아 혼재했다. `dmctl who-am-i` 가 `uuid=t201` 을 보이는 것이 그 표면이었다 |
| 마이그레이션 백업은 `*.v1.bak` 하나 | id 재작성 직전 상태를 `*.preuuid.bak` 으로 추가 백업 | `backupOnce` 는 백업이 이미 있으면 무동작이라, 이미 v2 인 홈에서는 직전 상태가 남지 않았다 |
